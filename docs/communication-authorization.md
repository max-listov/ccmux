...
### Откуда берётся значение для своего письма

У входящего письма оба UUID стоят в его теге. У собственного отправленного их даёт сама
отправка: успешная отправка сессии печатает строку `letter <peer thread uuid>#<message uuid>` —
это ссылка на только что принятое письмо, и она копируется в `sourceMessageRef` целиком, без
сборки из частей. Прозаическая ссылка у continuation отбивается как `reference-unreadable`, а
подмена основания на `user-instruction` записала бы в журнал не то, по которому шла переписка,
— поэтому значение печатается там, где оно возникает.

Названо только **принятое** письмо: задержанное до повторной попытки имеет id и запись в
outbox, но в ledger адресата ещё не лежит. Посмотреть позже — `ccmux msg sent [task] [--json]`:
отправленные этой сессией письма с адресатом, task, временем, состоянием и ссылкой. Состояние
отвечает на тот же вопрос, что и печать: `accepted` — письмо в ledger адресата (для сессии на
этой же машине это момент записи; когда его покажут в пейне, отвечает `msg pending`), `held` —
попытка ещё повторяется, `undelivered` — окно повторов истекло. Ссылку выдаёт только `accepted`:
у задержанного письма переписка не открыта. Письмо владельцу ссылки не имеет — для него основание
не требуется.

Ссылка принадлежит той жизни сессии, которая написала письмо: после restart или renew у той же
`machine:session` новый thread, и продолжение от новой жизни отклоняется. `msg sent` показывает
письма текущей жизни и отдельной строкой считает остальные, вместо того чтобы выдавать ссылки,
которые будут отклонены при использовании.
...
Ссылка на письмо, не дошедшее до адресата, отклоняется: у неудачной отправки на другую машину
есть и id, и запись, и полная расписка, и все остальные проверки на ней проходят — поэтому
переписку можно было бы объявить из письма, которого никто не получил. Originating host —
единственное место, где эта разница известна: ledger хранит уже принятое, outbound envelope
остаётся попыткой, пока транзит её не завершит.
