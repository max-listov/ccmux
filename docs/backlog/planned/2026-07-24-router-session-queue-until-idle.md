...
> Источник: голосовая постановка Max (2026-07-24), проработано в отдельной планирующей сессии.
> Строит эту фичу dev-сессия этого репо. **До явного «делай» от Max — только анализ/подготовка.**
...
Max работает с несколькими параллельными Claude-сессиями. Наговаривает войсом задачу, отправляет,
...
**Что нужно Max:** добавка приходит в целевую сессию **только когда агент ДОБРОВОЛЬНО завершил turn**,
...
эскалирует к Max только при реальном затыке — без «продолжить?».
...
3. **Hook и demon покрывают ДИЗЪЮНКТНЫЕ состояния, ни один не «primary/backstop»:**
   - **Stop hook физически НЕ стреляет по уже-idle сессии** (нет завершающегося turn'а). А это ровно
     частый кейс Max — follow-up часто прилетает *после* того как turn закончился. Hook-only → молча
...
4. **«Как будто написал человек» конфликтует с peer-trust границей.** Follow-up **исходит от
   владельца** (Max продиктовал роутеру) → его авторитет = user-level. Но `from=router` рендерит его
...
```
  Max (голос: мобилка Claude / терминал)  ──"добавь таргету: как доделаешь — сделай X"──▶
  ...
  │  router (Sonnet) │  ├─ ок  → закрыть, Max не дёргать
...
per-recipient `mkdir`-лок (паттерн уже есть в клоде-синке флота), если захочется belt-and-suspenders.
...
8. **Anti-nag (жёстко, топ-раздражитель Max):** НИКОГДА не пинговать owner «продолжить?/сделать?/готово?»;
...
| R2b | Как inline `--settings` мержит **массив** `hooks.Stop` с глобальным `~/.claude/settings.json` (у Max есть глобальный PostToolUse sync-hook) — replace или concat? | ⚠️ **ПРОВЕРИТЬ ЭМПИРИЧЕСКИ до кода** (пробник P1). Может тихо затереть юзерские хуки |
...
| N8 | Telegram-mirror зеркалит defer владельцу немедленно (`telegram.ts:17`) — Max видит каждый relay | Решить: гейтить mirror на `!defer` или рендерить отлично |
...
1. **P1 — merge/clobber: ✅ PASS.** `--settings '{"hooks":{"Stop":[…]}}'` — наш Stop выстрелил
   **И** одновременный проектный PostToolUse-хук тоже (`posttool.fired`). Мерж **по-событийно**,
   другие события не затираются → глобальный PostToolUse-sync-хук уцелеет. (Строго-решающий вариант —
...
- **Осталось:** ship в прод (по approval Max — правило репо: не шипить без явного «да») → затем `git mv` в `done/`.
