...
| R2b | Как inline `--settings` мержит **массив** `hooks.Stop` с глобальным `~/.claude/settings.json` (у the maintainer есть глобальный PostToolUse sync-hook) — replace или concat? | ⚠️ **ПРОВЕРИТЬ ЭМПИРИЧЕСКИ до кода** (пробник P1). Может тихо затереть юзерские хуки |
