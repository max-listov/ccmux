...
- [ ] Первый CI-релиз (v0.1.8+) после коммита: проверить тег==код, гейт валит красное,
      флот подхватил
- [ ] Ремедиация тега v0.1.7 (после коммита текущего дерева, по команде)

## Acceptance v2

- [ ] Релиз невозможен из грязного дерева и без зелёного check (проверить оба отказа)
- [ ] `git diff vA..vB` показывает ровно вошедшее в релиз
- [ ] Паблиш работает с любой машины без локального gh (вся публикация в CI)
- [ ] Флот подхватывает CI-релиз так же, как подхватывал локальные (≤300с, sha ок)
