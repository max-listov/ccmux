...
defrost: по решению владельца. Это отдельный продукт (пункт 3 траектории VISION), а не задача на вечер; размораживать под явное решение делать клиент, а не попутно.
---

> **Заморожено 2026-08-25 по решению владельца.** Заодно исправлено расхождение реестра: файл лежал
> в `inbox/` со статусом `planned` при пустом `planned/`, а ссылка `related` указывала в `inbox/` на
> записку, которая давно в `icebox/`.
