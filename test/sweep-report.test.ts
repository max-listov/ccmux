...
test('a caller that did NOT come back is named out loud to the owner', () => {
  const r = sweepReport('host-a', 'agent-a', ['agent-b', 'agent-a'], null, {
    known: true,
    running: false,
    canChat: true,
  });
...
test('a caller that cannot receive chat is a different sentence from one that is gone', () => {
  const mute = sweepReport('host-a', 'agent-a', ['agent-a'], null, {
    known: true,
    running: true,
    canChat: false,
  });
  expect(mute.body).toContain('cannot receive chat');
  const gone = sweepReport('host-a', 'agent-a', ['agent-a'], null, {
    known: false,
    running: false,
    canChat: false,
  });
