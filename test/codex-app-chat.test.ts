...
import { currentCodexAppThreadId, deliverCodexAppMessage, resolveCodexAppPeer } from "../src/chat/codexApp.ts";
import { chatPrincipalKey, codexAppAddress, humanLabel, principalLabel } from "../src/chat/identity.ts";
...
const fakeRpc = (handler: RpcHandler): CodexAppRpc => ({ request: async (method, params) => handler(method, params), close() {} });
...
test("Desktop tool environment becomes an exact App thread identity, never anonymous CLI", () => {
  expect(currentCodexAppThreadId({ CODEX_THREAD_ID: UUID, CODEX_APP_TOOLS_PIPE_PATH: "/tmp/tools.sock" })).toBe(UUID);
  expect(currentCodexAppThreadId({ CODEX_THREAD_ID: UUID, CODEX_INTERNAL_ORIGINATOR_OVERRIDE: "Codex Desktop" })).toBe(UUID);
  ...
  expect(currentCodexAppThreadId({ CODEX_THREAD_ID: "not-a-uuid", CODEX_APP_TOOLS_PIPE_PATH: "/tmp/tools.sock" })).toBeNull();
});
...
test("resolve verifies the exact thread and preserves the title only as a snapshot", async () => {
  const peer = await resolveCodexAppPeer(makeMachine(), UUID, async () => fakeRpc((method) => {
...
test("notLoaded resumes, revalidates idle, and starts one turn with immutable message id", async () => {
  ...
  const result = await deliverCodexAppMessage(
    ...
    "framed payload",
    async () => fakeRpc((method, params) => {
      calls.push({ method, params });
      if (method === "thread/read") return { thread: reads++ === 0 ? thread({ status: { type: "notLoaded" }, canAcceptDirectInput: null }) : thread() };
  ...
  expect(calls.map((call) => call.method)).toEqual(["thread/read", "thread/resume", "thread/read", "turn/start"]);
