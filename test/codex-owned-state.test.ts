...
const turn = (
  projection: OwnedCodexProjection,
...
test('a newer event defeats a stale initial snapshot and foreign events cannot change state', () => {
  ...
  expect(
    p.event({
      ...
      params: { threadId: randomUUID(), status: { type: 'idle' } },
    }),
  ).toBe(false);
...
test('native requests distinguish transport submission from provider resolution', () => {
  ...
  expect(p.snapshot().nativeItems.at(-1)).toMatchObject({
  ...
  expect(
    p.event({
      ...
      params: { threadId: session.uuid, requestId: 'approval-a' },
    }),
  ).toBe(true);
  expect(p.snapshot().nativeItems.at(-1)).toMatchObject({
    requestId: 's:approval-a',
...
test('resident events are bounded and readers detect cursor gaps and generation changes', () => {
  const p = new OwnedCodexProjection(machine, session, process.pid);
  state(p, { type: 'idle' });
  const first = codexRuntimeUpdates({
    protocol: 1,
    status: 'live',
    reason: null,
    snapshot: p.snapshot(),
  ...
  const read: OwnedCodexRead & { snapshot: ReturnType<OwnedCodexProjection['snapshot']> } = {
    protocol: 1,
    status: 'live',
    reason: null,
    snapshot: p.snapshot(),
  ...
  const delta = codexRuntimeUpdates(read, {
...
test('reader never exposes expired, dead, wrong identity or disconnected positive state', () => {
  ...
  expect(validateOwnedCodex(bytes, { ...identity, threadId: randomUUID() }, 10_001).reason).toBe(
    'identity-mismatch',
  );
  expect(validateOwnedCodex(bytes, identity, 9_999).reason).toBe('clock-skew');
  expect(
    validateOwnedCodex(
      JSON.stringify({ ...p.snapshot(), providerPid: 2147483647 }),
      identity,
      10_001,
    ).reason,
  ).toBe('producer-stopped');
  p.unavailable('lost', 10_001);
  expect(validateOwnedCodex(JSON.stringify(p.snapshot()), identity, 10_001)).toMatchObject({
    status: 'unavailable',
    snapshot: null,
  });
});
...
test('prepared file reader rejects symlinks, permissive files and oversize; coalesced writes retain last state', async () => {
  ...
  for (let i = 0; i < 100; i++) {
    state(p, { type: i % 2 ? 'idle' : 'active', activeFlags: [] });
    writes.push(writer.write(p.snapshot()));
  }
...
test('native launch keeps endpoint and UUID under owner control and disables only the client update prompt', () => {
  ...
  expect(argv.slice(0, 4)).toEqual([
    ...
    'app-server',
    '--listen',
    `unix://${ownedCodexSocket(machine, session.name)}`,
  ]);
  ...
  expect(ownedCodexFlags(['--sandbox', 'read-only', '--no-alt-screen'])).toEqual({
    server: ['-c', 'sandbox_mode="read-only"'],
    client: ['--no-alt-screen'],
  });
  expect(ownedCodexSocket(machine, session.name).length).toBeLessThan(100);
  expect(ownedCodexSocket(machine, session.name)).not.toBe(
    ownedCodexSocket({ stateDir: '/tmp/another-instance' }, session.name),
  );
});
