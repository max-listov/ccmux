  ...
  const server = Bun.serve<unknown>({
    ...
    websocket: {
      ...
      message(ws, raw) {
        ...
        if (decoded.method === undefined) {
          responses.push(decoded);
          return;
        }
        const message = z
          .object({ id: z.number().optional(), method: z.string(), params: z.unknown() })
          .parse(decoded);
        ...
        if (message.method === 'turn/start') {
          ...
          turnMessageIds.push(
            z.object({ clientUserMessageId: z.string() }).parse(message.params).clientUserMessageId,
          );
          const rollout = join(
            codexSessionsDir,
            '2026',
        ...
        if (message.method === 'thread/resume' || message.method === 'thread/read') {
          if (message.method === 'thread/resume' && admissionRace) {
            send('turn/started', turn('inProgress'));
            admissionRace = false;
          }
          ...
          respond({
            thread: {
              id: wrongIdentity ? crypto.randomUUID() : s.uuid,
              name: null,
              source: 'cli',
              status: native,
              canAcceptDirectInput: true,
        ...
        if (message.method === 'collaborationMode/list')
          respond({
            data: supportsCollaboration
              ? [{ name: 'Plan', mode: 'plan', model: null, reasoning_effort: 'medium' }]
              : [{ name: 'Default', mode: 'default', model: null, reasoning_effort: null }],
          });
        if (message.method === 'thread/turns/list')
          respond({ data: [{ id: 'turn-a', status: 'inProgress' }] });
      },
    },
  });
  return {
...
test('fresh admission retries the named empty-rollout failure only after committed session_meta', async () => {
  const f = fixture('delayed'),
    connection = new OwnedCodexConnection(f.m, f.s, process.pid);
  try {
    await connection.open(new AbortController().signal);
...
test('fresh managed admission applies the owner collaboration policy to every bootstrap retry', async () => {
  ...
  try {
    ...
    for (const params of f.turnParams)
...
test('fresh managed admission refuses an unsupported collaboration policy before its first turn', async () => {
  ...
  try {
    ...
    await expect(connection.admit(true, new AbortController().signal)).rejects.toMatchObject({
      code: 'COLLABORATION_MODE_UNAVAILABLE',
...
test('native subscription precedes snapshots, reconnect changes generation and retired connections cannot replay history', async () => {
  ...
  try {
    await first.open(new AbortController().signal);
    await first.admit(false, new AbortController().signal);
    ...
    while (
      readOwnedCodexStatus(f.m, f.s).snapshot?.turn?.status !== 'completed' &&
      Date.now() < deadline
    )
      await Bun.sleep(5);
    expect(readOwnedCodexStatus(f.m, f.s).snapshot?.turn?.status).toBe('completed');
    expect(readEvents(f.m).map(({ event, threadId }) => ({ event, threadId }))).toEqual([
      { event: 'turn-end', threadId: UUID },
    ]);
    f.disconnect();
    await Bun.sleep(20);
    ...
    await next.open(new AbortController().signal);
    await next.admit(false, new AbortController().signal);
    const resumed = readOwnedCodexStatus(f.m, f.s);
    expect(resumed.status).toBe('live');
    expect(resumed.snapshot?.generation).not.toBe(baseline.snapshot?.generation);
    ...
    await first.close('stopped');
    await next?.close('stopped');
    f.close();
  }
});
...
test('mismatched native resume and malformed active flags never admit an idle replacement', async () => {
  ...
  try {
    await connection.open(new AbortController().signal);
    f.mismatch();
    await expect(connection.admit(false, new AbortController().signal)).rejects.toThrow(
      'different thread identity',
    );
    expect(readOwnedCodexStatus(f.m, f.s).status).toBe('unavailable');
    expect(f.requests).not.toContain('thread/start');
  } finally {
  ...
  const malformed = fixture(),
    second = new OwnedCodexConnection(malformed.m, malformed.s, process.pid);
  try {
...
test('native version floor excludes older/unknown/floor prerelease binaries', () => {
  for (const version of [
    'codex-cli 0.151.0',
    'codex-cli 0.151.0+build.1',
    'codex-cli 0.152.0-alpha.8',
    'codex-cli 1.0.0',
  ])
    expect(supportsOwnedCodexVersion(version)).toBe(true);
  for (const version of [
    'unknown',
    'codex-cli 0.150.0',
    'codex-cli 0.151.0-alpha.1',
    'codex-cli 0.150.0-..',
    'codex-cli 0.150.0invalid',
  ])
    expect(supportsOwnedCodexVersion(version)).toBe(false);
});
...
test('approval and input responses stay on the owning RPC connection and reject stale projection generations', async () => {
  ...
  try {
    await connection.open(new AbortController().signal);
    await connection.admit(false, new AbortController().signal);
    f.request('approval-1', 'item/commandExecution/requestApproval', {
      threadId: f.s.uuid,
      turnId: 'turn-a',
      itemId: 'command-a',
    ...
    for (
      let i = 0;
      i < 100 && readOwnedCodexStatus(f.m, f.s).snapshot?.pendingRequests.length !== 1;
      i++
    )
      await Bun.sleep(5);
    const approval = readOwnedCodexStatus(f.m, f.s).snapshot;
    assert(approval);
    expect(approval.pendingRequests[0]).toMatchObject({
    ...
    await writeNativeCommand(f.m, f.s.name, {
      ...approvalCommand,
      fingerprint: nativeResponseFingerprint(approvalCommand),
    });
    ...
    expect(readNativeReceipt(f.m, f.s.name)).toMatchObject({
      outcome: 'submitted',
      requestId: 's:approval-1',
    });
    ...
    await writeNativeCommand(f.m, f.s.name, {
      ...inputCommand,
      fingerprint: nativeResponseFingerprint(inputCommand),
    });
    await connection.applyControlResponse();
    ...
    await writeNativeCommand(f.m, f.s.name, {
      ...staleCommand,
      fingerprint: nativeResponseFingerprint(staleCommand),
    });
    await connection.applyControlResponse();
    expect(f.responses).toHaveLength(before);
    expect(readNativeReceipt(f.m, f.s.name)).toMatchObject({
      outcome: 'rejected',
      reason: 'projection-generation-mismatch',
    });
  } finally {
    await connection.close('stopped');
    f.close();
