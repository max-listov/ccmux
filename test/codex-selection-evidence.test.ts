...
const settings = {
  ...
  params: { threadId: "native-thread", threadSettings: {
    model: "model-next", modelProvider: "provider-a", effort: "low",
    collaborationMode: { mode: "plan", settings: { developer_instructions: "private fixture instructions" } },
    cwd: "/private/fixture/workspace", config: { secret: "fixture-secret-value" },
  } },
};
...
test("native settings, not desired defaults, provide effective model/mode/effort evidence", () => {
  ...
  expect(evidence).toEqual({ model: { provider: "provider-a", model: "model-next" },
    options: { runtime: "codex", model: { provider: "provider-a", model: "model-next" }, mode: "plan", effort: "low" },
...
test("null native effort stays absent rather than becoming an invented default", () => {
  const event = { ...settings, params: { ...settings.params,
...
test("native reroute changes observed model without claiming effective mode or copying internal reason", () => {
  ...
  const event = { method: "model/rerouted", params: { threadId: "native-thread", turnId: "turn-a",
    fromModel: "model-next", toModel: "model-routed", reason: "private fixture provider detail" } };
  expect(codexSelectionEvent(event, "native-thread", previous)).toEqual({
    model: { provider: "provider-a", model: "model-routed" }, options: null, source: "reroute", turnId: "turn-a",
  });
...
test("unknown native mode does not masquerade as an admitted selection", () => {
  const event = { ...settings, params: { ...settings.params, threadSettings: {
    ...settings.params.threadSettings, collaborationMode: { mode: "unknown" },
  } } };
  expect(() => codexSelectionEvent(event, "native-thread", null)).toThrow();
  expect(codexSelectionEvent({ method: "thread/started", params: {} }, "native-thread", null)).toBeNull();
});
