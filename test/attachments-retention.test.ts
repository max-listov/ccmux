...
test('acceptance atomically pins ordered references and uncertainty never makes assets collectible', async () => {
  ...
  await expect(
    pin(f, messageId, [first, second], async () => {
      throw new Error('unknown-ledger-outcome');
    }),
  ...
  await expect(pin(f, messageId, [second, first], async () => 'wrong')).rejects.toMatchObject({
  ...
  const paths = await resolveMessageAttachments(
    f.machine,
    f.session,
    messageId,
    [first, second],
  ...
  const urls = await resolveMessageAttachments(
    f.machine,
    f.session,
    messageId,
    [first, second],
...
test('restart and archive preserve retained inputs while registration replacement cannot reuse them', async () => {
  ...
  expect(
    await resolveMessageAttachments({ ...f.machine }, restarted, messageId, [reference], f.signal),
  ).toHaveLength(1);
...
test('fork retention copies reachability, not bytes, and preserves exact caller authorization', async () => {
  ...
  expect(
    (await readAttachmentChunk(f.machine, f.principal, { target, reference, offset: 0 }, f.signal))
      .complete,
  ).toBe(true);
  await expect(
    readAttachmentChunk(
      f.machine,
      { ...f.principal, machine: 'host-c' },
      { target, reference, offset: 0 },
      f.signal,
    ),
  ).rejects.toMatchObject({ code: 'ATTACHMENT_UNAVAILABLE' });
  const counts = await withAttachmentStore(f.machine, 'fixture-inspect', async (tx) => ({
    rows: tx.store.records.length,
    pins: tx.store.pins.length,
  }));
