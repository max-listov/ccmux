...
test("bundled daemon self-update settles healing before clean SIGTERM and restored-bundle restart", async () => {
  ...
  const bundle = join(root, "data/app/ccmux.js"), bin = join(root, "bin"); mkdirSync(bin);
  for (const name of ["launchctl", "systemctl"]) writeFileSync(join(bin, name), "#!/bin/sh\necho 'forbidden self service-manager restart' >&2\nexit 91\n", { mode: 0o755 });
  const released = process.env.CCMUX_TEST_RELEASE_BUNDLE;
  if (released) { mkdirSync(join(root, "data/app"), { recursive: true }); copyFileSync(released, bundle); }
  else expect(await buildBundle(bundle)).toBe(true);
  const bytes = readFileSync(bundle), hash = new Bun.CryptoHasher("sha256").update(bytes).digest("hex");
  let requests = 0;
  const server = Bun.serve({ hostname: "127.0.0.1", port: 0,
    fetch(req): Response {
      requests++;
      if (new URL(req.url).pathname === "/bundle") return new Response(bytes);
      return Response.json({ version: VERSION, notes: "isolated self-update test", sha256: hash, url: new URL("/bundle", server.url).href });
    } });
  const machine = makeMachine({ rcPrefix: "host-a", stateDir: join(root, "state"), codexHome: join(root, "no-provider"),
    claudeBin: "/usr/bin/false", tmuxBin: Bun.which("tmux") ?? "/usr/bin/false", tmuxSocket: `ccmux-self-update-${crypto.randomUUID()}`,
    autoUpdate: true, ensureInterval: 1, updateCheckInterval: 1, releaseUrl: new URL("/release", server.url).href,
    sessionEvents: false, chatEnabled: false });
  const config = join(root, "machine.json"); writeFileSync(config, JSON.stringify(machine));
  const env = { ...process.env, CCMUX_CONFIG: config, CCMUX_RC_PREFIX: machine.rcPrefix, CCMUX_STATE_DIR: machine.stateDir,
    CCMUX_DATA_DIR: join(root, "data"), CCMUX_CACHE_DIR: join(root, "cache"), PATH: `${bin}:${process.env.PATH ?? ""}` };
  const start = () => Bun.spawn([process.execPath, "--no-env-file", bundle, "daemon"], { env, stdin: "ignore", stdout: "ignore", stderr: "pipe" });
  let daemon = start();
  ...
  const ready = async () => {
    ...
    try {
      for (let n = 0; n < 100; n++) {
        try { return await client.list(); } catch {}
        await Bun.sleep(20);
      }
  ...
  try {
    ...
    const code = await daemon.exited; clearTimeout(timeout);
    ...
    daemon = start(); errors = new Response(daemon.stderr).text();
    const second = await ready(); expect(second.generation).not.toBe(first.generation);
    daemon.kill("SIGTERM"); expect(await daemon.exited).toBe(143);
    expect(await errors).toContain('"outcome":"clean"');
  } finally {
    daemon.kill("SIGKILL"); await daemon.exited; await errors;
