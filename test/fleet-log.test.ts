...
const out = (o: {
  ...
  return {
    kind: 'msg',
    envelope: makeChatMessage({ id, ts: o.ts, from, to, body: o.body ?? 'do the thing' }),
    result,
  };
};
...
const src = (
  ...
  rows: LogRow[],
  ok = true,
): { machine: LogMachine; rows: LogRow[] } => ({
...
test('localRows merges ledger + outbox in time order', () => {
  const rows = localRows(
    'host-a',
    [msg({ ts: '2026-08-05T10:00:02Z' })],
    [out({ ts: '2026-08-05T10:00:01Z' })],
  );
...
test('a remote sender is rendered with its machine, so a reply address is readable off the line', () => {
  const [row] = localRows(
    'host-b',
    [msg({ ts: 't', from: makePeer({ machine: 'host-a', session: 'agent-a' }) })],
    [],
  );
...
test('mergeFleetLog interleaves machines by timestamp and keeps the newest N', () => {
  const a = src(
    ...
    localRows(
      'host-a',
      [msg({ ts: '2026-08-05T10:00:00Z' }), msg({ ts: '2026-08-05T10:00:04Z' })],
      [],
    ),
  );
  ...
  expect(mergeFleetLog([a, b], 2).map((r) => r.ts)).toEqual([
...
test('a current peer payload requires identity and origin while retaining optional display defaults', () => {
  ...
  expect(LogPayloadSchema.safeParse({ machines: [], rows: [{ kind: 'chat' }] }).success).toBe(
    false,
  );
});
