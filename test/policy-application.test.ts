...
import { AgentPoliciesSchema, ApplicationPolicyReferenceSchema, MaterializedPolicySchema } from "../src/policy/schema.ts";
import { applicationPolicyEvidence, resolveApplicationPolicy, verifyApplicationPolicy } from "../src/policy/resolve.ts";
...
function fixture() {
  ...
  const skill = { ...source("skill-a", "---\nname: skill-a\ndescription: A canonical fixture\n---\nUse the skill vocabulary.", "SKILL.md"), name: "skill-a" };
  const agentSource = source("agent-a", "---\ndescription: A canonical agent\nmode: primary\n---\nUse the canonical agent vocabulary.\n");
  const host = { agentPolicies: AgentPoliciesSchema.parse({
    "profile-a": { runtime: "codex", revision: "r1", trustedRoots: [root], instructionSources: [instructions], skills: [skill] },
    "profile-b": { runtime: "codex", revision: "r1", trustedRoots: [root],
      instructionSources: [source("instructions-b", "Use a different application vocabulary.")] },
    "agent-a": { runtime: "opencode", revision: "r1", trustedRoots: [root], agent: { name: "agent-a", source: agentSource }, denyTools: ["bash"] },
  }) };
...
test("public policy reference refuses paths, bodies, environment, permission grants and extra policy fields", () => {
  ...
  for (const fields of [{ path: "/private/source" }, { instructions: "private body" },
    { skills: ["skill-a"] }, { envFile: "/private/environment" }, { tools: { bash: true } },
    { mcpServers: {} }, { runtime: "codex" }, { digest: "a".repeat(64) }])
...
test("two profiles under one host remain distinct and repeated resolution preserves immutable metadata", () => {
  ...
  expect(composePolicyDeveloperInstructions(first, "Supervisor instructions")).toContain("canonical application vocabulary");
  expect(composePolicyDeveloperInstructions(second, "Supervisor instructions")).not.toContain("canonical application vocabulary");
...
test("unknown, removed, wrong revision and wrong runtime refuse before an admission callback", () => {
  ...
  expect(() => resolveApplicationPolicy(host, "codex", { ...ref, revision: "r2" })).toThrow(unavailable);
...
test("restart refuses edited bytes or edited definition even when the host pins a new matching digest", () => {
  ...
  definition.instructionSources = [{ ...instructions, sha256: policySha256("Edited application instructions.") }];
...
test("source paths are canonical, inside an explicit trust root and cannot be symlinks", () => {
  ...
  if (definition?.runtime !== "codex") throw new Error("fixture requires Codex policy");
  expect(AgentPoliciesSchema.safeParse({ unsafe: { ...definition,
    instructionSources: [{ ...f.instructions, path: `${f.root}/../source.md` }] } }).success).toBe(false);
...
test("bounded source reading rejects absent, directory, oversized, NUL and invalid UTF-8 sources", () => {
  ...
  for (const source of [{ ...f.instructions, path: join(f.root, "absent.md") },
    { ...f.instructions, path: dir }, f.source("large", "x".repeat(MAX_POLICY_SOURCE_BYTES + 1)),
    f.source("nul", "a\0b")]) {
...
test("desired/applied/unavailable projection excludes private materialization and error output stays generic", () => {
  ...
  const projected = JSON.stringify([
    applicationPolicyEvidence(resolved, "desired"), applicationPolicyEvidence(resolved, "applied"),
  ...
  try { verifyApplicationPolicy(f.host, "codex", resolved.metadata); throw new Error("expected refusal"); }
  catch (error) {
...
test("Codex preserves supervisor instructions and selects only exact enabled workspace-native skills", () => {
  ...
  expect(composePolicyDeveloperInstructions(policy, "SUPERVISOR")).toBe("SUPERVISOR\n\nUse the canonical application vocabulary.");
  const skill = { name: f.skill.name, path: f.skill.path, enabled: true };
  const inventory = { data: [{ cwd: f.root, errors: [], skills: [skill] }] };
  expect(policySkillInputs(policy, f.root, inventory)).toEqual([{ type: "skill", name: f.skill.name, path: f.skill.path }]);
  for (const bad of [null, { data: [] }, { data: [{ ...inventory.data[0], cwd: "/other" }] },
...
test("OpenCode selects a canonical native agent without exposing or replacing its body/configuration", () => {
  ...
  const agent = { name: "agent-a", mode: "primary", prompt: "Use the canonical agent vocabulary.\n",
    permission: [{ permission: "*", pattern: "*", action: "allow" }, { permission: "bash", pattern: "*", action: "deny" }],
  ...
  expect(JSON.stringify(applicationPolicyEvidence(policy, "applied"))).not.toContain("fixture-private-value");
  for (const candidate of [{ ...agent, prompt: "different" }, { ...agent, hidden: true },
    { ...agent, mode: "subagent" }, { ...agent, name: "different" }])
...
test("OpenCode denies all-resource grant exceptions and never appends allow to a host ceiling", () => {
  const f = fixture();
  const policy = resolveApplicationPolicy(f.host, "opencode", agentRef);
  const agent = { name: "agent-a", mode: "primary", prompt: "Use the canonical agent vocabulary.", permission: [] };
  const deny = { permission: "bash", pattern: "*", action: "deny" };
  for (const permission of [[], [{ ...deny, pattern: "dangerous *" }],
    [deny, { permission: "*", pattern: "safe *", action: "allow" }],
    [deny, { permission: "b*", pattern: "safe *", action: "ask" }]])
    expect(() => selectOpenCodePolicyAgent(policy, [{ ...agent, permission }])).toThrow(unavailable);
  expect(selectOpenCodePolicyAgent(policy, [{ ...agent, permission: [
    { permission: "bash", pattern: "safe *", action: "allow" }, deny,
    { permission: "bash", pattern: "dangerous *", action: "deny" },
  ] }])).toBe("agent-a");
  const definition = f.host.agentPolicies[agentRef.id];
  expect(AgentPoliciesSchema.safeParse({ unsafe: { ...definition, allowTools: ["bash"] } }).success).toBe(false);
  expect(AgentPoliciesSchema.safeParse({ unsafe: { ...definition, mcp: { allow: true } } }).success).toBe(false);
  const codex = f.host.agentPolicies[ref.id];
  expect(AgentPoliciesSchema.safeParse({ unsafe: { ...codex, denyTools: ["bash"] } }).success).toBe(false);
});
...
test("strict materialization refuses accidental public serialization as evidence metadata", () => {
  ...
  expect(() => verifyApplicationPolicy(f.host, "codex", { ...policy.metadata, digest: "0".repeat(64) })).toThrow(unavailable);
});
...
test("native agent source comparison normalizes CRLF and refuses broken frontmatter or an empty prompt", () => {
  ...
  definition.agent.source = f.source("crlf", "---\r\nmode: primary\r\n---\r\nCanonical prompt.\r\n");
  const policy = resolveApplicationPolicy(f.host, "opencode", agentRef);
  expect(selectOpenCodePolicyAgent(policy, [{ name: "agent-a", mode: "all", prompt: "Canonical prompt.\n",
    permission: [{ permission: "b?sh", pattern: "*", action: "deny" }] }])).toBe("agent-a");
  for (const body of ["---\nmode: primary\nmissing delimiter", "---\nmode: primary\n---\n\n", "  \n"]) {
