...
test('resultSummary: edit diff, write/read/run line counts, error short-circuit', () => {
  ...
  expect(resultSummary('Bash', null, 'boom: command not found', true)).toBe(
        ...
        content: [
          {
            ...
            id: callId,
            name: 'Edit',
            input: { file_path: '/foo.ts', old_string: 'a\nb', new_string: 'c' },
          },
        ],
      },
    }),
        ...
        content: [
          {
            ...
            tool_use_id: callId,
            is_error: false,
            content: 'The file /foo.ts has been updated.',
          },
        ],
      },
    }),
        ...
        content: [
          {
            ...
            id: 'tc',
            name: 'Bash',
            input: { command: 'sleep 5', description: 'long job' },
          },
        ],
      },
    }),
...
test('parse honors endLine — bounded window for backward pagination', () => {
  ...
  const texts = (start: number, endLine?: number) =>
    parse(lines, start, undefined, endLine).map((m) => m.text);
