...
import { courierNote, isExternalToken, externalNameOf, lookupExternal, outstandingExternal } from "../src/chat/external.ts";
import { chatTargetKey, externalAddress, externalTarget, humanTargetLabel, targetLabel } from "../src/chat/identity.ts";
...
const machine = (externals: Record<string, string> = {}) =>
  MachineConfigSchema.parse({ claudeBin: "/bin/claude", tmuxBin: "/bin/tmux", projectsDir: "/p", rcPrefix: "host-a", stateDir: "/tmp/x", bootLabel: "b", externals });

const cli: ChatPrincipal = { kind: "cli", source: "ccmux", machine: "host-a" };
const peer = (session: string): ChatPrincipal => ({
  kind: "managed", source: "ccmux", machine: "host-a", agent: "claude", session, threadId: "11111111-1111-4111-8111-111111111111",
});

function letter(to: ChatTarget, from: ChatPrincipal = cli, task: string | null = null, onBehalfOf: string | null = null, ts = "2026-08-25T10:00:00.000Z"): ChatMessage {
  return ChatMessageSchema.parse({ v: CHAT_GENERATION, id: randomUUID(), ts, from, to, body: "text", task, defer: false, onBehalfOf, notBefore: null });
}
...
test("a declared name resolves to where a person takes it", () => {
  const out = lookupExternal(machine({ "contract-owner": "another product, ping them there" }), "owner/contract-owner");
...
test("a recorded answer closes it, and only it", () => {
  const ledger = [
    ...
    letter({ kind: "managed", source: "ccmux", machine: "host-a", agent: "claude", session: "agent-a", threadId: "11111111-1111-4111-8111-111111111111" }, cli, null, "owner/contract-owner"),
  ];
...
test("two letters want two answers — one reply must not close both", () => {
  ...
  const back: ChatTarget = { kind: "managed", source: "ccmux", machine: "host-a", agent: "claude", session: "agent-a", threadId: "11111111-1111-4111-8111-111111111111" };
  const ledger = [
...
test("the mirror renders an errand, not a notification", () => {
  ...
  // work that out.
  const tg = formatForTg(letter(externalTarget("contract-owner"), peer("agent-a")), { "contract-owner": "another product" });
