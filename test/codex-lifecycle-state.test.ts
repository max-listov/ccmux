...
import { loadPendingSessions, promotePendingSession, removePendingSession, reservePendingSession } from "../src/config/pendingSessions.ts";
import { appendSession, loadSessions, removeSession, removeSessionIfGeneration } from "../src/config/sessions.ts";
...
import { readLifecycleBlockForSession, writeLifecycleBlock } from "../src/config/lifecycleBlocks.ts";
...
test("late rollback removes only the row promoted by its own generation", async () => {
  ...
  const replacement = SessionSchema.parse({ name: "agent-a", dir: "/home/user", agent: "codex", uuid: READY });
...
test("late promotion cannot overwrite a removed generation", async () => {
  ...
  await expect(promotePendingSession(m, GENERATION, READY)).rejects.toThrow("removed, replaced, or blocked");
  expect(loadSessions(m)).toEqual([]);
});

test("promotion rejects a claimed UUID and preserves unrelated registry rows", async () => {
  const m = setup();
  const existing = SessionSchema.parse({ name: "agent-b", dir: "/home/user", agent: "claude", uuid: READY });
...
test("a pending Codex name atomically reserves against a ready Claude append", async () => {
  const m = setup();
  await reservePendingSession(m, pending());
  const claude = SessionSchema.parse({ name: "agent-a", dir: "/home/user", agent: "claude", uuid: READY });
