...
import { prepareOpenCodeCatalog, preparedOpenCodeChoices } from "../src/agent/opencode/catalog.ts";
...
async function fixture() {
  ...
  const m = makeMachine({ rcPrefix: "host-a", stateDir: join(root, "state"), chatEnabled: true,
    opencodeBin: "/unavailable-native-provider", agentPolicies: { "policy-a": {
      runtime: "opencode", revision: "r1", trustedRoots: [root], agent: { name: "agent-a", source }, denyTools: [],
    } } });
  const policy = resolveApplicationPolicy(m, "opencode", { id: "policy-a", revision: "r1" });
  const model = { provider: "provider-a", model: "model-a" };
  const s = makeSession({ name: "worker-a", dir: workspace, agent: "opencode", runtime: "native", chatEnabled: true,
    registrationGeneration: crypto.randomUUID(), applicationPolicy: policy.metadata, modelSelection: model,
  ...
  const server = Bun.serve({ hostname: "127.0.0.1", port: 0, fetch(request) {
    ...
    if (path === "/config/providers") return Response.json({ default: { "provider-a": "model-a" }, providers: [{
      id: "provider-a", models: { "model-a": { id: "model-a", name: "Model A", status: "active", capabilities: { input: { text: true } } } },
    }] });
    return new Response(null, { status: 404 });
  } });
  const signal = new AbortController().signal;
  try {
    await prepareOpenCodeCatalog(m, s, createOpencodeClient({ baseUrl: server.url.href, throwOnError: true }), signal);
...
test("message admission refuses another catalog agent before appending policy-incompatible work", async () => {
  ...
  await expect(acceptControlMessage(f.m, makeCli(f.m.rcPrefix), { target: f.target, messageId: crypto.randomUUID(),
    body: "hello", options: { ...f.options, agent: "agent-b" } }, f.signal))
    .rejects.toMatchObject({ code: "APPLICATION_POLICY_UNAVAILABLE", message: "Application policy is unavailable" });
...
test("selection admission refuses a conflicting policy agent without journaling or consuming the operation", async () => {
  const f = await fixture();
  const before = readFileSync(f.selectionPath, "utf8");
  const input = { ...f.identity, operationId: crypto.randomUUID(), expectedRevision: 0, options: { ...f.options, agent: "agent-b" } };
  await expect(updateControlSelection(f.m, input, f.signal))
    .rejects.toMatchObject({ code: "APPLICATION_POLICY_UNAVAILABLE", message: "Application policy is unavailable" });
  expect(readFileSync(f.selectionPath, "utf8")).toBe(before);
  expect(loadLedger(f.m)).toEqual([]);
  const accepted = await updateControlSelection(f.m, { ...input, options: { ...f.options, agent: "agent-a" } }, f.signal);
...
for (const agent of ["agent-a", undefined]) {
  test(`policy permits ${agent ?? "omitted"} agent in both defaults and explicit message options`, async () => {
    ...
    const selection = await updateControlSelection(f.m, { ...f.identity, operationId: crypto.randomUUID(), expectedRevision: 0, options }, f.signal);
    ...
    expect((await acceptControlMessage(f.m, makeCli(f.m.rcPrefix), message, f.signal)).duplicate).toBe(true);
    expect(loadLedger(f.m)).toHaveLength(1);
    expect(loadLedger(f.m)[0]?.turnOptions).toEqual(selection.current);
    expect((await readControlSelection(f.m, f.identity, f.signal)).current).toEqual(selection.current);
  });
}
...
test("changed canonical policy refuses both admission paths without persisted work", async () => {
  ...
  await expect(acceptControlMessage(f.m, makeCli(f.m.rcPrefix), { target: f.target,
    messageId: crypto.randomUUID(), body: "hello" }, f.signal)).rejects.toMatchObject({ code: "APPLICATION_POLICY_UNAVAILABLE" });
  await expect(updateControlSelection(f.m, { ...f.identity, operationId: crypto.randomUUID(), expectedRevision: 0, options: f.options }, f.signal))
