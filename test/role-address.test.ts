...
import { isRoleToken, resolveRole, roleOf, ROLE_SIGIL, type RoleCandidate } from "../src/chat/roleAddress.ts";
...
const c = (name: string, role: string | null, dir = `/src/${name}`, lastText: string | null = null): RoleCandidate => ({ name, role, dir, lastText });
...
test("one match resolves to that session", () => {
  const out = resolveRole("@contract-owner", [c("agent-a", "contract-owner"), c("agent-b", "firmware")]);
...
test("a candidate that has said nothing says so, rather than showing an empty line", () => {
  const out = resolveRole("@firmware", [c("agent-a", "firmware", "/src/a", null), c("agent-b", "firmware", "/src/b", "   ")]);
...
test("no match names the roles that DO exist there", () => {
  const out = resolveRole("@contract-owner", [c("agent-a", "firmware"), c("agent-b", "panel"), c("agent-c", null)], "host-a:");
  const error = (out as { error: string }).error;
