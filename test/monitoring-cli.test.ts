...
test("bundled CLI concurrent reads, large output and cancellation never touch a pane", async () => {
  ...
  const m = makeMachine({ rcPrefix: "host-a", stateDir: root, projectsDir: join(root, "transcripts"), tmuxBin: bin });
  const config = join(root, "machine.json"); writeFileSync(config, JSON.stringify(m));
  ...
  try {
    ...
    const missing = Bun.spawn([process.execPath, bundle, "status", "--json"], { env, stdout: "pipe", stderr: "pipe" });
    ...
    p.begin(m);
    for (let i = 0; i < 200; i++) p.sample(m, makeSession({ name: "agent-" + i, dir: "/" + "x".repeat(2000) }), undefined, null, UNSEEN);
    const snapshot = await p.publish(m);
    const results = await Promise.all(Array.from({ length: 20 }, async () => {
      const child = Bun.spawn([process.execPath, bundle, "status", "--json"], { env, stdout: "pipe", stderr: "pipe" });
    ...
    const blocked = Bun.spawn([process.execPath, bundle, "status", "--json"], { env, stdout: "pipe", stderr: "pipe" });
    await Bun.sleep(100);
    const start = performance.now(); blocked.kill("SIGTERM"); await blocked.exited;
