...
import { emptyListText, inventoryLabel, type FleetLoad } from "../src/tui/fleet.ts";
...
test("switching the inventory off is reported as off, not as an empty result", () => {
  expect(inventoryLabel(load({ externalOn: false, externalScanning: true }), 0)).toBe("external off");
});
