...
const session = () =>
  makeSession({ name: 'agent-a', uuid: '11111111-1111-4111-8111-111111111111' });
...
test('an event carries the FULL address, so no reader resolves anything against its own machine', () => {
  const e = buildEvent(
    machine(),
    session(),
    { event: 'turn-end', durationMs: 1800_000 },
...
test('filters by session, and -n keeps the tail', () => {
  const m = machine();
  appendEvent(m, session(), { event: 'turn-start' });
  appendEvent(m, makeSession({ name: 'agent-b', uuid: '44444444-4444-4444-8444-444444444444' }), {
    event: 'turn-start',
  });
...
const status = (event: string, state: 'working' | 'idle', ts: number): LifecycleStatus => ({
  state,
  ts,
  event,
});
...
test('a turn we never saw start still ends — without an invented duration', () => {
  ...
  expect(eventForLifecycle(status('Stop', 'idle', 5), status('SessionStart', 'idle', 1))).toEqual({
    event: 'turn-end',
  });
});
...
test('a prompt landing INSIDE a running turn does not start another one', () => {
  ...
  expect(
    eventForLifecycle(status('UserPromptSubmit', 'working', 9_000), status('Stop', 'idle', 8_000)),
  ).toEqual({ event: 'turn-start' });
});
...
test("a prompt inside a running turn does not move the turn's start — the duration is not a lie", () => {
  ...
  expect(eventForLifecycle(status('Stop', 'idle', 1_801_000), queued)).toEqual({
    event: 'turn-end',
...
test('a genuinely new turn keeps its own start instant', () => {
  const fresh = lifecycleToWrite(
    status('UserPromptSubmit', 'working', 5_000),
    status('Stop', 'idle', 4_000),
  );
  expect(fresh.ts).toBe(5_000);
  // …and ending is never rewritten by this rule.
  expect(
    lifecycleToWrite(status('Stop', 'idle', 9_000), status('UserPromptSubmit', 'working', 5_000))
      .ts,
  ).toBe(9_000);
});
...
test('a Stop arriving after the supervisor already closed that turn says nothing', () => {
  ...
  expect(
    eventForLifecycle(
      status('Stop', 'idle', 9_000),
      status(SUPERVISOR_CLOSED_EVENT, 'idle', 8_000),
    ),
  ).toBeNull();
});
...
test("a supervisor-closed turn does not swallow the NEXT turn's start", () => {
  ...
  expect(eventForLifecycle(status('UserPromptSubmit', 'working', 9_000), closed)).toEqual({
    event: 'turn-start',
  });
...
test("a session that is not in a turn has no instant at all — not zero, not the last turn's", () => {
  const stampFromAFinishedTurn: LifecycleStatus = {
    state: 'working',
    ts: 1_000,
    event: 'UserPromptSubmit',
  };
...
test('a session stopping is reported, and nothing else is said about a ghost', () => {
  const out = transitions(observed({ waitingAt: 'Do you want to proceed?' }), {
    ...UNSEEN,
    running: false,
  });
...
test('leaving the menu closes the pair, because answering a prompt starts no new turn', () => {
  ...
  expect(transitions(observed({ waitingAt: 'Continue?' }), observed())).toEqual([
    { event: 'resumed' },
  ]);
});
...
test('an interrupted turn is closed by observation — Stop never fires for one', () => {
  const out = transitions(
    observed({ turnStartedMs: 1_000 }),
    observed({ turnStartedMs: 1_000, turnOverMs: 43_000, turnInterrupted: true }),
  );
...
test('a turn whose Stop hook never fired is closed too, and is NOT called interrupted', () => {
  ...
  const out = transitions(
    observed({ turnStartedMs: 1_000 }),
    observed({ turnStartedMs: 1_000, turnOverMs: 61_000 }),
  );
...
test('the duration runs to when the transcript stopped, not to when we noticed', () => {
  ...
  const out = transitions(
    observed({ turnStartedMs: 0 }),
    observed({ turnStartedMs: 0, turnOverMs: 30_000 }),
  );
...
test('ONE abandoned turn is announced once, however much the signal flickers', () => {
  ...
  expect(
    transitions(
      announced,
      observed({ turnStartedMs: turn, turnOverMs: null, turnClosedFor: turn }),
    ),
  ).toEqual([]);
  ...
  expect(
    transitions(
      announced,
      observed({ turnStartedMs: turn, turnOverMs: 50_000, turnClosedFor: turn }),
    ),
  ).toEqual([]);
});
...
test('a NEW turn abandoned the same way is announced again', () => {
  ...
  const out = transitions(
    prev,
    observed({ turnStartedMs: 90_000, turnOverMs: 100_000, turnInterrupted: true }),
  );
...
test('the two-level switch defaults ON, and a session can opt out', () => {
  ...
  expect(
    eventsEnabledFor(makeSession({ eventsEnabled: true }), makeMachine({ sessionEvents: false })),
  ).toBe(true);
});
...
test('the human line names the address, the duration and the interruption', () => {
  const e = buildEvent(
    machine(),
    session(),
    { event: 'turn-end', durationMs: 1_800_000, interrupted: true },
    '22222222-2222-4222-8222-222222222222',
    '2026-08-25T09:41:07.000Z',
  );
...
test('--framed produces exactly the envelope a resuming transport accepts', () => {
  const event = buildEvent(
    machine(),
    session(),
    { event: 'turn-end', durationMs: 1000 },
    '22222222-2222-4222-8222-222222222222',
    '2026-08-25T09:41:07.000Z',
  );
...
test("an explicit --since wins — a person's question outranks a transport's mechanism", () => {
  expect(resolveSince('2026-08-25T08:00:00.000Z', '2026-08-25T09:00:00.000Z')).toBe(
