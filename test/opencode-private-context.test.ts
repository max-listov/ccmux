...
const internal =
  'Called the Read tool with the following input: /private/owner-store/internal-image.png';
const authored = 'Called the Read tool with the following input: /workspace/user-authored.png';
function fixture() {
  const root = mkdtempSync(join(tmpdir(), 'ccmux-private-context-'));
  roots.push(root);
...
test('native history omits synthetic and compaction text by metadata, not authored text patterns', async () => {
  ...
  const data = [
    {
      info: {
        ...
        role: 'user',
        summary: { title: 'picker title' },
        time: { created: 1 },
      },
      parts: [
        { id: 'authored', type: 'text', text: authored },
    ...
    },
    {
      ...
      parts: [{ id: 'summary', type: 'text', text: summary }],
    },
    {
      info: {
        id: 'msg_reply',
      ...
      parts: [{ id: 'reply', type: 'text', text: 'Visible reply' }],
    },
  ];
  ...
  const page = await openCodeContextApi(m, s, client).history(
    { limit: 4 },
  ...
  expect(page.entries[1]).toMatchObject({
    kind: 'other',
    text: null,
    omittedBytes: Buffer.byteLength(internal),
  });
  expect(page.entries[2]).toMatchObject({
    kind: 'compaction',
    text: null,
    omittedBytes: Buffer.byteLength(summary),
  });
...
function nativeEvents() {
  const message = (id: string, role: 'user' | 'assistant', summary?: boolean) => ({
    ...
    properties: {
      info: {
        ...
        sessionID: 'ses_private',
        role,
        ...(role === 'assistant' ? { parentID: 'msg_user' } : {}),
        ...(summary === undefined ? {} : { summary }),
        time: { created: 1 },
      },
    },
  });
  const part = (messageID: string, id: string, text: string, synthetic?: boolean) => ({
  ...
  return [
    ...
    part('msg_user', 'user_text', authored),
    part('msg_user', 'private_user', internal, true),
    delta('msg_user', 'private_user', internal),
    message('msg_summary', 'assistant', true),
    part('msg_summary', 'summary_text', internal),
    delta('msg_summary', 'summary_text', internal),
    message('msg_reply', 'assistant'),
    part('msg_reply', 'private_reply', internal, true),
    delta('msg_reply', 'private_reply', internal),
    part('msg_reply', 'reply_text', authored),
    delta('msg_reply', 'reply_text', ' visible'),
  ];
}
...
test('native content and bounded projection exclude private context on snapshots and deltas', () => {
  ...
  expect(
    buffer
      .snapshot()
      .baseline.filter((item) => item.kind === 'assistant')
      .map((item) => item.text),
  ).toEqual([`${authored} visible`]);
  ...
  expect(
    projection
      .snapshot()
      .nativeItems.some((item) => item.kind === 'user' && item.text === authored),
  ).toBe(true);
  expect(
    projection
      .snapshot()
      .nativeItems.some((item) => item.kind === 'assistant' && item.text === `${authored} visible`),
  ).toBe(true);
});
