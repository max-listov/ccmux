...
import { mkdtempSync, rmSync, writeFileSync, renameSync, utimesSync, mkdirSync, symlinkSync } from "node:fs";
...
import { MonitoringReadSchema, STATUS_MAX_BYTES, STATUS_MAX_ITEMS } from "../src/monitoring/schema.ts";
...
import { observationExecCount, observedPane, observedSessionInventory } from "../src/monitoring/tmux.ts";
...
test("lifecycle blocks are identity-pinned and never leak their error into the monitoring DTO", () => {
  const m = fixture(); const s = makeSession();
  mkdirSync(join(m.stateDir, "lifecycle-blocks"));
  const path = join(m.stateDir, "lifecycle-blocks", s.name + ".json");
  writeFileSync(path, JSON.stringify({ name: s.name, agent: s.agent, uuid: s.uuid, error: "internal detail", at: new Date().toISOString() }));
...
test("snapshot symlinks and FIFOs cannot turn a bounded read into an arbitrary or blocking read", () => {
  const m = fixture(); const target = join(m.stateDir, "target");
  writeFileSync(target, "{}"); symlinkSync(target, monitoringStatusPath(m));
...
test("producer inventory uses printable delimiters under a service locale", async () => {
  const m = fixture(); const script = join(m.stateDir, "inventory");
...
test("managed status distinguishes work, idle, prompt, unknown and stopped", () => {
  ...
  expect(projectMonitoringRow(m, s, undefined, null, UNSEEN)).toMatchObject({
    state: "stopped", running: false, model: null, contextPercent: null, uptimeSeconds: null, lastActivityAt: null,
  });
  expect(row("Do you trust the files in this folder?\n❯ 1. Yes, I trust this folder\n2. No, exit").state).toBe("prompt");
});

test("metadata cache bounds retained bytes and invalidates inode replacement", () => {
  const m = fixture(); const file = join(m.stateDir, "data");
  writeFileSync(file, "a"); const at = new Date(100000); utimesSync(file, at, at);
  const cache = new MtimeCache<string>(1024, 2);
  expect(cache.get(file, () => "first")).toBe("first");
  expect(cache.get(file, () => "wrong")).toBe("first");
  const next = join(m.stateDir, "next"); writeFileSync(next, "b"); utimesSync(next, at, at); renameSync(next, file);
  expect(cache.get(file, () => "second")).toBe("second");
  for (let i = 0; i < 10; i++) {
    const path = join(m.stateDir, String(i)); writeFileSync(path, "x");
    cache.get(path, () => "x".repeat(100));
    expect(cache.retainedBytes).toBeLessThanOrEqual(1024);
  }
  rmSync(file); expect(cache.get(file, () => "bad")).toBeNull();
});
...
test("row and byte limits report omissions", async () => {
  const m = fixture(); const p = new MonitoringPublisher(); p.begin(m);
  for (let i = 0; i < 300; i++) p.sample(m, makeSession({ name: `agent-${i}` }), undefined, null, UNSEEN);
  const full = await p.publish(m);
  expect(full.sessions.length).toBe(STATUS_MAX_ITEMS);
  expect(full.omitted).toBe(44);
  p.begin(m);
  for (let i = 0; i < 300; i++) p.sample(m, makeSession({ name: `agent-${i}`, dir: `/${"x".repeat(8000)}` }), undefined, null, UNSEEN);
...
test("producer rejects oversized output and counts invalid identities without truncation", async () => {
  const m = fixture(); const script = join(m.stateDir, "oversized");
  writeFileSync(script, "#!/bin/sh\nexec head -c 70000 /dev/zero\n", { mode: 0o700 });
  m.tmuxBin = script;
  await expect(observedPane(m, "agent-a")).rejects.toThrow("output limit");
  const p = new MonitoringPublisher(); p.begin(m);
...
test("unavailable/stale snapshots never masquerade as empty live inventory", async () => {
  const m = fixture();
  expect(readMonitoringStatus(m)).toMatchObject({ status: "unavailable", reason: "missing", snapshot: null });
  const p = new MonitoringPublisher(); p.begin(m); const s = await p.publish(m);
  expect(readMonitoringStatus(m, Date.parse(s.observedAt) + 10001)).toMatchObject({ status: "stale", snapshot: null });
...
test("new, removed and restarted identities replace the complete bounded snapshot", async () => {
  ...
  p.begin(m); expect((await p.publish(m)).sessions).toEqual([]);
  p.stop(); expect(readMonitoringStatus(m).reason).toBe("missing");
  const restarted = new MonitoringPublisher(); restarted.begin(m);
