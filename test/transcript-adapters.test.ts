...
test("claude adapter folds a tool_result into its tool_call (one request→outcome block)", () => {
  const lines = [
    L({ type: "assistant", uuid: "u1", message: { role: "assistant", content: [{ type: "tool_use", id: "c1", name: "Bash", input: { command: "ls" } }] } }),
    L({ type: "user", uuid: "u2", message: { role: "user", content: [{ type: "tool_result", tool_use_id: "c1", content: "file.txt" }] } }),
  ];
...
test("claude adapter surfaces assistant text as a message", () => {
  const msgs = parseClaude([L({ type: "assistant", uuid: "u1", message: { role: "assistant", content: [{ type: "text", text: "hi there" }] } })], 1);
...
test("codex adapter parses a response_item message's output_text", () => {
  const msgs = parseCodex([L({ type: "response_item", payload: { type: "message", role: "assistant", content: [{ type: "output_text", text: "hello" }] } })], 1);
  expect(msgs).toHaveLength(1);
  expect(msgs[0]?.role).toBe("assistant");
