...
import { staleReasons, type LaunchStamp } from "../src/agent/launchStamp.ts";
...
test("a package name or an email is not an import — chasing it would add noise and stat calls", () => {
  const entry = write("pkg/GLOBAL.md", "use @anthropic-ai/sdk, ask someone@example.com\n@./real.md\n");
  write("pkg/real.md", "x");
  const files = ruleSetFiles(entry);
  expect(files.length).toBe(2);
...
test("an MCP table digest ignores everything else in the file it lives in", () => {
  ...
  const path = write("cfg/config.json", JSON.stringify({ numStartups: 1, mcpServers: { a: { command: "x" } } }));
  const before = jsonFieldDigest(path, "mcpServers");
  writeFileSync(path, JSON.stringify({ numStartups: 2, cached: { flags: [1, 2, 3] }, mcpServers: { a: { command: "x" } } }));
...
test("a TOML table digest ignores the project list the agent appends to the same file", () => {
  const path = write("cfg3/config.toml", '[mcp_servers.node]\ncommand = "node"\n[projects."/a"]\ntrust = "on"\n');
  const before = tomlTableDigest(path, "mcp_servers");
  writeFileSync(path, '[mcp_servers.node]\ncommand = "node"\n[projects."/a"]\ntrust = "on"\n[projects."/b"]\ntrust = "on"\n');
  utimesSync(path, new Date(), new Date(Date.now() + 5_000));
...
test("the env files considered are the ones the runtime actually reads", () => {
  ...
  expect(envFiles("/p", "production")).toEqual(["/p/.env", "/p/.env.local", "/p/.env.production", "/p/.env.production.local"]);
});
...
test("a changed rule set is reported as 'rules', not as a generic 'config'", () => {
  ...
  expect(staleReasons(stamp(), now({ inputs: { rules: "r2", mcp: "m1", env: null } }))).toEqual(["rules"]);
  expect(staleReasons(stamp(), now({ inputs: { rules: "r1", mcp: "m2", env: null } }))).toEqual(["mcp"]);
});

test("an env file that appeared (or changed) since launch is reported", () => {
  expect(staleReasons(stamp(), now({ inputs: { rules: "r1", mcp: "m1", env: "e1" } }))).toEqual(["env"]);
});
...
test("a stamp written before inputs existed is UNKNOWN, never stale", () => {
  ...
  expect(staleReasons(stamp({ inputs: null }), now({ inputs: { rules: "r9", mcp: "m9", env: "e9" } }))).toEqual([]);
});
...
test("nothing changed → nothing reported, and the env reason is not doubled", () => {
  ...
  const out = staleReasons(stamp({ envKeys: ["A"] }), now({ envKeys: ["A", "B"], inputs: { rules: "r1", mcp: "m1", env: "e1" } }));
