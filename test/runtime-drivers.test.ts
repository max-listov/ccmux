...
import { writeSessionsUnlocked, loadSessions } from "../src/config/sessions.ts";
...
import { runtimeCapabilities, hasNativeRuntime } from "../src/runtime/capabilities.ts";
...
function fixture() {
  ...
  const s = makeSession({ name: "native-a", agent: "opencode", runtime: "native", registrationGeneration: crypto.randomUUID(),
...
test("runtime selection is separate from inference selection and unavailable drivers fail before mutation", async () => {
  const { m } = fixture();
  expect(ControlCreateSchema.parse({ requestId: crypto.randomUUID(), name: "a", workspace: "/work",
    runtime: "opencode", modelSelection: { provider: "external", model: "model-a" } })).toMatchObject({ runtime: "opencode", flags: [] });
  expect(ControlCreateSchema.safeParse({ requestId: crypto.randomUUID(), name: "a", workspace: "/work", runtime: "shell", executable: "sh" }).success).toBe(false);
  const workspace = mkdtempSync("/tmp/ccmux-runtime-workspace-");
  await expect(createControlSession(m, { requestId: crypto.randomUUID(), name: "a", workspace, runtime: "custom" },
    new AbortController().signal)).rejects.toMatchObject({ code: "UNSUPPORTED" });
  expect(loadSessions(m)).toEqual([]);
  expect(existsSync(join(m.stateDir, "control", "create-requests.json"))).toBe(false);
  expect(readRuntimeCatalog(m).runtimes.find(row => row.runtime === "custom")).toMatchObject({ availability: "unavailable" });
  expect(runtimeCapabilities({ agent: "claude", runtime: "tui" })).toMatchObject({ structured: false, approval: false });
...
test("native create retries keep one registration and reject a changed runtime", async () => {
  ...
  const create = async (_m: typeof m, input: CreateManagedInput) => {
    calls++;
    const session = makeSession({ name: input.name, agent: input.agent, dir: input.dir, runtime: "native",
      registrationGeneration: input.registrationGeneration,
      nativeSession: { runtime: "opencode", id: "ses_one_writer", version: "1.18.20" } });
    await writeSessionsUnlocked(m, [session]); return session;
  };
  const input = { requestId: crypto.randomUUID(), name: "native", workspace, runtime: "opencode" } satisfies Parameters<typeof createControlSession>[1];
  ...
  expect(first.target).toEqual(second.target); expect(second.duplicate).toBe(true); expect(calls).toBe(1);
  expect(first.nativeSession?.id).toBe("ses_one_writer");
  await expect(createControlSession(m, { ...input, runtime: "codex" }, new AbortController().signal, create))
  ...
  await expect(createControlSession(m, input, new AbortController().signal, create)).rejects.toMatchObject({ code: "CORRUPT_STATE" });
  expect(calls).toBe(1);
});

test("CLI OpenCode defaults to native mode and unsupported adoption never enters Claude handling", async () => {
  const { m } = fixture();
  await expect(createManagedSession({ ...m, opencodeBin: undefined }, { name: "native", dir: "/tmp", agent: "opencode", flags: [], router: false }))
    .rejects.toThrow("OpenCode executable is not configured");
  await expect(createManagedSession(m, { name: "native", dir: "/tmp", agent: "codex", runtime: "native", flags: [], router: false }))
...
test("native completion requires correlated assistant evidence, never idle or transport closure", () => {
  ...
  projection.start("msg_a"); projection.status({ type: "idle" });
  expect(projection.snapshot().turn?.status).toBe("inProgress");
  projection.unavailable("disconnected");
  expect(projection.snapshot()).toMatchObject({ connected: false, state: "unknown", turn: { status: "inProgress" } });
  const message = { id: "msg_assistant", sessionID: "ses_native_a", role: "assistant", parentID: "msg_other",
    time: { created: 1, completed: 2 }, finish: "stop" } satisfies Parameters<typeof openCodeTerminal>[0];
  ...
  expect(projection.snapshot().nativeItems.at(-1)).toMatchObject({ kind: "terminal", turnId: "msg_a" });
  expect(openCodeTerminal({ ...message, error: { name: "MessageAbortedError", data: {} } })).toBe("interrupted");
});

test("native projection retains causal tool order, bounded data and exact input IDs", () => {
  const { projection } = fixture();
  projection.start("msg_a"); projection.status({ type: "busy" });
  projection.message({ id: "msg_assistant", sessionID: "ses_native_a", role: "assistant", parentID: "msg_a", time: { created: 1 } });
  for (let index = 0; index < 200; index++) projection.part({ id: `part_${index}`, messageID: "msg_assistant", sessionID: "ses_native_a",
  ...
  projection.question({ id: "que_a", sessionID: "ses_native_a", questions: [{ header: "Pick", question: "Which?",
    options: [{ label: "A", description: "First" }], custom: false }] });
  expect(projection.snapshot()).toMatchObject({ state: "waiting-input", pendingRequests: [{ requestId: "que_a",
    questions: [{ id: "0", isOther: false, multiple: false }] }] });
  projection.resolve("que_foreign"); expect(projection.snapshot().pendingRequests).toHaveLength(1);
  projection.resolve("que_a"); expect(projection.snapshot().pendingRequests).toHaveLength(0);
});
...
test("prepared native reader verifies registry, provider, lease and continuation identity", async () => {
  ...
  expect(readManagedRuntimeStatus(m, { ...s, registrationGeneration: crypto.randomUUID() }).reason).toBe("identity-mismatch");
  expect(readManagedRuntimeStatus(m, { ...s, nativeSession: { runtime: "opencode", id: "ses_other", version: "1.18.20" } }).status).toBe("unavailable");
  expect(readManagedRuntimeStatus(m, s, Date.now() + 6_000).status).toBe("stale");
  projection.unavailable("disconnected"); await writer.write(projection.snapshot());
