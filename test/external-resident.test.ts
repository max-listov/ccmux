...
import { EXTERNAL_MAX_BYTES, EXTERNAL_MAX_READERS, ExternalStatusSnapshotSchema, currentExternalStatus } from "../src/external/resident-schema.ts";
import { nativeTurnState } from "../src/external/turnState.ts";

const OTHER = "22222222-2222-4222-8222-222222222222";
const cleanup: (() => Promise<void>)[] = [];
afterEach(async () => { for (const close of cleanup.splice(0).reverse()) await close(); });
const RequestSchema = z.object({ method: z.string(), id: z.number().optional(), params: z.unknown().optional() });
const row = (id = UUID, type = "active") => ({ id, name: "Native title", cwd: "/project", updatedAt: 1_787_900_000,
  status: { type, activeFlags: [] }, preview: "private body never exported", turns: [{ secret: true }] });

async function fixture() {
  const root = mkdtempSync("/tmp/ccmux-external-resident-");
  const dir = join(root, "app-server-control"); mkdirSync(dir);
  const machine = makeMachine({ stateDir: root, codexHome: root, rcPrefix: "host-a" });
  const state = { rows: [row(), row(OTHER, "idle")], connections: 0, requests: 0,
    methods: new Set<string>(), hang: false, broken: false, cursor: false, race: false, version: "codex/0.150.0-alpha.12.2" };
  const provider = Bun.serve({ unix: join(dir, "app-server-control.sock"),
    fetch(req, server) { if (server.upgrade(req)) return; return new Response(null, { status: 400 }); },
    websocket: {
      open(ws) { state.connections++; ws.subscribe("native-status"); },
      message(ws, bytes) {
        const request = RequestSchema.parse(JSON.parse(String(bytes))); state.methods.add(request.method);
        if (request.method === "initialize") ws.send(JSON.stringify({ id: request.id, result: { userAgent: state.version } }));
        if (request.method !== "thread/list") return;
        state.requests++;
        expect(request.params).toMatchObject({ useStateDbOnly: true, limit: 128 });
        if (state.hang) return;
        if (state.race) ws.send(JSON.stringify({ method: "thread/status/changed", params: { threadId: UUID,
          status: { type: "active", activeFlags: ["waitingOnApproval"] } } }));
        ws.send(JSON.stringify({ id: request.id, result: { data: state.broken ? "broken" : state.rows, nextCursor: state.cursor ? "repeated" : null } }));
      },
    },
  });
  const external = new ExternalStatusPublisher(machine.rcPrefix), observer = new ExternalStatusObserver(machine, external);
  ...
  cleanup.push(async () => { await client.close(); await observer.close(); managed.close();
    await owned.server.shutdown({ gracePeriodMs: 100, forceTimeoutMs: 100 }); await owned.observability.close();
    provider.stop(true); rmSync(root, { recursive: true, force: true }); });
...
test("resident HTTP reads and subscriptions share one provider connection without importing managed identities", async () => {
  ...
  expect(initial.sessions[0]).toMatchObject({ identity: { provider: "codex", machine: "host-a", threadId: UUID }, name: "Native title", turnState: { state: "working" } });
  ...
  f.provider.publish("native-status", JSON.stringify({ method: "thread/status/changed", params: { threadId: UUID, status: { type: "idle" } } }));
...
test("a notification arriving during reconciliation wins over the older list response", async () => {
  const f = await fixture(); f.state.race = true;
  await f.observer.refresh();
  expect(f.external.read().sessions.find((s) => s.identity.threadId === UUID)?.turnState.state).toBe("waiting-approval");
  f.state.race = false; f.state.rows = [];
  await f.observer.refresh();
...
test("native event states, unknown flags and future clocks remain distinct without refreshing unrelated leases", async () => {
  const f = await fixture(); await f.observer.refresh();
  const baseline = f.external.read();
  const cases = [
    { status: { type: "active", activeFlags: [] }, expected: "working" },
    { status: { type: "active", activeFlags: ["waitingOnApproval"] }, expected: "waiting-approval" },
  ...
  const abort = new AbortController(), stream = await f.client.watchExternal.withOptions({ signal: abort.signal });
  await stream.next();
  for (const c of cases) {
    const next = stream.next();
    f.provider.publish("native-status", JSON.stringify({ method: "thread/status/changed", params: { threadId: UUID, status: c.status } }));
    const snapshot = ExternalStatusSnapshotSchema.parse((await next).value);
    expect(snapshot.sessions.find((s) => s.identity.threadId === UUID)?.turnState.state).toBe(c.expected);
    expect(snapshot.expiresAt).toBe(baseline.expiresAt);
    expect(snapshot.sessions.find((s) => s.identity.threadId === OTHER)?.turnState).toEqual(baseline.sessions.find((s) => s.identity.threadId === OTHER)?.turnState);
  }
  abort.abort(); await stream.return?.();
...
test("cancel before connect, cancel pending reconciliation and configuration replacement cannot revive a retired generation", async () => {
  ...
  await f.observer.refresh(); const generation = f.external.read().generation;
  f.state.hang = true;
  const stop = new AbortController(), pending = f.observer.refresh(f.machine, stop.signal);
...
test("deadline invalidates positives, reconnect replaces generation, and a failed host leaves another host untouched", async () => {
  const healthy = await fixture(), broken = await fixture();
  ...
  for (const session of unavailable.sessions) expect(session.turnState).toMatchObject({ state: "unknown", evidence: "unavailable" });
  expect(healthy.external.read().sessions[0]?.turnState.state).toBe("working");
  broken.state.hang = false; broken.state.rows = [row(UUID, "idle")];
  await broken.observer.refresh();
...
test("expiry, malformed pages, unsupported runtime and root migration never infer idle", async () => {
  ...
  for (const session of expired.sessions) expect(session.turnState).toMatchObject({ state: "unknown", evidence: "stale" });
  f.state.broken = true; await f.observer.refresh();
  expect(f.external.read()).toMatchObject({ status: "unavailable", reason: "invalid-response" });
  f.state.broken = false; f.state.version = "codex/0.140.0";
  const count = f.state.requests; await f.observer.refresh();
...
test("repeated cursors and closed provider connections fail closed; cancellation owns no provider threads", async () => {
  const f = await fixture(); f.state.cursor = true;
  await f.observer.refresh();
  expect(f.external.read().status).toBe("unavailable");
  expect(f.state.requests).toBe(2);
  f.state.cursor = false; await f.observer.refresh();
...
test("reader queues coalesce, row/byte bounds declare omission, and stopped publishers reject new readers", async () => {
  ...
  const rows = Array.from({ length: 513 }, () => ({ identity: { provider: "codex", machine: "host-a", threadId: crypto.randomUUID() },
    name: "x".repeat(4096), dir: "x".repeat(4096), updatedAt: null, turnState: nativeTurnState({ type: "idle" }, Date.now()) } satisfies Parameters<typeof p.publish>[0][number]));
  for (let n = 0; n < 50; n++) p.publish(rows, false, Date.now());
  const read = p.read();
  expect(read.omitted).toBeGreaterThan(0); expect(read.truncated).toBe(true);
  ...
  stops.forEach((s) => s.abort()); expect(p.subscribers).toBe(0);
  p.close(); expect(() => p.subscribe(new AbortController().signal)).toThrow("stopped");
});

test("the real daemon owns observation across restart but never stops its external provider", async () => {
  const f = await fixture(), root = mkdtempSync("/tmp/ccmux-external-daemon-");
  const machine = { ...f.machine, stateDir: root, autoUpdate: false, chatEnabled: false, sessionEvents: false,
    tmuxSocket: `ccmux-external-${crypto.randomUUID()}`, tmuxBin: Bun.which("tmux") ?? "/usr/bin/false" };
  const config = join(root, "machine.json"); writeFileSync(config, JSON.stringify(machine));
  const env = { ...process.env, CCMUX_CONFIG: config, CCMUX_RC_PREFIX: machine.rcPrefix, CCMUX_STATE_DIR: root,
    CCMUX_DATA_DIR: join(root, "data"), CCMUX_CACHE_DIR: join(root, "cache") };
  const start = () => Bun.spawn([process.execPath, "--no-env-file", join(import.meta.dir, "../src/cli.ts"), "daemon"],
    { env, stdin: "ignore", stdout: "ignore", stderr: "pipe" });
  let generation: string | undefined;
  try {
    for (let run = 0; run < 2; run++) {
      const daemon = start(), client = createControlClient({ socket: controlSocket(machine), timeoutMs: 1000 });
      const log = new Response(daemon.stderr).text();
      try {
        let live: Awaited<ReturnType<typeof client.external>> | undefined;
        for (let n = 0; n < 100; n++) {
          try { const result = await client.external(); if (result.status === "live") { live = result; break; } } catch {}
          await Bun.sleep(50);
        }
        expect(live?.sessions.map((s) => s.identity.threadId)).toEqual([UUID, OTHER]);
        expect(live?.generation).not.toBe(generation); generation = live?.generation;
        const abort = new AbortController(), stream = await client.watchExternal.withOptions({ signal: abort.signal });
        await stream.next();
        const next = stream.next();
        f.provider.publish("native-status", JSON.stringify({ method: "thread/status/changed", params: { threadId: UUID, status: { type: "idle" } } }));
        expect((await next).value.sessions.find((s: { identity: { threadId: string } }) => s.identity.threadId === UUID)?.turnState.state).toBe("idle");
        abort.abort(); await stream.return?.();
      } finally {
        await client.close(); daemon.kill("SIGTERM"); await daemon.exited;
