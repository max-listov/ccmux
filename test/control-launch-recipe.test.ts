...
function configured(root: string) {
  ...
  const definition = {
    ...
    flags: [
      ...
      '-c',
      'model_providers.provider-a.wire_api="responses"',
    ],
...
test('public create accepts only a safe immutable recipe reference', () => {
  ...
  expect(ControlCreateSchema.parse(safe).launchRecipe).toEqual({
    id: 'provider-a',
    revision: 'r1',
  });
  for (const extra of [
    ...
    expect(() =>
      ControlCreateSchema.parse({ ...safe, launchRecipe: { ...safe.launchRecipe, ...extra } }),
    ).toThrow();
});
...
test('host recipe reuses flags and envFile without putting secret values in public metadata or argv', () => {
  ...
  try {
    ...
    const resolved = resolveControlLaunchRecipe(
      f.machine,
      root,
      { id: 'provider-a', revision: 'r1' },
      [],
    );
    expect(resolved.envFile).toBe(f.envFile);
    expect(resolved.launchRecipe).toMatchObject({
      id: 'provider-a',
      revision: 'r1',
    ...
    const session = makeSession({
      ...
      launchRecipe: resolved.launchRecipe,
    });
    ...
    expect(() =>
      resolveControlLaunchRecipe(f.machine, root, { id: 'provider-a', revision: 'r1' }, [
        '--model',
        'other',
      ]),
...
test('recipe create is one-writer idempotent and rejects unknown, removed or changed definitions before spawn', async () => {
  ...
  try {
    ...
    const restarted = await createControlSession(
      restartedMachine,
      input,
    ...
    const changed = makeMachine({
      ...f.machine,
      launchRecipes: {
        'provider-a': {
          ...f.definition,
          flags: [...f.definition.flags, '-c', 'model="changed"'],
        },
      },
    });
    await expect(
      createControlSession(changed, input, new AbortController().signal, create),
    ).rejects.toMatchObject({ code: 'IDEMPOTENCY_CONFLICT' });
    expect(calls).toBe(1);

    const modeChanged = makeMachine({
      ...f.machine,
      launchRecipes: {
        'provider-a': {
          ...f.definition,
    ...
    await expect(
      createControlSession(modeChanged, input, new AbortController().signal, create),
    ).rejects.toMatchObject({ code: 'IDEMPOTENCY_CONFLICT' });
    expect(calls).toBe(1);
    const persisted = loadSessions(f.machine)[0];
    expect(persisted).toBeDefined();
    if (persisted === undefined) throw new Error('persisted recipe session missing');
    expect(() => verifyManagedLaunchRecipe(changed, persisted)).toThrow(
      'Launch recipe is unavailable',
    );

    const removed = makeMachine({ ...f.machine, launchRecipes: {} });
    await expect(
      createControlSession(removed, input, new AbortController().signal, create),
    ).rejects.toMatchObject({
      code: 'LAUNCH_RECIPE_UNAVAILABLE',
    ...
    try {
      ...
      await expect(
        createControlSession(
          unknown,
          { ...input, requestId: crypto.randomUUID(), workspace: unknownRoot },
          new AbortController().signal,
          create,
        ),
    ...
    try {
      const unavailable = makeMachine({
        ...f.machine,
        stateDir: join(unavailableRoot, 'state'),
        launchRecipes: {
          'provider-a': { ...f.definition, envFile: join(unavailableRoot, 'missing.env') },
        },
      });
      await expect(
        createControlSession(
          unavailable,
          { ...input, requestId: crypto.randomUUID(), workspace: unavailableRoot },
          new AbortController().signal,
          create,
        ),
      ).rejects.toMatchObject({ code: 'LAUNCH_RECIPE_UNAVAILABLE' });
