...
import { capturePane, loadPasteBuffer, sendKeysLiteral, setPaneInputEnabled, submitPasteBuffer, tmuxArgv } from "../src/tmux/tmux.ts";
import { makeMachine } from "./helpers.ts";

test("pane input cannot interleave with the classified chat paste+Enter queue", async () => {
  const tmuxBin = Bun.which("tmux");
  if (tmuxBin === null) throw new Error("tmux is required for the chat input-gate integration test");
  ...
  try {
    ...
    expect(await submitPasteBuffer(m, name, buffer, "11111111-1111-4111-8111-111111111111")).toBe(true);
    await Bun.sleep(100);
