...
import { inheritsUndeclaredEnv, isReservedEnvKey, parseEnvFile, sessionEnvRecipe } from "../src/agent/sessionEnv.ts";
...
const inherited = { PATH: "/usr/bin", PROJECT_SECRET: "leak-me", SHARED: "from-dotenv", REAL: "kept" };
...
test("quotes, comments, export, expansion and multi-line values", () => {
  ...
  expect(parsed).toEqual({ A: "plain", B: "double", C: "raw $A", D: "pre-plain", E: "line1\nline2" });
});
...
test("a project file cannot reconfigure its own supervisor", () => {
  ...
  writeFileSync(join(dir, "proj", "hijack.env"), "CCMUX_STATE_DIR=/tmp/hijacked\nCCMUX_SESSION=other\nOK=1\n");
  const { env, refused } = sessionEnvRecipe({ dir: proj(), envFile: "hijack.env" }, inherited, undefined);
...
const stamp = (inputs: LaunchStamp["inputs"]): LaunchStamp => ({
  version: "0", hash: "h", permissionMode: "auto", chatEnabled: true, promptModules: [], envKeys: [], inputs, ts: 0,
});
...
test("a session launched BEFORE the recipe existed is on the migration list", () => {
  ...
  expect(inheritsUndeclaredEnv({ dir: proj(), archived: false }, stamp(null), undefined)).toBe(true);
});

test("a session launched UNDER the recipe with nothing declared is not — the file beside it is inert", () => {
  expect(inheritsUndeclaredEnv({ dir: proj(), archived: false }, stamp({ env: null }), undefined)).toBe(false);
});

test("a stamp that digested a directory file is still carrying it", () => {
  expect(inheritsUndeclaredEnv({ dir: proj(), archived: false }, stamp({ env: "d1" }), undefined)).toBe(true);
});

test("declared, archived, or unknown are never on the list", () => {
  expect(inheritsUndeclaredEnv({ dir: proj(), archived: false, envFile: ".env" }, stamp(null), undefined)).toBe(false);
  expect(inheritsUndeclaredEnv({ dir: proj(), archived: true }, stamp(null), undefined)).toBe(false);
...
test("the pane re-exec tells the runtime not to load .env from wherever it lands", () => {
  expect(withNoEnvFile(["/bin/bun", "/opt/ccmux.js"])).toEqual(["/bin/bun", "--no-env-file", "/opt/ccmux.js"]);
});
