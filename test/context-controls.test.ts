...
test('compact ACK is not completion; same-ID retry, stale events and cursor reset are exact', async () => {
  ...
  const api: NativeContextApi = {
    history: async () => boundedHistoryPage(f.m, f.s, [], null, 'complete'),
...
test('lost compact ACK survives restart without retry and reconciles only a new native marker', async () => {
  const f = await fixture();
  let calls = 0,
    marker: string | null = 'old-marker';
  const api: NativeContextApi = {
    history: async () => boundedHistoryPage(f.m, f.s, [], null, 'complete'),
    compactionMarker: async () => marker,
    compact: async () => {
      calls++;
      throw new Error('private provider cause');
    },
  };
  const request = { operationId: crypto.randomUUID(), generation: f.generation };
  try {
...
test('completed compact receipt waits for durable replay boundary and duplicates do not republish', async () => {
  ...
  try {
    ...
    const completion = observeContextCompletion(
      f.m,
      f.s,
      f.generation,
      'completed-marker',
...
test('context boundary flush drains the offered reset behind an in-flight content write', async () => {
  const f = await fixture(),
    producer = new ContentProducer(f.m, f.s, f.generation);
  try {
    producer.buffer.text('assistant', 'turn-a', 'item-a', 'before compact', 'replace', true);
    producer.publish();
    const cursor = { generation: f.generation, sequence: producer.buffer.snapshot().sequence };
    const prior = producer.writer.flush();
    producer.buffer.resetContext();
    producer.publish();
    await producer.writer.flushPending();
    await prior;
    expect(readContent(f.m, f.s, cursor)).toMatchObject({
      reset: 'context',
      baseline: [],
      records: [],
    });
...
test('history projects native IDs and reasoning summaries, never hidden reasoning or raw tool data', async () => {
  const f = await fixture();
  const calls: string[] = [];
  const api = codexContextApi(f.m, f.s, {
    close() {},
    async request(method) {
      ...
      return {
        ...
        data: [
          {
            turnId: 'turn-a',
            item: { id: 'assistant-a', type: 'agentMessage', text: '界'.repeat(12_000) },
          },
          {
            turnId: 'turn-a',
            item: {
...
test('fork ACK preserves destination identity; lost ACK never dispatches another fork', async () => {
  ...
  let calls = 0,
    resumes = 0;
  const source = {
    target: managedPeer(f.m.rcPrefix, f.s),
    registration: f.s.registrationGeneration ?? f.s.uuid,
  ...
  try {
    ...
    const adapter = {
      fork: async () => {
        calls++;
        return { id: 'destination-native' };
      },
      identity: (response: { id: string }) => response.id,
    ...
    const broken = {
      ...
      fork: async () => {
        calls++;
        throw new Error('private missing ACK');
      },
    };
    await expect(
...
test('compact refuses already accepted native input before dispatch', async () => {
  const f = await fixture();
  try {
    await writeRuntimeInput(f.m, f.s, {
...
test('current Codex completion is the completed item, not start or retired thread notification', () => {
  ...
  expect(isCodexContextCompletion({ method: 'item/completed', params }, 'another-thread')).toBe(
    false,
  );
        ...
        return new Response(
          JSON.stringify([
            {
              info: {
                ...
                role: 'user',
                summary: { title: 'user summary metadata' },
                time: { created: 1 },
              },
              parts: [
                {
          ...
          ]),
          {
            headers: {
              ...
              'X-Next-Cursor': 'opaque-native-cursor',
            },
          },
        );
      },
...
test('Codex history cancellation rejects before a delayed native reply without closing the shared writer', async () => {
  ...
  try {
    const api = codexContextApi(f.m, f.s, rpc),
...
test('cancelled Codex context RPC retains one slot until its late ACK and never accumulates native reads', async () => {
  ...
  const rpc = {
    ...
    async request() {
      calls++;
      if (calls === 1) await released.promise;
      return { data: [], nextCursor: null };
    },
  };
  try {
    await expect(
      codexContextApi(f.m, f.s, rpc).history({ limit: 1 }, AbortSignal.abort()),
    ).rejects.toThrow();
    expect(calls).toBe(0);
    await expect(
      codexContextApi(f.m, f.s, rpc).history({ limit: 1 }, AbortSignal.timeout(20)),
    ).rejects.toThrow();
    for (let attempt = 0; attempt < 20; attempt++) {
      await expect(
        codexContextApi(f.m, f.s, rpc).history({ limit: 1 }, AbortSignal.timeout(100)),
      ).rejects.toMatchObject({ code: 'CONTEXT_RPC_PENDING' });
    }
    await expect(
      codexContextApi(f.m, f.s, rpc).compactionMarker(AbortSignal.timeout(100)),
    ).rejects.toMatchObject({ code: 'CONTEXT_RPC_PENDING' });
    await expect(
      codexContextApi(f.m, f.s, rpc).compact(AbortSignal.timeout(100)),
    ).rejects.toMatchObject({ code: 'CONTEXT_RPC_PENDING' });
    expect(calls).toBe(1);
    expect(closed).toBe(0);
    released.resolve();
    await Bun.sleep(0);
    expect(
      (await codexContextApi(f.m, f.s, rpc).history({ limit: 1 }, AbortSignal.timeout(100)))
        .entries,
    ).toEqual([]);
    expect(calls).toBe(2);
    expect(closed).toBe(0);
...
test('single background context pump leaves heartbeat and approval usable during a stalled history read', async () => {
  ...
  try {
    await writeHistoryMailbox(f.m, f.s, {
    ...
    const run = (signal: AbortSignal) =>
      applyContextCommands(f.m, f.s, f.generation, codexContextApi(f.m, f.s, rpc), signal);
    ...
    for (let tick = 0; tick < 5; tick++) {
      ...
      await status.write(f.projection.snapshot());
    }
    await rpc.respond();
...
test('OpenCode owner pump keeps native status and permission RPC usable while history fetch is pending', async () => {
  ...
  let histories = 0,
    ...
    approvals = 0;
  const errors: unknown[] = [],
    pump = new NativeContextPump((error) => errors.push(error));
  const client = createOpencodeClient({
    baseUrl: 'http://native.invalid',
    throwOnError: true,
    fetch: Object.assign(
  ...
  try {
    await writeHistoryMailbox(f.m, session, {
      id: crypto.randomUUID(),
      generation: f.generation,
      expiresAt: Date.now() + 5_000,
      query: { limit: 1 },
    ...
    const run = (signal: AbortSignal) =>
      applyContextCommands(
        ...
        session,
        f.generation,
        openCodeContextApi(f.m, session, client),
        signal,
      );
    pump.start(AbortSignal.timeout(5_000), run);
    await ready.promise;
