...
test('lost native create response reconciles the reservation without a second POST', async () => {
  ...
  const server = Bun.serve({
    ...
    fetch(request) {
      if (request.method === 'POST') {
        posts++;
        return new Response('lost after commit', { status: 502 });
      }
  ...
  try {
    await expect(
      admitOpenCode(m, initial, { client, version: '1.18.20' }, true, new AbortController().signal),
    ).rejects.toBeDefined();
    const resumed = await admitOpenCode(
      m,
      initial,
      { client, version: '1.18.20' },
      true,
      new AbortController().signal,
    );
    expect(posts).toBe(1);
    expect(resumed.uuid).toBe(initial.uuid);
    expect(resumed.nativeSession?.id).toBe(native.id);
  } finally {
...
test('missing or corrupt admission evidence refuses instead of spawning a replacement conversation', async () => {
  ...
  try {
    writeFileSync(path, JSON.stringify({ generation: s.registrationGeneration, nativeId: null }), {
      mode: 0o600,
    });
    await expect(
      admitOpenCode(m, initial, { client, version: '1.18.20' }, true, new AbortController().signal),
...
test('lost prompt ACK is proven by the exact native user record and never reposted', async () => {
  ...
  const input = {
    ...
    turnOptions: {
      revision: 0,
      options: { runtime: 'opencode', model: { provider: 'provider-a', model: 'model-a' } },
    },
  } satisfies Parameters<typeof writeRuntimeInput>[2];
  ...
  const server = Bun.serve({
    ...
    fetch(request) {
      if (request.method === 'POST') {
        posts++;
        return new Response('lost after native admission', { status: 502 });
      }
  ...
  try {
    await expect(
      applyOpenCodeInput(m, s, client, projection, new AbortController().signal),
    ).rejects.toBeDefined();
