...
test("full policy → option 2 (keep all context)", () => {
  expect(promptAnswer(PICKER, cfg("full"))).toBe("2");
});

test("summary policy → option 1", () => {
  expect(promptAnswer(PICKER, cfg("summary"))).toBe("1");
});

test("off policy → never answers (a human will)", () => {
  expect(promptAnswer(PICKER, cfg("off"))).toBeNull();
});

test("no picker on screen → null", () => {
  expect(promptAnswer("just a normal prompt\n❯ ", cfg("full"))).toBeNull();
});
...
test("reordered menu → reads the ACTUAL option number, not a hardcoded one", () => {
  ...
  expect(promptAnswer(reordered, cfg("full"))).toBe("1");
  expect(promptAnswer(reordered, cfg("summary"))).toBe("2");
});
...
test("both exact option labels required — a passing mention doesn't trigger", () => {
  ...
  expect(promptAnswer("we could resume from summary of the doc", cfg("full"))).toBeNull();
});
