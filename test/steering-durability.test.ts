...
test('concurrent same-ID requests submit once; changed payload and caller cannot acquire the operation', async () => {
  ...
  await expect(
    steerNativeTurn(
      f.machine,
      f.principal,
      { ...f.input, body: 'A different correction.' },
      f.signal,
      f.deps,
    ),
  ).rejects.toMatchObject({ code: 'IDEMPOTENCY_CONFLICT' });
  await expect(
    steerNativeTurn(f.machine, { ...f.principal, machine: 'host-c' }, f.input, f.signal, f.deps),
  ).rejects.toMatchObject({ code: 'IDEMPOTENCY_CONFLICT' });
  expect(f.submissions).toHaveLength(1);
});
...
test('lost ack survives restart and reconciles only exact client identity on the original turn', async () => {
  const f = await steeringFixture();
  f.setSteer(async () => {
    throw new Error('private provider transport fixture');
  });
  const uncertain = await f.run();
  expect(uncertain.state).toBe('uncertain');
  f.setState({
    protocol: 1,
    status: 'live',
    reason: null,
    snapshot: { ...f.projection.snapshot(), generation: crypto.randomUUID(), state: 'idle' },
  });
  ...
  f.receipts([
    {
      id: 'different-turn',
      status: 'completed',
      items: [{ type: 'userMessage', clientId: uncertain.clientUserMessageId }],
    },
  ]);
  expect((await f.read()).operation?.state).toBe('uncertain');
  f.receipts([
    {
      id: f.input.expectedTurnId,
      status: 'interrupted',
      items: [{ type: 'userMessage', clientId: uncertain.clientUserMessageId }],
    },
  ]);
  expect((await f.read()).operation).toMatchObject({
    state: 'submitted',
  ...
  const journal = readFileSync(
    join(managedRuntimeRoot(f.machine, f.session), 'steering.json'),
    'utf8',
  );
  expect(journal).not.toContain(f.input.body);
  ...
  f.receipts([
    {
      id: f.input.expectedTurnId,
      status: 'inProgress',
      items: [{ type: 'userMessage', clientId: f.input.operationId }],
    },
  ]);
  expect((await f.read()).operation?.state).toBe('uncertain');
  f.receipts([
    {
      id: f.input.expectedTurnId,
      status: 'inProgress',
      items: [{ type: 'userMessage', clientId: receipt.clientUserMessageId }],
    },
  ]);
  expect((await f.read()).operation?.state).toBe('submitted');
  expect(f.submissions).toHaveLength(1);
});
...
test('turn completion at native admission is not converted into a replacement turn or implicit retry', async () => {
  ...
  const mismatch = await steeringFixture();
  mismatch.setSteer(async () => ({ turnId: 'replacement-turn' }));
...
test('registration replacement cannot consume old receipts; archive preserves accepted operation evidence', async () => {
  ...
  await writeSessionsUnlocked(f.machine, [
    { ...f.session, registrationGeneration: crypto.randomUUID() },
  ]);
...
test('journal refuses symlinks and corruption without changing target content or sending native input', async () => {
  const f = await steeringFixture();
  readSteeringJournal(f.machine, f.session);
  const outside = join(f.root, 'outside.json'),
    journal = join(managedRuntimeRoot(f.machine, f.session), 'steering.json');
  ...
  const broken = await steeringFixture();
  readSteeringJournal(broken.machine, broken.session);
  writeFileSync(join(managedRuntimeRoot(broken.machine, broken.session), 'steering.json'), '{', {
    mode: 0o600,
  });
  await expect(broken.run()).rejects.toMatchObject({ code: 'STEERING_UNAVAILABLE' });
  expect(broken.submissions).toEqual([]);
});
...
test('aborted calls cannot write intent, and native generation recheck catches changes during preflight', async () => {
  ...
  abort.abort();
  await expect(
    steerNativeTurn(f.machine, f.principal, f.input, abort.signal, f.deps),
  ).rejects.toThrow();
  expect(f.submissions).toEqual([]);
  const stale = await steeringFixture();
  stale.deps.capture = async () => {
    stale.setState({
      protocol: 1,
      status: 'live',
      reason: null,
      snapshot: { ...stale.projection.snapshot(), generation: crypto.randomUUID() },
    });
    return '›\n gpt-fixture · /workspace';
  };
  await expect(stale.run()).rejects.toMatchObject({ code: 'IDENTITY_MISMATCH' });
  expect(readSteeringJournal(stale.machine, stale.session).operations).toEqual([]);
  expect(stale.submissions).toEqual([]);
});
