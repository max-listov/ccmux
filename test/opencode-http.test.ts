...
import {
  boundedOpenCodeFetch,
  OPENCODE_EVENT_MAX_BYTES,
  OPENCODE_HTTP_MAX_BYTES,
} from '../src/agent/opencode/http.ts';
  ...
  const server = Bun.serve({
    ...
    fetch(request) {
      return new URL(request.url).pathname === '/redirect'
        ? Response.redirect('http://127.0.0.1:1/')
        : new Response('x'.repeat(OPENCODE_HTTP_MAX_BYTES + 1), {
...
test('native SSE limits apply per frame, including fragmented CRLF delimiters', async () => {
    ...
    fetch(request) {
      ...
      return new Response(
        new ReadableStream<Uint8Array>({
          start(controller) {
            if (oversized)
              controller.enqueue(Buffer.from(`data: ${'x'.repeat(OPENCODE_EVENT_MAX_BYTES)}`));
            else
              for (let i = 0; i < 12; i++) {
  ...
  try {
    expect((await (await boundedOpenCodeFetch(server.url)).text()).length).toBeGreaterThan(
      OPENCODE_EVENT_MAX_BYTES,
    );
    await expect(
      (await boundedOpenCodeFetch(new URL('/large', server.url))).text(),
    ).rejects.toThrow('bounded frame');
  } finally {
    await server.stop(true);
  }
});
