...
test('content append offsets preserve UTF-8 and answers beyond the old tail', () => {
  ...
  expect(
    Buffer.byteLength(JSON.stringify(ContentSnapshotSchema.parse(buffer.snapshot()))),
  ).toBeLessThan(CONTENT_FILE_MAX_BYTES);
});
...
test('large output is explicitly truncated while terminal and requests survive a burst', () => {
  ...
  for (let i = 0; i < 1000; i++)
    buffer.text('assistant', 'active', 'long', 'x'.repeat(1024), 'append');
  ...
  expect(contentFrame(snapshot, { generation: crypto.randomUUID(), sequence: 0 }).reset).toBe(
    'generation',
  );
  ...
  buffer.resetContext();
  expect(contentFrame(buffer.snapshot(), { generation, sequence: snapshot.sequence }).reset).toBe(
    'context',
  );
});
...
test('truncated completed text rejects late deltas and stale started events', () => {
  ...
  expect(final.baseline[0]).toMatchObject({
    complete: false,
    totalBytes: 80_000,
...
test('Codex allows assistant and explicit summary content but no private reasoning or tool output', () => {
  ...
  observeCodexContent(buffer, s.uuid, {
  ...
  observeCodexContent(buffer, s.uuid, {
    ...
    params: { ...params, delta: 'hello' },
  });
  observeCodexContent(buffer, s.uuid, {
    method: 'item/reasoning/summaryTextDelta',
    params: { ...params, delta: 'public summary', summaryIndex: 0 },
  });
  observeCodexContent(buffer, s.uuid, {
    method: 'item/agentMessage/delta',
    params: { ...params, threadId: crypto.randomUUID() },
  });
  expect(JSON.stringify(buffer.snapshot())).not.toContain('PRIVATE_CANARY');
  expect(buffer.snapshot().baseline.map((item) => item.kind)).toEqual([
    'assistant',
    'reasoning-summary',
  ]);
});
...
test('OpenCode only streams known assistant text parts and correlated native completion', () => {
  ...
  expect(buffer.snapshot().baseline.at(-1)).toMatchObject({
    kind: 'terminal',
    turnId: 'msg_user',
    status: 'completed',
  });
...
test('Codex native Plan deltas reconcile to the authoritative completed plan', () => {
  ...
  observeCodexContent(buffer, s.uuid, {
    ...
    params: { ...params, delta: 'Draft plan' },
  });
  ...
  expect(buffer.snapshot().baseline[0]).toMatchObject({
    kind: 'assistant',
    text: 'Final plan',
    complete: true,
  });
