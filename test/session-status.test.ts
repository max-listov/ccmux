...
test('resolveLiveState requires positive or bounded evidence for each transition', () => {
  expect(
    resolveLiveState('working', lifecycle('idle', 'Stop'), { settled: true, why: 'turn-ended' }),
  ).toBe('working');
  expect(
    resolveLiveState('idle', lifecycle('working', 'UserPromptSubmit'), {
      settled: false,
      why: 'working',
    }),
  ).toBe('idle');
  expect(
    resolveLiveState('indeterminate', lifecycle('idle', 'Stop'), {
      settled: false,
      why: 'quiet-unproven',
    }),
  ).toBe('idle');
  expect(
    resolveLiveState('indeterminate', lifecycle('working', 'UserPromptSubmit'), {
      settled: false,
      why: 'quiet-unproven',
    }),
  ).toBe('working');
...
test('one ready frame without a spinner does not split one working turn', () => {
  ...
  expect(
    frames.map((paneState) =>
      resolveLiveState(paneState, lifecycle('working', 'UserPromptSubmit'), evidence),
    ),
...
test('anonymous work expires instead of becoming permanently working', () => {
  ...
  expect(
    resolveLiveState('indeterminate', lifecycle('idle', SUPERVISOR_CLOSED_EVENT), settled),
  ).toBe('idle');
  expect(
    resolveLiveState('indeterminate', lifecycle('idle', 'SessionStart'), {
      settled: false,
      why: 'not-drawn',
    }),
  ).toBe('idle');
