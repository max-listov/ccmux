...
test("coalesced content publication, slow reader recovery and cancellation stay bounded", async () => {
  ...
  try {
    producer.buffer.text("assistant", "turn-a", "item-a", "", "replace"); producer.publish(); await producer.writer.flush();
    ...
    for (let i = 0; i < 700; i++) { producer.buffer.text("assistant", "turn-a", "item-a", "hello", "append"); producer.publish(); }
    ...
    expect(() => readContent(m, { ...s, registrationGeneration: crypto.randomUUID() })).toThrow("Native content is unavailable");
  } finally { abort.abort(); await producer.close(); rmSync(root, { recursive: true, force: true }); }
});
...
test("reader does not publish content from a retired observer generation", async () => {
  ...
  try {
    old.buffer.text("assistant", "old-turn", "old-item", "old content", "replace", true); old.publish(); await old.writer.flush();
    expect(readContent(m, s)).toMatchObject({ status: "unavailable", records: [], baseline: [] });
  } finally { await old.close(); rmSync(root, { recursive: true, force: true }); }
});
...
test("failed subscriber construction closes partially opened filesystem watchers", async () => {
  ...
  `;
  const child = Bun.spawn([process.execPath, "--no-env-file", "--eval", source], { stdout: "pipe", stderr: "pipe" });
  try {
...
test("idle lease and native settings updates stream without inventing content sequence", async () => {
  ...
  producer.publish(); await producer.writer.flush();
  const abort = new AbortController();
  const iterator = subscribeControlNative(m, managedPeer(m.rcPrefix, s), null, abort.signal)[Symbol.asyncIterator]();
  try {
    const first = await deadline(iterator.next(), "Initial native metadata missing");
    projection.reconcile({ type: "idle" }, 0);
    const fresh = projection.snapshot();
    fresh.nativeSelection = { model: { provider: "openai", model: "model-new" }, options: null, source: "settings", turnId: null };
    ...
    expect(Date.parse(next.value?.expiresAt ?? "")).toBeGreaterThan(Date.parse(first.value?.expiresAt ?? ""));
    expect(next.value?.records).toEqual([]);
  } finally {
    abort.abort(); await iterator.return?.(); await producer.close(); rmSync(root, { recursive: true, force: true });
  }
});
