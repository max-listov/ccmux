...
import { droppedDeadAgentKeys, staleAgentSocket, withoutDeadAgentEnv } from "../src/agent/sshEnv.ts";
...
test("a socket that is already gone is dropped, with the connection vars beside it", () => {
  ...
  const env = { SSH_AUTH_SOCK: "/tmp/ssh-gone/agent.404", SSH_CONNECTION: "1.2.3.4 22 5.6.7.8 22", PATH: "/bin" };
...
test("what was dropped is reportable, so the reason survives in the log", () => {
  expect(droppedDeadAgentKeys({ SSH_AUTH_SOCK: "/tmp/ssh-gone/a", SSH_TTY: "/dev/pts/0" }).sort())
