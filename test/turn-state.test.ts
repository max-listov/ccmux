...
test('a pane that has not painted yet is never written into', () => {
  ...
  expect(
    turnState(
      facts({ paneReady: false, endedOnAssistantText: false, msSinceActivity: 10 * 60_000 }),
    ),
...
test('a selection prompt is not an idle session', () => {
  ...
  expect(turnState(facts({ atMenu: true, msSinceActivity: 60 * 60_000 }))).toEqual({
    settled: false,
    why: 'at-menu',
  });
});
...
test('a clean end settles fast, and the mid-turn gap it was written for still holds', () => {
  expect(turnState(facts({ msSinceActivity: SETTLE_MS }))).toEqual({
    settled: true,
    why: 'turn-ended',
  });
...
test('evidence strength picks the threshold — that is the whole mechanism', () => {
  ...
  expect(turnState(facts({ endedOnAssistantText: true, msSinceActivity: quiet })).settled).toBe(
    true,
  );
  expect(turnState(facts({ endedOnAssistantText: false, msSinceActivity: quiet })).settled).toBe(
    false,
  );
});
...
test('a session that never had a turn is its OWN answer, not an interrupted one', () => {
  ...
  expect(turnState(facts({ endedOnAssistantText: false, msSinceActivity: null }))).toEqual({
    settled: true,
    why: 'never-spoke',
  });
});
