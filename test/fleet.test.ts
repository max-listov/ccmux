...
function row(
  ...
  return {
    session: {
      name,
      dir: `/tmp/${name}`,
      uuid: `00000000-0000-4000-8000-${name.padStart(12, '0')}`,
      flags: [],
      archived: false,
      resumeText: 'continue',
      agent: 'claude',
...
function ext(
  ...
  const suffix = String([...dir].reduce((sum, ch) => sum + (ch.codePointAt(0) ?? 0), 0)).padStart(
    12,
    '0',
  );
  return {
    ...
    writerRuntime: {
      ...
      startTime: null,
      processGroup: null,
      reason: 'desktop owns the writer',
    },
    capabilities: {
      inspect: true,
      attemptAdopt: false,
      ...
      reasons: ['release at source first'],
    },
    lastActivityMs,
    lastModel: null,
    usedTokens: null,
...
describe('buildItems ordering', () => {
  test('managed sort newest-activity first', () => {
    ...
    const { items } = buildItems(
      [row('old', now - 60 * MIN), row('fresh', now), row('mid', now - 5 * MIN)],
      [],
      'host-A',
    );
  ...
  test('activityText is precomputed for managed and external', () => {
    const now = Date.now();
    const { items } = buildItems(
      [row('m', now - 5 * MIN), row('silent', null)],
      [ext('e', now)],
      'host-A',
    );
...
describe('fleet route identity and external evidence', () => {
  test('external selection survives metadata churn but not route identity changes', () => {
    ...
    const metadataChanged: DiscoveredSession = {
      ...original,
      dir: '/tmp/renamed',
      origin: 'app-server',
      lastActivityMs: null,
    ...
    expect(externalSelectionKey({ ...original, host: 'host-B' })).not.toBe(
      externalSelectionKey(original),
    );
    expect(externalSelectionKey({ ...original, provider: 'claude' })).not.toBe(
      externalSelectionKey(original),
    );
  });
  ...
  test('fresh resolution uses the stable key and rejects a replaced route', () => {
    ...
    expect(resolveFleetItem(built, externalSelectionKey(first))?.ext?.threadId).toBe(
      first.threadId,
    );
  ...
  test('writer not observed is not presented as a running external writer', () => {
    const item = ext('worktree', Date.now());
    expect(
      externalToRow({ ...item, writerEvidence: 'none-observed', writerRuntime: null }).running,
    ).toBe(false);
  });
});
