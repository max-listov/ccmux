  ...
  const behavior =
    mode === 'crash'
      ...
      : mode === 'timeout'
        ? ['sleep 30']
        : [
            // Both markers must become visible in ONE step. Written one after the other, a reader
            // landing between them sees a single unambiguous candidate and promotes it — the
            // ambiguity this case is about simply is not there yet. That window is small enough to
            // pass alone and to fail on a loaded machine, which is the worst kind of test: it
            // reddens by luck.
            //
            // So they are staged and the directory is renamed in — and the staging directory is a
            // SIBLING of the scanned root, not a subdirectory of it. Correlation globs
            // `**/rollout-*.jsonl` under `codexSessionsDir`, which descends into any directory
            // there, staging included: measured over 200 runs, a reader's first non-empty
            // observation saw one marker 199 times with the staging directory inside, and 0 times
            // with it beside. Same filesystem, so the rename is still one atomic step.
            `mkdir -p "${codexSessionsDir}.staging"`,
            `printf '{"type":"session_meta","payload":{"id":"11111111-1111-4111-8111-111111111111","originator":"%s"}}\\n' "$CODEX_INTERNAL_ORIGINATOR_OVERRIDE" > "${codexSessionsDir}.staging/rollout-a-11111111-1111-4111-8111-111111111111.jsonl"`,
            `printf '{"type":"session_meta","payload":{"id":"22222222-2222-4222-8222-222222222222","originator":"%s"}}\\n' "$CODEX_INTERNAL_ORIGINATOR_OVERRIDE" > "${codexSessionsDir}.staging/rollout-b-22222222-2222-4222-8222-222222222222.jsonl"`,
            `mv "${codexSessionsDir}.staging" "${codexSessionsDir}/probe"`,
            'sleep 30',
          ];
