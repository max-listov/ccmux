...
import { readSession, parseSession, detect, DEFAULT_TEXT_LIMIT } from "../src/lib.ts";

const claudeLine = (role: "user" | "assistant", text: string, id = role) =>
  JSON.stringify({ type: role, uuid: id, timestamp: id, message: { role, content: [{ type: "text", text }] } });
