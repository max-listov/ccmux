...
test('image-only steer pins exact ordered assets before intent and emits private native paths only', async () => {
  ...
  f.input.body = '';
  f.input.images = [await upload(f), await upload(f)];
  f.setSteer(async (raw) => {
    const native = z
      .object({
        input: z.array(z.object({ type: z.literal('localImage'), path: z.string() }).strict()),
      })
      .parse(raw);
    expect(native.input).toHaveLength(2);
    const store = AttachmentStoreSchema.parse(
      JSON.parse(readFileSync(join(f.root, 'attachments', 'index.json'), 'utf8')),
    );
    expect(store.pins[0]).toMatchObject({
      messageId: f.input.operationId,
      registration: f.input.registrationGeneration,
      target: f.target,
      principal: f.principal,
      references: f.input.images,
    });
  ...
  expect(readSteeringJournal(f.machine, f.session).operations[0]?.receipt.turnId).toBe(
    f.input.expectedTurnId,
  );
  const refs = f.input.images;
  f.input.images = [...refs].reverse();
...
test('future selection defaults cannot claim active-turn image support', async () => {
  const f = await steeringFixture();
  f.input.images = [await upload(f)];
  await expect(
    validateSteeringImages(f.machine, f.session, f.input, f.signal),
