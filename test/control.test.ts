...
import { mkdtempSync, rmSync, statSync, writeFileSync, mkdirSync, symlinkSync } from "node:fs";
...
import { loadLedger, loadCursors, saveCursors } from "../src/chat/store.ts";
...
import { CONTROL_MAX_READERS, CONTROL_MAX_BYTES, currentControlSnapshot } from "../src/control/schema.ts";
...
test("Unix HTTP, CLI and peer-free tools share contract output and auth; no public listener", async () => {
  ...
  const lines: string[] = []; let exitCode = -1;
  await createCli({ name: "control", version: "0.0.0", argv: ["sessions"], services: [proxy], stdin: async () => null,
    stdout: (line) => { lines.push(line); }, stderr: (line) => { throw new Error(line); }, exit: (code) => { exitCode = code; } });
  ...
  await expect(createToolInvoker(bad, { transport: "MCP" }).invokeOrThrow("sessions", {})).rejects.toThrow();
});

test("authorization precedes body handling, managed credentials rotate, and exact identities fail closed", async () => {
  const f = await fixture();
  const invalid = await fetch("http://ccmux.local/control/message", { unix: f.socket, method: "POST",
    headers: { "x-ccmux-session": f.s.name, authorization: "Bearer wrong", "content-type": "application/json" }, body: "{" });
  ...
  for (const target of [{ ...f.target, threadId: crypto.randomUUID() }, { ...f.target, machine: "host-b" }, { ...f.target, agent: "codex" as const }]) {
    await expect(f.client.get({ target })).rejects.toMatchObject({ code: "IDENTITY_MISMATCH", status: 409 });
    await expect(f.client.start({ target })).rejects.toMatchObject({ code: "IDENTITY_MISMATCH" });
    await expect(f.client.message({ target, messageId: crypto.randomUUID(), body: "must not land" })).rejects.toMatchObject({ code: "IDENTITY_MISMATCH" });
  }
  expect(loadLedger(f.m)).toEqual([]);
  expect((await fetch("http://ccmux.local/control/sessions", { unix: f.socket, headers: { origin: "http://example.invalid" } })).status).toBe(403);
});
...
test("message acceptance is durable, identity-authenticated and idempotent without starting a turn", async () => {
  ...
  expect(loadLedger(f.m)[0]).toMatchObject({ from: f.target, to: f.target, body: input.body, defer: true });
  ...
  try { await expect(client.message({ ...input, messageId: crypto.randomUUID() })).rejects.toMatchObject({ code: "BUSY", status: 429 }); }
  finally { if (busy.outcome === "leased") busy.lease.release(); }
...
test("100 reads and resident subscribers reuse the producer; baseline, cancel and reconnect are real socket operations", async () => {
  ...
  const streams = await Promise.all(controllers.map((c) => f.client.watch.withOptions({ signal: c.signal })));
  ...
  reconnect.abort(); await stream.return?.();
}, 10_000);
...
test("slow readers retain only the latest revision; capacity and abort-before-read do not leak", async () => {
  ...
  for (const stream of streams) expect((await stream.next()).value.sequence).toBe(f.p.read().sequence);
...
test("oversize bodies refuse early and cancelled lock waiters cannot append later", async () => {
  ...
  const response = await fetch("http://ccmux.local/control/message", { unix: f.socket, method: "POST",
    headers: { "content-type": "application/json" }, body: JSON.stringify({ body: "x".repeat(70_000) }) });
  expect(response.status).toBe(400);
  expect(await response.json()).toMatchObject({ error: { code: "BAD_REQUEST", message: "JSON body exceeds the 65536-byte limit" } });
  const entered = Promise.withResolvers<void>(), release = Promise.withResolvers<void>();
  const lock = withSessionRegistryLock(f.m, async () => { entered.resolve(); await release.promise; });
  await entered.promise;
  const stop = new AbortController();
  try {
    const request = f.client.message.withOptions({ target: f.target, messageId: crypto.randomUUID(), body: "must never append" }, { signal: stop.signal });
    const rejected = request.catch((error: unknown) => error);
    for (let i = 0; i < 50 && f.owned.controls.mutations.getSnapshot().active === 0; i++) await Bun.sleep(10);
  ...
  for (let i = 0; i < 100 && f.owned.controls.mutations.getSnapshot().active; i++) await Bun.sleep(10);
...
test("native approval/input/working states remain distinct; interruption cannot answer them", async () => {
  ...
  for (const [flag, state] of [["waitingOnApproval", "waiting-approval"], ["waitingOnUserInput", "waiting-input"]] as const) {
    p.event({ method: "thread/status/changed", params: { threadId: f.s.uuid, status: { type: "active", activeFlags: [flag] } } });
    await writer.write(p.snapshot()); await f.publish();
    expect((await f.client.get({ target: f.target })).state).toBe(state);
    await expect(f.client.interrupt({ target: f.target, turnId: "unrelated" })).rejects.toMatchObject({ code: "TURN_MISMATCH" });
    expect((await f.client.wait({ target: f.target, timeoutMs: 20 })).outcome).toBe("timeout");
  }
  p.event({ method: "thread/status/changed", params: { threadId: f.s.uuid, status: { type: "idle" } } });
  await writer.write(p.snapshot()); await f.publish();
  ...
  await f.client.message({ target: f.target, messageId: crypto.randomUUID(), body: "pending pickup" });
...
test("wait never returns a cached idle observation from before the call", async () => {
  ...
  await Bun.sleep(20);
  p.event({ method: "turn/started", params: { threadId: f.s.uuid, turn: { id: "turn-current", status: "inProgress" } } });
  await writer.write(p.snapshot()); await f.publish();
  await Bun.sleep(20);
  p.event({ method: "turn/completed", params: { threadId: f.s.uuid, turn: { id: "turn-current", status: "interrupted" } } });
  await writer.write(p.snapshot()); await f.publish();
  expect(await pending).toMatchObject({ outcome: "interrupted", state: { turn: { id: "turn-current" } } });
});
