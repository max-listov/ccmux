...
import { readControlModels, type ControlModelsConnector } from "../src/control/models.ts";
import {
...
const providerModel = (id: string, over: Record<string, unknown> = {}) => ({
  ...
  displayName: id.toUpperCase(), description: `${id} description`, modelSpecialty: null,
  hidden: false, supportedReasoningEfforts: [effort("low"), effort("medium"), effort("high")],
  defaultReasoningEffort: "medium", inputModalities: ["text", "image"], supportsPersonality: false,
  additionalSpeedTiers: [], serviceTiers: [tier], defaultServiceTier: null, isDefault: false,
...
function fakeAppServer(codexHome: string) {
  ...
  const server = Bun.serve<unknown>({ unix: join(codexHome, "app-server-control", "app-server-control.sock"),
    ...
    websocket: {
      ...
      message(ws, raw) {
        ...
        if (behavior === "error") { ws.send(JSON.stringify({ id: decoded.id, error: { code: -32000, message: "provider failed" } })); return; }
        if (behavior === "malformed") { ws.send(JSON.stringify({ id: decoded.id, result: { data: [{ id: 42 }], nextCursor: null } })); return; }
        if (behavior === "oversize") { ws.send(JSON.stringify({ id: decoded.id, result: { data: [providerModel("model-a", { description: "x".repeat(2_049) })], nextCursor: null } })); return; }
        if (behavior === "oversize-page") {
          const heavy = providerModel("model-heavy", { description: "x".repeat(2_048),
            supportedReasoningEfforts: Array.from({ length: 32 }, (_, index) => ({
              reasoningEffort: `e${index}`, description: "r".repeat(1_024) })) });
          const light = providerModel("model-light", { description: "y".repeat(64) });
          ws.send(JSON.stringify({ id: decoded.id, result: { data: [...Array.from({ length: 10 }, () => heavy), ...Array.from({ length: 54 }, () => light)], nextCursor: null } }));
          return;
        }
  ...
  return {
    requests,
    set(value: Behavior) { behavior = value; },
...
async function fixture(options: { extraClaudeSession?: boolean } = {}) {
  ...
  const m = makeMachine({ stateDir: root, rcPrefix: "host-a", projectsDir: join(root, "history"), chatEnabled: true, codexHome });
  const s = makeSession({ name: "agent-a", dir: root, agent: "codex", runtime: "app-server", chat: true });
  const sessions = [s];
  if (options.extraClaudeSession) sessions.push(makeSession({ name: "agent-b", dir: root, agent: "claude", runtime: "tui", chat: true }));
  await writeSessionsUnlocked(m, sessions);
  const p = new ControlPublisher(m);
  const monitoring = new MonitoringPublisher();
  const publish = async () => {
    monitoring.begin(m); monitoring.sample(m, s, 1, "❯\n? for shortcuts", UNSEEN);
  ...
  cleanup.push(async () => { await client.close(); p.close(); provider.close();
    await owned.server.shutdown({ gracePeriodMs: 200, forceTimeoutMs: 100 }); await owned.observability.close();
    rmSync(root, { recursive: true, force: true }); });
  return { m, s, p, publish, owned, client, socket, root, codexHome, provider,
    target: managedPeer(m.rcPrefix, s),
    claudeTarget: options.extraClaudeSession ? managedPeer(m.rcPrefix, sessions[1]!) : null,
...
test("pagination is deterministic, provider-cursored and bounded by the strict input schema", async () => {
  ...
  expect(f.provider.requests[1]).toEqual({ method: "model/list", params: { limit: 1, cursor: "1" } });
  ...
  expect(ControlModelsReadSchema.safeParse({ target: f.target, cursor: "x".repeat(4_097) }).success).toBe(false);
...
test("unknown identities fail closed before any provider contact", async () => {
  const f = await fixture();
  for (const target of [{ ...f.target, threadId: crypto.randomUUID() }, { ...f.target, machine: "host-b" }, { ...f.target, agent: "claude" as const }]) {
    await expect(f.client.models({ target })).rejects.toMatchObject({ code: "IDENTITY_MISMATCH", status: 409 });
    await expect(f.remote.models({ target })).rejects.toMatchObject({ code: "IDENTITY_MISMATCH", status: 409 });
  }
...
test("non-owned runtimes have no model catalog", async () => {
  ...
  await expect(f.client.models({ target: f.claudeTarget! })).rejects.toMatchObject({ code: "UNSUPPORTED", status: 409 });
  expect(f.provider.requests).toEqual([]);
});
...
test("provider failures fail closed instead of substituting a local catalog", async () => {
  ...
  await expect(f.client.models({ target: f.target })).rejects.toMatchObject({ code: "UNAVAILABLE", status: 503 });
  await expect(f.remote.models({ target: f.target })).rejects.toMatchObject({ code: "UNAVAILABLE", status: 503 });
...
test("unconfigured provider homes fail closed", async () => {
  ...
  await expect(readControlModels(m, { target: managedPeer(m.rcPrefix, s), cursor: null, limit: 64, includeHidden: false },
    new AbortController().signal)).rejects.toMatchObject({ code: "UNAVAILABLE", status: 503 });
});

test("handler bounds drop provider extras, honor optional efforts and reject oversized pages", async () => {
  const root = mkdtempSync("/tmp/ccmux-models-unit-");
  cleanup.push(async () => rmSync(root, { recursive: true, force: true }));
  const m = makeMachine({ stateDir: root, rcPrefix: "host-a", projectsDir: join(root, "history"), codexHome: join(root, "codex") });
  const s = makeSession({ name: "agent-a", dir: root, agent: "codex", runtime: "app-server" });
  await writeSessionsUnlocked(m, [s]);
  ...
  const connectThrowing: ControlModelsConnector = async () => { throw new Error("connect failed"); };
  await expect(readControlModels(m, { target, cursor: null, limit: 64, includeHidden: false },
    new AbortController().signal, connectThrowing)).rejects.toMatchObject({ code: "UNAVAILABLE", status: 503 });
  await expect(readControlModels(m, { target, cursor: null, limit: 64, includeHidden: false },
    new AbortController().signal, connect(() => ({ data: Array.from({ length: 65 }, () => providerModel("model-a")), nextCursor: null }))))
    .rejects.toMatchObject({ code: "UNAVAILABLE", status: 503 });
  const cancel = new AbortController();
  cancel.abort();
  const connectHanging: ControlModelsConnector = (_machine, options) => new Promise<CodexAppRpc>((_resolve, reject) => {
    options.signal?.throwIfAborted();
    options.signal?.addEventListener("abort", () => reject(new Error("cancelled")), { once: true });
  });
  await expect(readControlModels(m, { target, cursor: null, limit: 64, includeHidden: false }, cancel.signal,
    connectHanging)).rejects.toMatchObject({ code: "UNAVAILABLE", status: 503 });
});
...
test("declared service dispatch keeps the envelope, effect metadata and response budget for model.list", async () => {
  ...
  await expect(f.remote.models({ target: f.target })).rejects.toMatchObject({ code: "RESPONSE_TOO_LARGE", status: 500 });
});
...
test("reads admission bounds provider connections; cancellation and deadline release the caller", async () => {
  ...
  const cancelled = f.client.models.withOptions({ target: f.target }, { signal: stop.signal }).catch((error: unknown) => error);
  await Bun.sleep(50);
  stop.abort();
  ...
  await expect(f.client.models({ target: f.target })).rejects.toMatchObject({ code: "TIMEOUT", status: 504 });
  for (let i = 0; i < 100 && f.owned.controls.reads.getSnapshot().active; i++) await Bun.sleep(20);
  expect(f.owned.controls.reads.getSnapshot().active).toBe(0);
  f.provider.set("ok");
  expect((await f.client.models({ target: f.target })).data).toHaveLength(2);
