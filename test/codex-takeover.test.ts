...
function external(
  kind: WriterRuntime['kind'],
  terminateAndAdopt: boolean,
  pid = 42,
): ExternalSession {
  return ExternalSessionSchema.parse({
    ...
    capabilities: {
      ...
      reasons: [
        terminateAndAdopt
          ? 'dedicated CLI can be terminated after confirmation'
          : 'release at source',
      ],
    },
    lastActivityMs: 1,
...
describe('Codex external takeover', () => {
  ...
  test('signals a freshly revalidated dedicated CLI once, then adopts the same UUID', async () => {
    ...
    const result = await takeoverCodexExternalWithDependencies(
      machine,
      THREAD,
      42,
      'cc-adopted',
  ...
  test('fails closed before signaling shared, Desktop, App Server, self, and unknown writers', async () => {
    const kinds: WriterRuntime['kind'][] = [
      'shared',
      'desktop',
      'vscode',
      'app-server',
          ...
          snapshot: () => [
            {
              ...
              processGroup: 40,
              startTime: 'start-a',
              command: '/opt/tools/codex resume',
            },
          ],
          adopt: async () => {
            adopted = true;
    ...
    await expect(
      takeoverCodexExternalWithDependencies(
        ...
        dependencies({
          ...
          snapshot: () => [],
          adopt: async () => {
            throw new Error('resume admission rejected by respawned writer');
          },
        }),
      ),
