...
import { appendAttachmentChunk, beginAttachmentUpload, finalizeAttachmentUpload } from "../src/attachments/service.ts";
...
export function imageBytes(format: "png" | "jpeg" = "png", width = 4, height = 3, noise = false): Buffer<ArrayBuffer> {
...
export async function attachmentFixture() {
  ...
  temporary.push(root);
  const machine = makeMachine({ stateDir: root, rcPrefix: "host-a", projectsDir: join(root, "history") });
  const session = makeSession({ agent: "codex", runtime: "app-server", name: "image-agent", dir: root,
    registrationGeneration: crypto.randomUUID() });
  await writeSessionsUnlocked(machine, [session]);
  const principal = makeCli("host-b"), target = managedPeer(machine.rcPrefix, session);
...
export async function upload(f: AttachmentFixture, bytes = imageBytes(), mediaType: "image/png" | "image/jpeg" = "image/png") {
  const selector = { target: f.target, uploadId: crypto.randomUUID() };
  await beginAttachmentUpload(f.machine, f.principal, { ...selector, mediaType, totalBytes: bytes.length,
    digest: digest(bytes) }, f.signal);
  for (let offset = 0; offset < bytes.length; offset += ATTACHMENT_LIMITS.chunkBytes) {
    await appendAttachmentChunk(f.machine, f.principal, { ...selector, offset,
      data: bytes.subarray(offset, offset + ATTACHMENT_LIMITS.chunkBytes).toString("base64") }, f.signal);
  }
  return finalizeAttachmentUpload(f.machine, f.principal, selector, f.signal);
}

export async function pin<T>(f: AttachmentFixture, messageId: string, refs: AttachmentReference[], accept: () => Promise<T>) {
  return withSessionRegistryLock(f.machine, () => withPinnedAttachments(f.machine, f.principal, f.target,
    messageId, refs, accept, f.signal));
}
