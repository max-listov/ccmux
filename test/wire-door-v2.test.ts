...
test("bounded Unix door preserves additive replies and exact command outcomes", async () => {
  ...
  cleanup.push(() => { server.stop(true); rmSync(root, { recursive: true, force: true }); });
  expect(existsSync(socket)).toBe(true);
  expect(await runWire(makeMachine({ wire: { socket, peers: ["host-b"] } }), "host-b", ["ccmux", "list", "--json"]))
...
test("oversized and stalled local responses terminate and release their Unix transport", async () => {
  ...
  cleanup.push(() => { server.stop(true); rmSync(root, { recursive: true, force: true }); });
  await expect(callWireDoor({ socket, body: "{}", deadlineMs: 1_000, maxResponseBytes: 1_024 }))
    .rejects.toThrow();
  mode = "stalled";
  await expect(callWireDoor({ socket, body: "{}", deadlineMs: 50, maxResponseBytes: 1_024 }))
    .rejects.toThrow();
});
...
test("unavailable socket stays a transport failure and starts nothing", async () => {
  ...
  const result = await runWire(makeMachine({ wire: { socket, peers: ["host-b"] } }), "host-b", ["ccmux", "list"]);
...
test("version, malformed data and every refusal class remain distinct", async () => {
  ...
  reply = { v: 3, code: 0, stdout: "", stderr: "", failure: "none", refusal: "none", detail: "", truncated: false };
  expect(await invoke()).toMatchObject({ transportFailed: true, permanent: true, failureDetail: expect.stringContaining("API v3") });

  reply = { v: 2, stdout: "missing required code" };
  expect(await invoke()).toMatchObject({ transportFailed: true, failureDetail: "stitchwire agent returned an unreadable result" });
  ...
  for (const [failure, refusal, permanent] of [
    ...
    reply = { v: 2, code: 1, stdout: "", stderr: "", failure, refusal, retryAfterMs: refusal === "capacity" ? 250 : null,
      detail: "owner outcome", truncated: false, additive: true };
