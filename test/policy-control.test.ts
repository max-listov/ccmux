...
import { ControlCreateReceiptSchema, ControlCreateSchema } from '../src/control/schema/session.ts';
...
import { applicationPolicyEvidence, resolveApplicationPolicy } from '../src/policy/resolve.ts';
...
function fixture() {
  ...
  const source = {
    id: 'instructions-a',
    path: join(privateDir, 'instructions.md'),
    sha256: policySha256(body),
  };
...
test('Codex applies private thread instructions plus native skills and preserves native Plan settings', async () => {
  ...
  const rpc: CodexAppRpc = {
    ...
    request: async (method, params) => {
          ...
          data: [
            {
              cwd: f.dir,
              errors: [],
              skills: [{ name: f.skill.name, path: f.skill.path, enabled: true }],
            },
          ],
        };
      if (method === 'collaborationMode/list')
        return {
          data: [
            { name: 'Plan', mode: 'plan', model: 'native-default', reasoning_effort: 'medium' },
          ],
        };
  ...
  expect(policy?.collaborationMode?.settings).toEqual({
    model: 'model-a',
  ...
  expect(
    await startCodexAppTurn(rpc, f.s.uuid, crypto.randomUUID(), codexTextInput('hello'), policy),
  ).toBe('turn-a');
  const sent = z
    .object({ input: z.array(z.unknown()), collaborationMode: z.unknown() })
    .parse(calls.find((call) => call.method === 'turn/start')?.params);
  ...
  expect(JSON.stringify(calls.find((call) => call.method === 'turn/start')?.params)).not.toContain(
    'skillInputs',
  );
  ...
  writeFileSync(f.source.path, 'Changed source');
  await expect(
    prepareManagedCodexTurn(rpc, f.m, f.s, {
      thread: {
        id: f.s.uuid,
        name: null,
        source: null,
...
test('source failure prevents provider restart while status/stamp reads remain body-free', async () => {
  ...
  expect(
    staleReasons(
      { ...stamp, ts: Date.now() },
      { ...stamp, applicationPolicy: { ...f.policy.metadata, digest: 'f'.repeat(64) } },
    ),
  ).toContain('policy');
...
test('policy applied evidence requires native identity and cannot survive stale/disconnected status', () => {
  ...
  expect(
    nativePolicySkillsAcknowledged(f.policy, f.s.uuid, {
      threadId: f.s.uuid,
      item: {
        type: 'userMessage',
        content: [{ type: 'skill', name: f.skill.name, path: f.skill.path }],
      },
    }),
  ).toBe(true);
  expect(
  ...
  expect(
    nativePolicySkillsAcknowledged(f.policy, f.s.uuid, {
      threadId: f.s.uuid,
      item: { type: 'userMessage', content: [{ type: 'text', text: 'skill-a' }] },
    }),
  ).toBe(false);
  projection.policyEvidence(applicationPolicyEvidence(f.policy, 'applied'));
  expect(
    projectApplicationPolicy(f.policy.metadata, 'live', projection.snapshot().applicationPolicy)
      .state,
  ).toBe('applied');
  expect(
    projectApplicationPolicy(f.policy.metadata, 'stale', projection.snapshot().applicationPolicy)
      .state,
  ).toBe('unavailable');
...
test('OpenCode native policy selection survives exact receipt reconciliation without another prompt', async () => {
  ...
  const server = Bun.serve({
    ...
    fetch: async (request) => {
      ...
      if (path.endsWith(`/message/${nativeId}`))
        return Response.json({
          info: {
            id: nativeId,
  ...
  try {
    ...
    await expect(
      applyOpenCodeInput(f.m, s, client, invalidReceipt, new AbortController().signal),
    ).rejects.toThrow('Application policy is unavailable');
    expect(readRuntimeInput(f.m, s)?.phase).toBe('accepted');
...
test('non-native policy and mismatched OpenCode source refuse before reservation or native create', async () => {
  ...
  await expect(
    createManagedSession(f.m, {
      ...
      router: false,
      runtime: 'tui',
      applicationPolicy: f.policy.metadata,
    }),
  ).rejects.toThrow('Application policy is unavailable');
  ...
  try {
    await expect(
      admitOpenCode(
        ...
        s,
        {
          client: createOpencodeClient({ baseUrl: server.url.href, throwOnError: true }),
          version: '1.18.20',
        },
        true,
...
test('native canonical agent catalog accepts null hidden while excluding explicitly hidden agents', async () => {
  ...
  const s = makeSession({
    ...
    runtime: 'native',
    registrationGeneration: crypto.randomUUID(),
    nativeSession: { runtime: 'opencode', id: 'ses_catalog', version: '1.18.20' },
  });
  mkdirSync(managedRuntimeRoot(f.m, s), { recursive: true, mode: 0o700 });
  const server = Bun.serve({
    hostname: '127.0.0.1',
    ...
    fetch(request) {
      ...
      if (new URL(request.url).pathname === '/config/providers')
  ...
  try {
    ...
    const result = z
      .object({ agents: z.array(z.string()) })
      .parse(JSON.parse(readFileSync(join(managedRuntimeRoot(f.m, s), 'models.json'), 'utf8')));
