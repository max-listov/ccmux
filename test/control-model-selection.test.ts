...
test("one profile serves two model selections; retries and restart preserve one writer and selection", async () => {
  ...
  const machine = makeMachine({ stateDir: join(root, "state"), codexBin: "/bin/codex", launchRecipes: {
    generic: { revision: "1", flags: [], environment: [], capabilities: ["input-requests"], collaborationMode: "plan" },
  } });
  let spawns = 0;
  const create = async (_machine: typeof machine, input: CreateManagedInput) => {
    spawns++;
    const session = makeSession({ ...input, uuid: crypto.randomUUID(), dir: input.dir,
      registrationGeneration: input.registrationGeneration, agent: "codex", runtime: "app-server" });
    await writeSessionsUnlocked(machine, [...loadSessions(machine), session]);
    return session;
  };
  let validations = 0;
  const validate = async () => { validations++; };
  const input = { requestId: crypto.randomUUID(), name: "model-a", workspace: root, flags: [],
    launchRecipe: { id: "generic", revision: "1" }, modelSelection: { provider: "openai", model: "model-a" } };
  try {
    const first = await createControlSession(machine, input, AbortSignal.timeout(3_000), create, validate);
    const retry = await createControlSession({ ...machine }, input, AbortSignal.timeout(3_000), create, validate);
    expect(retry.target).toEqual(first.target);
    expect(retry.modelSelection).toEqual(input.modelSelection);
    expect(retry.duplicate).toBe(true);
    expect(validations).toBe(1);
    await expect(createControlSession(machine, { ...input, modelSelection: { provider: "openai", model: "model-b" } },
      AbortSignal.timeout(3_000), create, validate)).rejects.toMatchObject({ code: "IDEMPOTENCY_CONFLICT" });
    const second = await createControlSession(machine, { ...input, requestId: crypto.randomUUID(), name: "model-b",
      modelSelection: { provider: "openai", model: "model-b" } }, AbortSignal.timeout(3_000), create, validate);
    ...
    for (const session of loadSessions(machine)) {
      ...
      expect(ownedCodexThreadParams(session, machine)).toMatchObject({ model: session.modelSelection?.model, modelProvider: "openai" });
      expect(ownedCodexArgv(session, machine)).toContain(`model="${session.modelSelection?.model}"`);
    }
    const count = loadSessions(machine).length;
    await expect(createControlSession(machine, { ...input, requestId: crypto.randomUUID() }, AbortSignal.timeout(3_000),
      create, async () => { throw new Error("unsupported model"); })).rejects.toThrow("unsupported model");
    expect(loadSessions(machine)).toHaveLength(count);
    expect(spawns).toBe(2);
    await expect(createControlSession(machine, { ...input, flags: ["-m", "other"] },
      AbortSignal.timeout(3_000), create, validate)).rejects.toMatchObject({ code: "INVALID_INPUT" });
    expect(ControlCreateSchema.safeParse({ ...input, modelSelection: { ...input.modelSelection, endpoint: "https://example.invalid" } }).success).toBe(false);
...
test("model validation cannot create a provider or infer an unsupported selection", async () => {
  ...
  await expect(validateModelSelection(machine, { flags: [] }, "/work", { provider: "openai", model: "model-a" },
