...
test("an unknown recipient points at the likely cause — it was meant for another machine", () => {
  const r = holdReason(msg(), { recipient: undefined, chatEnabled: true, running: false, nowMs: 0 });
...
test("chat-off and stopped are named with the exact command that fixes them", () => {
  expect(holdReason(msg(), { recipient: makeSession({ name: "api" }), chatEnabled: false, running: true, nowMs: 0 }).text).toContain("ccmux chat on api");
  expect(holdReason(msg(), { recipient: enabled, chatEnabled: true, running: false, nowMs: 0 }).text).toContain("ccmux start api");
});

test("a scheduled message reports the remaining wait, not a fault", () => {
  const now = Date.parse("2026-08-05T00:00:00.000Z");
  const r = holdReason(msg({ notBefore: "2026-08-05T00:00:30.000Z" }), { recipient: enabled, chatEnabled: true, running: true, nowMs: now });
...
test("the daemon's live reason wins over the generic ones", () => {
  ...
  const r = holdReason(msg({ defer: true }), { recipient: enabled, chatEnabled: true, running: true, nowMs: 0, daemonHold: hold });
...
test("a live reason is only claimed for the message it was actually recorded about", () => {
  ...
  const r = holdReason(msg({ id: "m1", defer: true }), { recipient: enabled, chatEnabled: true, running: true, nowMs: 0, daemonHold: hold });
...
test("a recipient whose agent cannot receive chat is told so — never 'queued'", () => {
  ...
  // that never resolves.
  const r = holdReason(msg(), { recipient: enabled, chatEnabled: true, running: true, nowMs: 0, chatDeliverable: false });
...
test("mail to the human is not a fault — there is no pane to deliver into", () => {
  const r = holdReason(msg({ to: makeOwner() }), { recipient: undefined, chatEnabled: true, running: false, nowMs: 0, isOwner: true });
...
test("defer with no live record reads as waiting for the turn to end; plain mail as simply queued", () => {
  expect(holdReason(msg({ defer: true }), { recipient: enabled, chatEnabled: true, running: true, nowMs: 0 }).kind).toBe("awaiting-turn-end");
  expect(holdReason(msg(), { recipient: enabled, chatEnabled: true, running: true, nowMs: 0 }).kind).toBe("pending");
});
