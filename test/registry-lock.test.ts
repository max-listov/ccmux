...
} from '../src/config/registryLock.ts';
...
test('a LIVE owner is never taken, however old the lock is', () => {
  ...
  expect(lockVerdict(hourOld, 0, { pid: 1234, token: TOKEN }, alive)).toEqual({
    kind: 'held',
    pid: 1234,
  });
});
...
test('a directory that NEVER got an owner is a wreck, and is cleared', () => {
  ...
  expect(lockVerdict(60_000, 0, null, dead)).toMatchObject({
    kind: 'reap',
    why: 'no owner was ever recorded',
  });
});
