...
const read = (input: unknown) => readControlDirectory(ControlDirectoryReadSchema.parse(input), AbortSignal.timeout(5000));
test("directory listing paginates names, omits hidden entries and reports but refuses symlinks", async () => {
  ...
  expect(first.entries.map(e => [e.name, e.kind])).toEqual([["a", "dir"], ["b", "file"]]);
  ...
  expect(JSON.stringify(await read({ path, includeHidden: true }))).not.toContain("secret-like-fixture");
  await expect(read({ path: join(path, "c") })).rejects.toMatchObject({ code: "SYMLINK_REFUSED" });
  await expect(read({ path: join(path, "c", "child") })).rejects.toMatchObject({ code: "SYMLINK_REFUSED" });
});
test("directory mutation and different selectors invalidate cursors rather than skip entries", async () => {
  ...
  await expect(read({ path, cursor: first.nextCursor, includeHidden: true })).rejects.toMatchObject({ code: "STALE_CURSOR" });
  writeFileSync(join(path, "aa"), "");
  await expect(read({ path, cursor: first.nextCursor })).rejects.toMatchObject({ code: "STALE_CURSOR" });
...
test("directory errors and bounds stay explicit and contain no file contents", async () => {
  ...
  await expect(readControlDirectory(ControlDirectoryReadSchema.parse({ path }), signal)).rejects.toBeDefined();
});
