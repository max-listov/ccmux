...
const bodies = (frames: LogFrame[]): string[] => frames.flatMap((f) => (f.kind === "row" ? [f.row.body] : []));
...
test("a malformed cursor is refused with what was expected", () => {
  expect(parseCursor("nonsense")).toMatchObject({ error: expect.stringContaining("<generation>.<ledger>.<outbox>") });
...
test("an oversized record is REPLACED with a named refusal, never cut", () => {
  ...
  const row = { machine: "host-a", ts: "2026-08-26T10:00:00.000Z", kind: "chat" as const, from: "a", to: "b", task: null, body: "x".repeat(MAX_FRAME_BYTES * 2), note: "" };
...
test("the cap counts BYTES, so a long Unicode body is bounded by what the transport carries", () => {
  ...
  const row = { machine: "host-a", ts: "t", kind: "chat" as const, from: "a", to: "b", task: null, body: emoji, note: "" };
  const bounded = boundFrame({ kind: "row", cursor: "2.1.0", row });
  expect(Buffer.byteLength(JSON.stringify(bounded))).toBeLessThanOrEqual(MAX_FRAME_BYTES);
  if (bounded.kind !== "row") throw new Error("unreachable");
  expect(bounded.row.body).toContain("body omitted");
...
test("a long Unicode body that FITS is passed through untouched", () => {
  ...
  const row = { machine: "host-a", ts: "t", kind: "chat" as const, from: "a", to: "b", task: null, body, note: "" };
  const bounded = boundFrame({ kind: "row", cursor: "2.1.0", row });
  if (bounded.kind !== "row") throw new Error("unreachable");
...
test("an oversized record still advances the stream — it keeps its position", () => {
  ...
  for (const f of out.frames) expect(Buffer.byteLength(JSON.stringify(f))).toBeLessThanOrEqual(MAX_FRAME_BYTES);
});
...
test("one strict schema covers both frame kinds, and rejects an unknown key", () => {
  ...
  expect(LogFrameSchema.safeParse({ kind: "machine", cursor: "2.0.0", machine: { machine: "host-a", ok: true, error: null }, extra: 1 }).success).toBe(false);
...
test("a record this build cannot read becomes a row, not a gap", () => {
  ...
  const path = chatLedgerPath(machine());
  mkdirSync(dirname(path), { recursive: true });
  appendFileSync(path, `${JSON.stringify({ v: 99, id: "x", ts: "t", from: "a", to: "b", body: "y" })}\n`);
...
test("a CUT answer is named as cut, not blamed on an old build", () => {
  ...
  // machine entirely.
  const cut = unreadableReason("{\"rows\":[{\"body\":\"aaa", "[wire] output truncated at the stream cap\n");
