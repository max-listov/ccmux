...
test("a local fleet row carries the directory exactly as `list --json` reports it", () => {
  ...
  const item = ListJsonSchema.shape.sessions.element.parse({
    name: "agent-a", agent: "claude", dir, uuid: "11111111-1111-4111-8111-111111111111", rc: "rc",
    running: true, archived: false, state: "idle", lifecycleError: null, model: null,
    context: { text: null, usedTokens: null, limitTokens: null, percent: null },
    uptime: { text: "1m", seconds: 60 }, createdAt: null, lastMessage: null,
  });
...
test("the field rides through the fleet view untouched", () => {
  const machine: FleetMachine = {
    machine: "host-a", alias: null, ok: true, error: null, version: "0.37.0", release: null, behind: null,
