...
import {
  ...
  resumingUuids,
  type Writer,
} from '../src/agent/claude/writers.ts';
...
const PROCS: PsProc[] = [
  {
    pid: 13456,
    ppid: 95593,
    command: `/Applications/Claude.app/Contents/Helpers/disclaimer /Users/u/Library/Application Support/Claude/claude-code/2.1.165/claude.app/Contents/MacOS/claude --output-format stream-json --resume ${UUID}`,
  },
  {
    pid: 13457,
    ppid: 13456,
    command: `/Users/u/Library/Application Support/Claude/claude-code/2.1.165/claude.app/Contents/MacOS/claude --output-format stream-json --resume ${UUID}`,
  },
  { pid: 89360, ppid: 89342, command: `/Users/u/.local/bin/claude --resume ${UUID}` },
  {
    pid: 1683,
    ppid: 1626,
    command: '/Users/u/.local/bin/claude --resume 99999999-9999-4999-8999-999999999999',
  },
...
test('classifyWriters: matches --session-id too (first boot, no history yet)', () => {
  const procs: PsProc[] = [
    { pid: 7, ppid: 1, command: `/Users/u/.local/bin/claude --session-id ${UUID}` },
  ];
...
test('externalResumingUuids: pane-internal processes never surface as external', () => {
  ...
  const procs: PsProc[] = [
    { pid: 1300, ppid: 1, command: 'tmux pane shell' },
    {
      pid: 1301,
      ppid: 1300,
      command: '/Users/u/.bun/bin/bun /Users/u/.cache/ccmux/app/ccmux.js _run alpha',
    },
    ...
    { pid: 99421, ppid: 1342, command: '/Users/u/.local/bin/claude daemon run --origin transient' },
    {
      pid: 99437,
      ppid: 99421,
      command: `/Users/u/.local/bin/claude --bg-pty-host /tmp/cc/pty.sock -- x --session-id ${FORK}`,
    },
