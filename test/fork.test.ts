...
describe('detectFork', () => {
  test('follows a titled fork whose messages are newer than the pinned file', () => {
    ...
    writeJsonl(projDir, FORK, [
      title(RC),
      msg('2026-07-11T01:23:45.000Z'),
      msg('2026-07-14T03:50:50.000Z'),
    ]);
...
describe('lastMessageMs', () => {
  test('newest timestamp from the tail; null for missing/untimestamped files', () => {
    const { projDir } = setup();
    writeJsonl(projDir, PINNED, [
      title(RC),
      msg('2026-07-01T00:00:00.000Z'),
      msg('2026-07-02T00:00:00.000Z'),
    ]);
    expect(lastMessageMs(join(projDir, `${PINNED}.jsonl`))).toBe(
