...
function project(value: string, chunk = 1) {
  const elider = new InlineImageElider(), bytes = Buffer.from(value), output: number[] = [];
  for (let offset = 0; offset < bytes.length; offset += chunk)
    for (const byte of bytes.subarray(offset, offset + chunk)) elider.feed(byte, part => output.push(part));
...
test("native image elision preserves JSON byte boundaries and unrelated strings", () => {
  const original = { info: { id: "msg_example", role: "user" }, parts: [
    { type: "file", mime: "image/png", url: "data:image/png;base64,Aa0+/==", filename: "fixture.png" },
  ...
  for (const chunk of [1, 2, 7, 31, 1024]) expect(JSON.parse(project(JSON.stringify(original), chunk))).toEqual(expected);
...
test("large native image echo is bounded before SDK JSON without widening non-image bytes", async () => {
  const server = Bun.serve({ hostname: "127.0.0.1", port: 0, fetch(request) {
    ...
    return Response.json({ type: "file", [key]: "data:image/png;base64," + "A".repeat(3 * 1024 * 1024), mime: "image/png" });
  } });
  try {
    ...
    await expect((await boundedOpenCodeFetch(new URL("/text", server.url))).text()).rejects.toThrow("bounded frame");
