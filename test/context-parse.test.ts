...
test("parseContext handles a bare used-only label (no limit / percent)", () => {
  expect(parseContext("40k")).toEqual({ text: "40k", usedTokens: 40_000, limitTokens: null, percent: null });
});

test("parseContext on null → all-null (unknown is never invented as 0)", () => {
  expect(parseContext(null)).toEqual({ text: null, usedTokens: null, limitTokens: null, percent: null });
});
