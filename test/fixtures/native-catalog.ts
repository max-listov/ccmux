...
export function nativeCatalogFixture(m: MachineConfig, session: Session) {
  ...
  const RequestSchema = z.object({ id: z.union([z.string(), z.number()]).optional(), method: z.string() });
  return Bun.serve({ unix: socket, fetch(request, server) {
    ...
    const result = request.method === "initialize" ? { userAgent: "codex/0.151.0" }
      : request.method === "config/read" ? { config: { model_provider: "openai" } }
      : { data: [{ id: model, model, displayName: model, description: "fixture", hidden: false,
        isDefault: true, inputModalities: ["text", "image"], serviceTiers: [],
