...
const server = Bun.serve({ unix: address.slice(7),
  ...
  websocket: { message(ws, raw) {
    ...
    else if (request.method === "config/read") ws.send(JSON.stringify({ id: request.id,
      result: { config: { model_provider: "openai" } } }));
    else if (request.method === "model/list" && request.params.cursor !== "hang") ws.send(JSON.stringify({
      id: request.id, result: { data: [{ id: "preset-a", model: "model-a", displayName: "A", description: "",
        hidden: false, isDefault: true, inputModalities: ["text"], serviceTiers: [] }], nextCursor: null },
    }));
    else if (request.method !== "model/list") ws.send(JSON.stringify({ id: request.id,
