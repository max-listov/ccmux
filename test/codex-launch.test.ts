...
function rollout(root: string, ymd: string, id: string, cwd: string, originator: string): void {
  ...
  writeFileSync(path, `${JSON.stringify({ type: "session_meta", payload: { id, session_id: id, cwd, originator } })}\n`);
}
...
function forkRollout(root: string, id: string, originator: string, sourceThreadId: string): void {
  ...
  const prompt = JSON.stringify({
    type: "response_item",
    payload: { type: "message", role: "user", content: [{ type: "input_text", text: `ccmux launch correlation: ${originator}` }] },
  });
  const largePrelude = JSON.stringify({
    type: "response_item",
    payload: { type: "message", role: "developer", content: [{ type: "input_text", text: "x".repeat(600 * 1024) }] },
  });
  const inheritedRecords = Array.from({ length: 80 }, (_item, index) => JSON.stringify({
    type: "response_item",
    payload: { type: "message", role: "assistant", content: [{ type: "output_text", text: `inherited-${index}` }] },
  }));
...
test("correlation never uses cwd and exposes ambiguity instead of choosing newest", () => {
  ...
  const m = codexMachine({ codexSessionsDir: root });
  expect(rolloutIdsForMarker(m, marker)).toEqual(expect.arrayContaining([
    CODEX_ID,
...
test("fork correlation requires both exact launch marker and provider-recorded parent", () => {
  ...
  forkRollout(root, "bbbbbbbb-1111-4111-8111-111111111111", marker, "cccccccc-1111-4111-8111-111111111111");
  const m = codexMachine({ codexSessionsDir: root });
