...
import { detectPrompt, policyAnswers, promptAnswer, atInteractiveMenu } from "../src/agent/claude/prompts.ts";
...
test("the escalated level accepts declared permissions, and off answers neither", () => {
  expect(policyAnswers("declared-permissions", makeMachine({ trustPrompt: "declared" }))).toBe(true);
...
test("the keystroke is read from the pane, so a reordered menu still gets the right option", () => {
  ...
  const reordered = TRUST.replace("❯ 1. Yes, I trust this folder\n   2. No, exit", "❯ 1. No, exit\n   2. Yes, I trust this folder");
...
test("a session at a menu reads as needing an answer, not as idle", () => {
  ...
  const waiting = deriveStatus({ running: true, isWorking: false, lastMessage: null, atPrompt: "trust this folder" });
