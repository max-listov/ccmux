...
test("captures permission_mode / effort / transcript_path when present", () => {
  const s = parseLifecycle(L({ hook_event_name: "Stop", permission_mode: "auto", effort: "high", transcript_path: "/p/x.jsonl" }), 42);
  expect(s).toEqual({ state: "idle", ts: 42, event: "Stop", permissionMode: "auto", effort: "high", transcriptPath: "/p/x.jsonl" });
});
