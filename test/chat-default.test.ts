...
test('an override wins in BOTH directions — silence on a chatty box, and the reverse', () => {
  ...
  expect(
    chatEnabledFor(makeSession({ chatEnabled: false }), makeMachine({ chatEnabled: true })),
  ).toBe(false);
  expect(
    chatEnabledFor(makeSession({ chatEnabled: true }), makeMachine({ chatEnabled: false })),
  ).toBe(true);
});
...
test('changing the MACHINE default marks sessions for restart', () => {
  ...
  expect(
    staleReasons(before, computeStamp(s, makeMachine({ chatEnabled: true }), 'ccmux')),
  ).toContain('chat');
});
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) walk(p);
        ...
        for (const line of readFileSync(p, 'utf8').split('\n')) {
          ...
          if (/\b(s|me|session|recipient)\.chatEnabled\b/.test(line))
