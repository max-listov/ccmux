...
test("host catalog needs no managed row and metadata child is reaped", async () => {
  const f = fixture();
  try {
    const page = await readControlModels(f.machine, ControlModelsReadSchema.parse({}), AbortSignal.timeout(3_000));
...
test("cancelled host read reaps its metadata process without creating a writer", async () => {
  const f = fixture();
  try {
    await expect(readControlModels(f.machine, ControlModelsReadSchema.parse({ cursor: "hang" }),
      AbortSignal.timeout(400))).rejects.toMatchObject({ code: "UNAVAILABLE" });
    expect(loadSessions(f.machine)).toEqual([]);
    reaped(f.root);
  } finally { rmSync(f.root, { recursive: true, force: true }); }
});
...
test("unknown profile and invalid selectors fail before metadata spawn", async () => {
  const f = fixture();
  try {
    await expect(readControlModels(f.machine, ControlModelsReadSchema.parse({ launchRecipe: { id: "missing", revision: "1" } }),
      AbortSignal.timeout(1_000))).rejects.toMatchObject({ code: "LAUNCH_RECIPE_UNAVAILABLE" });
    expect(existsSync(join(f.root, "fixture.pid"))).toBe(false);
    for (const extra of [{ executable: "/bin/sh" }, { endpoint: "https://example.invalid" }, { path: "/work" },
      { credential: "fixture" }]) expect(ControlModelsReadSchema.safeParse(extra).success).toBe(false);
  } finally { rmSync(f.root, { recursive: true, force: true }); }
});
