...
test('native working/approval/input/unknown and unavailable snapshots never open a send connection', async () => {
  for (const state of ['working', 'waiting-approval', 'waiting-input', 'unknown'] satisfies Array<
    NonNullable<OwnedCodexRead['snapshot']>['state']
  >) {
    const f = fixture();
    f.setRead({
      protocol: 1,
...
test('conditional acknowledgement follows terminal native turn, not acceptance or idle before completion', async () => {
  ...
  await f.run();
  expect(f.calls.filter((call) => call === 'ack')).toHaveLength(1);
  expect(f.cursors.pickups[f.key]).toBeUndefined();
});

test('future delivery, rate holds and changed registration cannot submit', async () => {
  const future = fixture();
  future.msg.notBefore = new Date(Date.now() + 60_000).toISOString();
  expect(await future.run()).toBe(0);
  expect(future.calls).toEqual([]);
  const rate = fixture();
  expect(await rate.run(true)).toBe(0);
