...
import { promotedPending, recoverPromotionsUnlocked } from './promotion.ts';
import { findSession, loadSessions, writeSessionsUnlocked } from './registry.ts';
...
export async function reservePendingSession(
  m: MachineConfig,
  ...
  await withSessionRegistryLock(m, async () => {
    ...
    if (
      findSession(sessions, pending.session.name) ||
      current.some((item) => item.session.name === pending.session.name)
    ) {
...
export async function markPendingBlocked(
  m: MachineConfig,
  generation: string,
  error: string,
): Promise<void> {
  await withSessionRegistryLock(m, async () => {
    ...
    await writePendingRows(
      m,
      current.map((item) =>
        item.generation === generation ? { ...item, status: 'blocked', error } : item,
      ),
    );
  });
}
...
export async function removePendingSession(m: MachineConfig, generation: string): Promise<void> {
  await withSessionRegistryLock(m, async () => {
    ...
    await writePendingRows(
      m,
      current.filter((item) => item.generation !== generation),
    );
  });
}
...
export async function promotePendingSession(
  ...
  return withSessionRegistryLock(m, async () => {
    ...
    const target = pending.find(
      (item) => item.generation === generation && item.status === 'pending',
    );
    if (!target)
      throw new Error('pending launch was removed, replaced, or blocked before promotion');
    const sessions = loadSessions(m);
    if (findSession(sessions, target.session.name))
      throw new Error('session name was claimed before promotion');
    if (sessions.some((item) => item.uuid === uuid))
      throw new Error('provider thread id is already claimed');
    if (
      nativeSession &&
      sessions.some(
        (item) =>
          item.nativeSession?.runtime === nativeSession.runtime &&
          item.nativeSession.id === nativeSession.id,
      )
    )
    ...
    await writePendingRows(
      m,
      pending.map((item) =>
        item.generation === generation ? promotedPending({ ...item, session: fields }, uuid) : item,
      ),
    );
    await writeSessionsUnlocked(m, [...sessions, ready]);
    await writePendingRows(
      m,
      pending.filter((item) => item.generation !== generation),
    );
    return ready;
  });
}
