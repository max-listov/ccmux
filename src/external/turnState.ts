    ...
    if (value.type === 'notLoaded') result.reason = 'not-loaded';
      ...
      result = {
        ...
        state:
          value.type === 'idle'
            ? 'idle'
            : value.activeFlags.includes('waitingOnApproval')
              ? 'waiting-approval'
              : value.activeFlags.includes('waitingOnUserInput')
                ? 'waiting-input'
                : 'working',
      };
    }
...
export async function observeExternalTurns(
  ...
  ) => Promise<CodexAppRpc> = connectCodexAppServer,
): Promise<ExternalSession[]> {
  const wanted = new Set(
    sessions.filter((row) => row.provider === 'codex').map((row) => row.threadId),
  );
  if (wanted.size === 0)
  ...
  try {
    ...
    for (let pageIndex = 0; pageIndex < MAX_PAGES; pageIndex++) {
      ...
      if (page.nextCursor === null) {
        missing = unknownTurnState('codex-app-server', 'not-reported');
        break;
      }
