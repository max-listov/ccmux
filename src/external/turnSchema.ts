...
export const ExternalTurnStateSchema = z.object({
  ...
  reason: z.enum(["native-status", "not-loaded", "system-error", "unsupported-status", "unsupported-provider", "unsupported-runtime", "not-observed", "not-reported", "read-limit", "connection-unavailable", "deadline"]),
}).strict().superRefine((value, ctx) => {
  if ((value.state !== "unknown") !== (value.evidence === "observed")) {
    ctx.addIssue({ code: "custom", message: "only observed native evidence may claim a turn state" });
  }
  if (value.evidence === "observed" && (value.observedAt === null || value.expiresAt === null || value.source !== "codex-app-server")) {
    ctx.addIssue({ code: "custom", message: "observed turn state requires native provenance and freshness" });
  }
});
