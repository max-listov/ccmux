...
export function codexWriterRuntime(
  ...
  if (inspection.holders.length !== 1 || rows === null) {
    return {
      ...
      processGroup: null,
      reason:
        inspection.holders.length > 1
          ? 'multiple processes hold the exact writer lock'
  ...
  if (!row) {
    return {
      ...
      startTime: null,
      processGroup: null,
      reason: 'lock holder left the process snapshot',
    };
  }
  ...
  const tmuxIndex = ancestors.findIndex(
    (item) => basename(item.command.trim().split(/\s+/, 1)[0] ?? '') === 'tmux',
  );
  ...
  if (ownAncestors.has(holder.pid)) {
    ...
    commands.some((command) => /(?:chatgpt|codex)\.app\/contents\/macos\//.test(command))
  ) {
    kind = 'desktop';
    ...
    commands.some(
      (command) =>
        ...
        command.includes('extensionhost'),
    )
  ) {
    kind = 'vscode';
...
export function discoverCodex(m: MachineConfig): ExternalSession[] {
  ...
  const ids = new Set(
    [...stored.map((item) => item.meta.payload.id), ...lockOnlyIds(m)].filter(
      (id) => !managed.has(id),
    ),
  );
  ...
  for (const threadId of ids) {
    ...
    const writerRuntime = codexWriterRuntime(
      inspection,
      rows,
      holderThreadCounts,
      process.pid,
