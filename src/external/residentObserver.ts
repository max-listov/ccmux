...
import type { ExternalStatusPublisher } from './residentPublisher.ts';
import {
  EXTERNAL_MAX_ROWS,
...
export class ExternalStatusObserver {
  ...
  constructor(
    private initial: MachineConfig,
    readonly publisher: ExternalStatusPublisher,
  ...
  refresh(machine: MachineConfig = this.initial, signal?: AbortSignal): Promise<void> {
    ...
    this.running = this.reconcile(machine, signal).finally(() => {
  ...
  private async reconcile(machine: MachineConfig, signal?: AbortSignal): Promise<void> {
    ...
    if (this.connection && this.connection.root !== machine.codexHome)
      this.disconnect('config-changed');
    ...
    const deadline = setTimeout(
      () => this.disconnect('deadline', connection),
      TURN_OBSERVATION_DEADLINE_MS,
    );
    try {
      if (!connection.rpc) {
      ...
      if (!supportsNativeStatus(connection.rpc.userAgent)) {
        this.disconnect('unsupported-runtime', connection);
        return;
      }
      ...
      for (const row of inventory.rows) {
        ...
        rows.set(row.id, {
          identity: { provider: 'codex', machine: machine.rcPrefix, threadId: row.id },
          name: row.name ?? null,
          dir: row.cwd ?? null,
          updatedAt:
      ...
      for (const [id, event] of this.events) {
        if (event.revision > revision && !rows.has(id) && !this.managed.has(id))
          rows.set(id, this.eventRow(id, event));
      }
  ...
  private eventRow(id: string, event: Observation): ExternalStatusRow {
    return {
      identity: { provider: 'codex', machine: this.initial.rcPrefix, threadId: id },
      name: null,
      dir: null,
      updatedAt: null,
  ...
  private disconnect(
    reason: NonNullable<ExternalStatusSnapshot['reason']>,
  ...
  metrics() {
    return { ...this.stats, connected: this.connection?.rpc !== undefined };
  }
  async close(): Promise<void> {
    if (this.stopped) return;
    this.stopped = true;
    this.disconnect('daemon-stopped');
    await this.running;
