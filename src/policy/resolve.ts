...
export function resolveApplicationPolicy(
  host: PolicyHost,
  runtime: string,
  reference: ApplicationPolicyReference,
): MaterializedPolicy {
  ...
  if (definition.revision !== ref.data.revision)
    policyUnavailable(ref.data.id, 'revision-mismatch');
  ...
  if (definition.runtime === 'codex') {
    ...
    const metadata = ApplicationPolicyMetadataSchema.parse({
      ...ref.data,
      runtime,
      digest,
      sources: [
      ...
      capabilities: [
        ...(instructionSources.length ? ['developer-instructions'] : []),
  ...
  return MaterializedPolicySchema.parse({
    runtime,
    metadata,
    agent: { name: definition.agent.name, source, prompt },
    denyTools: [...new Set(definition.denyTools)].sort(),
  });
}

/** Re-run at every start/resume and turn admission. Accepted digests never follow edited sources. */
export function verifyApplicationPolicy(
  host: PolicyHost,
  runtime: string,
  accepted: ApplicationPolicyMetadata,
): MaterializedPolicy {
  ...
  const resolved = resolveApplicationPolicy(host, runtime, {
    id: parsed.data.id,
