...
function validatePath(policyId: string, roots: readonly string[], path: string): void {
  ...
  while (true) {
    ...
    if (current !== path && !stat.isDirectory()) policyUnavailable(policyId, "invalid-source-parent");
...
export function readPolicySource(policyId: string, roots: readonly string[], source: PolicySource): string {
  ...
  try {
    ...
    if (length !== before.size || length > MAX_POLICY_SOURCE_BYTES || before.ino !== visible.ino ||
      before.dev !== visible.dev || before.size !== after.size || before.mtimeMs !== after.mtimeMs ||
      before.ctimeMs !== after.ctimeMs || realpathSync(source.path) !== source.path)
      policyUnavailable(policyId, "source-changed-during-read");
    const bodyBytes = bytes.subarray(0, length);
    if (policySha256(bodyBytes) !== source.sha256) policyUnavailable(policyId, "source-digest-mismatch");
    ...
    if (error instanceof Error && error.message === "Application policy is unavailable") throw error;
