import { z } from "zod";
import { PolicyIdSchema as IdSchema, PolicyDigestSchema as DigestSchema, ApplicationPolicyMetadataSchema } from "./reference.ts";
export { ApplicationPolicyReferenceSchema, ApplicationPolicyMetadataSchema, ApplicationPolicyEvidenceSchema } from "./reference.ts";
export type { ApplicationPolicyReference, ApplicationPolicyMetadata, ApplicationPolicyEvidence } from "./reference.ts";
const PathSchema = z.string().min(1).max(4096).refine((value) =>
  value.startsWith("/") && !value.includes("\0") && (value === "/" ||
    value.slice(1).split("/").every((part) => part !== "" && part !== "." && part !== "..")), "canonical absolute path required");

/** Host configuration only. Public requests carry a reference, never these paths or source text. */
export const PolicySourceSchema = z.object({ id: IdSchema, path: PathSchema, sha256: DigestSchema }).strict();
...
export const CodexAgentPolicySchema = z.object({
  ...hostFields,
...
}).strict().refine((value) => value.instructionSources.length + value.skills.length > 0,
  "policy requires a canonical source");
export const OpenCodeAgentPolicySchema = z.object({
  ...hostFields,
...
export const HostAgentPolicySchema = z.discriminatedUnion("runtime", [CodexAgentPolicySchema, OpenCodeAgentPolicySchema]);
...
export const MaterializedPolicySchema = z.discriminatedUnion("runtime", [
  z.object({ runtime: z.literal("codex"), metadata: ApplicationPolicyMetadataSchema,
    instructionSources: z.array(PrivateSourceSchema),
    skills: z.array(PrivateSourceSchema.extend({ name: IdSchema }).strict()),
  }).strict(),
  z.object({ runtime: z.literal("opencode"), metadata: ApplicationPolicyMetadataSchema,
