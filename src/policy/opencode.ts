...
const NativeRuleSchema = z.object({ permission: z.string().max(256), pattern: z.string().max(4096), action: z.enum(["allow", "ask", "deny"]) });
const NativeAgentsSchema = z.array(z.object({
  name: z.string().max(128), mode: z.enum(["primary", "subagent", "all"]), hidden: z.boolean().nullish(),
  prompt: z.string().max(256 * 1024).optional(), permission: z.array(NativeRuleSchema).max(4096),
})).max(1024);
...
function permissionMatches(pattern: string, tool: string): boolean {
  ...
  while (cursor < tool.length) {
    if (pattern[position] === "?" || pattern[position] === tool[cursor]) { position++; cursor++; }
    else if (pattern[position] === "*") { star = position++; restart = cursor; }
    else if (star >= 0) { position = star + 1; cursor = ++restart; }
    else return false;
  }
...
export function selectOpenCodePolicyAgent(policy: MaterializedPolicy, nativeAgents: unknown): string {
  ...
  if (agents.length !== 1 || agent === undefined || agent.hidden === true || agent.mode === "subagent")
