...
const SkillInputSchema = z.object({ type: z.literal("skill"), name: z.string(), path: z.string() }).strict();
export type PolicySkillInput = z.infer<typeof SkillInputSchema>;
const InventorySchema = z.object({ data: z.array(z.object({
  cwd: z.string().max(4096), errors: z.array(z.unknown()).max(1024),
  skills: z.array(z.object({ name: z.string().max(128), path: z.string().max(4096), enabled: z.boolean() })).max(4096),
})).max(128) });

/** Private thread/start + thread/resume parameter. Never add this materialization to process argv. */
export function composePolicyDeveloperInstructions(policy: MaterializedPolicy, supervisorInstructions: string): string {
...
export function policySkillInputs(policy: MaterializedPolicy, workspace: string, nativeInventory: unknown): PolicySkillInput[] {
  if (policy.runtime !== "codex") policyUnavailable(policy.metadata.id, "codex-policy-required");
  ...
  return policy.skills.map((skill) => {
    ...
    if (candidates.length !== 1 || candidates.some((item) => item.path !== skill.path || !item.enabled))
...
export function nativePolicySkillsAcknowledged(policy: MaterializedPolicy, threadId: string, params: unknown): boolean {
  if (policy.runtime !== "codex" || policy.skills.length === 0) return false;
  const parsed = z.object({ threadId: z.string(), item: z.object({
    type: z.literal("userMessage"), content: z.array(z.unknown()).max(128),
  }) }).safeParse(params);
  ...
  return policy.skills.every((skill) => selected.some((item) => item.name === skill.name && item.path === skill.path));
}
