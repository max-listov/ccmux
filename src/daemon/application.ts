import { createApplication, createManagedSchedule, defineManagedResource, managedServerResource,
  type ApplicationHandle } from "stitchkit/application";
...
export function createDaemonApplication(initial: MachineConfig) {
  ...
  const control = managedServerResource({ id: "control", dependsOn: [controlOwner],
    server: (ctx) => ctx.use(controlOwner).server });
  const observation = createManagedSchedule({ id: "observation", dependsOn: [projection.id],
    everyMs: STATUS_INTERVAL_MS, startAfterMs: 0, overlap: { mode: "skip" },
    run: async ({ signal }) => {
      const m = machine();
      monitoring.begin(m);
      ...
      if (m.stateDir !== initial.stateDir || m.rcPrefix !== initial.rcPrefix) publisher.unavailable("config-changed");
  ...
  const healing = createManagedSchedule({ id: "healing", everyMs: 1000, startAfterMs: 0, overlap: { mode: "skip" },
    run: async ({ signal }) => {
      ...
      try {
        try {
          await cmdEnsure();
          if (!IS_DEV && !guardCleared) { clearBootGuard(BOOT_ATTEMPTS); guardCleared = true; }
        } catch (error) { log.error({ msg: "ensure pass failed", err: String(error) }); }
        signal.throwIfAborted();
