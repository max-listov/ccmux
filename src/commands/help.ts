...
export const COMMANDS: HelpEntry[] = [
  ...
  { verb: "runtime", args: "<name|machine:name> [--json]", desc: "bounded native runtime state, generation and event window; exit 2 stale, 3 unavailable", example: "ccmux runtime agent-a --json" },
  ...
  { verb: "new", args: "<name> <dir> [--agent claude|codex|opencode] [--runtime tui|app-server|native] [-- flags]", desc: "create a managed session; OpenCode uses authenticated native control, Codex optionally uses App Server", example: "ccmux new agent-a ~/code/demo --agent opencode" },
  ...
  { verb: "role", args: "<name|machine:name> <role|--none> | (no args to list)", desc: "declare what a session is FOR, and address it by that: ccmux msg <machine>:@<role>. Applies at once — no restart. A role matching two sessions REFUSES the address and shows both, rather than silently picking one", example: "ccmux role cc-api contract-owner" },
  { verb: "relay", args: "owner/<name> [--task X] \"<their answer>\"", desc: "bring an answer back from an owner OUTSIDE the fleet — recorded as a relay (on behalf of), delivered to whoever wrote, and the letter stops waiting", example: "ccmux relay owner/contract-owner \"shipped in 1.2.0\"" },
  ...
  { verb: "msg", args: "<to|machine:to|app/UUID|machine:app/UUID|owner> <text...> [--to-agent claude|codex|opencode] [--to-thread UUID] [--task X] [--defer] [--after <sec>] [--on-behalf-of <who>]  |  cancel <task>", desc: "chat a managed session, an exact Codex App thread, or 'owner'; --to-agent/--to-thread pin replies; --defer holds until turn end, --after N is a timer", note: "sender is automatic and verified: managed session, exact App thread, or 'cli'", example: "ccmux msg dev:app/4e117aea-… 'build is green' --to-agent codex --to-thread 4e117aea-…" },
