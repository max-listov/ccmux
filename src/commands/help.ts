...
export const COMMANDS: HelpEntry[] = [
  ...
  { verb: "restart", args: "<name> | --all", desc: "bounce a session (survives killing the caller); --all sweeps every session on this machine, one at a time", example: "ccmux restart --all" },
  ...
  { verb: "transcript", args: "<name> <--json [--tail N] [--cursor LINE] | --last-message>", desc: "conversation history as JSON (incremental reads via --cursor), or --last-message for just the agent's final answer as text (full, not clipped)", example: "ccmux transcript cc-api --last-message" },
  { verb: "wait", args: "<name> [--timeout N] [--quiet]", desc: "block until the session finishes its turn — exit 0 settled, 2 timed out (no polling loops; works without chat)", example: "ccmux wait cc-api && ccmux transcript cc-api --last-message" },
