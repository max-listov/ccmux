...
export async function cmdDaemon(): Promise<number> {
  ...
  const signals = bindProcessSignals(application, {
    onShutdown: (sig) => { code = signalExitCode(sig); log.info({ msg: "daemon stopping", sig, code }); },
    onError: (phase, error) => log.error({ msg: "daemon shutdown failed", phase, err: String(error) }),
    onEscalationBlocked: (sig) => process.exit(signalExitCode(sig)),
  });
  try {
    const state = await application.start();
    log.info({ msg: "ccmux daemon up", rcPrefix: m.rcPrefix, interval: m.ensureInterval, lifecycle: state.lifecycle });
