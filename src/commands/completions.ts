...
const cleanDesc = (d: string): string => d.replace(/['`]/g, "").replace(/:/g, " ").split("\n")[0] ?? "";
...
function fish(): string {
  const header = "# ccmux fish completion — install: ccmux completions fish > ~/.config/fish/completions/ccmux.fish";
...
const isShell = (s: string | undefined): s is Shell => s !== undefined && (SHELLS as readonly string[]).includes(s);
