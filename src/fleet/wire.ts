...
/** Mirrors stitchwire's `Result`. Parsed leniently on purpose: a newer agent may add fields, and a
 *  strict shape would turn a routine upgrade into fleet-wide breakage. */
...
export async function runWire(
  ...
  if (parsed.failure !== "none") {
    return { code: 1, stdout: parsed.stdout, stderr: parsed.stderr, transportFailed: true, failureDetail: `${parsed.failure}: ${parsed.detail}` };
  }
  if (parsed.truncated) {
