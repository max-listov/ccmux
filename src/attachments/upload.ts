...
function checkQuota(tx: AttachmentTransaction, target: ManagedPeer, bytes: number): void {
  ...
  assertAttachment(
    records.reduce((sum, row) => sum + row.bytes, 0) + bytes <= ATTACHMENT_LIMITS.hostBytes,
    'attachment-disk-quota',
  );
  const uploads = records.filter((row) => row.phase === 'uploading');
  assertAttachment(
    uploads.length < ATTACHMENT_LIMITS.hostUploads &&
      uploads.filter((row) => sameTarget(row.target, target)).length <
        ATTACHMENT_LIMITS.targetUploads,
...
export function beginUpload(
  ...
  assertAttachment(
    !tx.store.cancelled.some((item) => item.id === input.uploadId),
  ...
  const row: AttachmentRecord = {
    ...
    phase: 'uploading',
    createdAt: now,
    expiresAt: now + ATTACHMENT_LIMITS.uploadTtlMs,
    reference: null,
  };
...
export function appendUpload(
  ...
  assertAttachment(
    data.length > 0 &&
      data.length <= ATTACHMENT_LIMITS.chunkBytes &&
      data.toString('base64') === input.data,
    'attachment-chunk-encoding',
  );
  ...
  try {
    ...
    if (input.offset < row.received) {
      ...
      assertAttachment(
        readSync(fd, prior, 0, prior.length, input.offset) === prior.length && prior.equals(data),
        'attachment-chunk-conflict',
      );
      return uploadReceipt(row);
    }
