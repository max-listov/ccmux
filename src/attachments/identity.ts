...
export function attachmentSession(m: MachineConfig, target: ManagedPeer, write: boolean): Session {
  ...
  assertAttachment((session.agent === "codex" && session.runtime === "app-server")
    || (session.agent === "opencode" && session.runtime === "native"), "unsupported-runtime");
...
export function ownedAttachment(tx: AttachmentTransaction, session: Session, target: ManagedPeer,
  principal: ChatPrincipal, id: string): AttachmentRecord {
  const row = tx.store.records.find((item) => item.id === id);
  assertAttachment(row !== undefined && row.registration === registration(session)
    && sameTarget(row.target, target) && samePrincipal(row.principal, principal), "attachment-scope");
  return row;
}
...
export function sameReference(left: AttachmentReference, right: AttachmentReference): boolean {
  return left.id === right.id && left.digest === right.digest && left.mediaType === right.mediaType
    && left.bytes === right.bytes && left.width === right.width && left.height === right.height;
}

export function verifiedReference(row: AttachmentRecord, reference?: AttachmentReference): AttachmentReference {
  assertAttachment(row.phase !== "uploading" && row.reference !== null, "attachment-incomplete");
  assertAttachment(reference === undefined || sameReference(row.reference, reference), "attachment-reference-changed");
  return row.reference;
}

export function readableAttachment(tx: AttachmentTransaction, session: Session, target: ManagedPeer,
  principal: ChatPrincipal, reference: AttachmentReference): AttachmentRecord {
  const row = tx.store.records.find((item) => item.id === reference.id);
  assertAttachment(row !== undefined, "attachment-scope");
  const direct = row.registration === registration(session) && sameTarget(row.target, target) && samePrincipal(row.principal, principal);
  const inherited = tx.store.pins.some((pin) => pin.registration === registration(session)
    && sameTarget(pin.target, target) && samePrincipal(pin.principal, principal)
