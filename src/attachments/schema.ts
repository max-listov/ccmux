...
import { ATTACHMENT_LIMITS, AttachmentDigestSchema, AttachmentMediaTypeSchema, AttachmentReferenceSchema,
  AttachmentReferencesSchema } from "./reference.ts";

export const AttachmentUploadSelectorSchema = z.object({ target: ManagedPeerSchema, uploadId: z.uuid() }).strict();
...
export const AttachmentChunkSchema = AttachmentUploadSelectorSchema.extend({
  ...
  data: z.string().min(4).max(ATTACHMENT_LIMITS.chunkBytes / 3 * 4)
...
export const AttachmentReadReceiptSchema = z.object({
  reference: AttachmentReferenceSchema,
...
export const AttachmentCancelReceiptSchema = z.object({ uploadId: z.uuid(), cancelled: z.literal(true) }).strict();

export const AttachmentRecordSchema = z.object({
  id: z.uuid(), target: ManagedPeerSchema, principal: ChatPrincipalSchema, registration: z.uuid(),
  mediaType: AttachmentMediaTypeSchema, bytes: z.number().int().positive().max(ATTACHMENT_LIMITS.imageBytes),
  digest: AttachmentDigestSchema, received: z.number().int().nonnegative().max(ATTACHMENT_LIMITS.imageBytes),
  phase: z.enum(["uploading", "verified", "retained"]),
  createdAt: z.number().int().nonnegative(), expiresAt: z.number().int().nonnegative(),
  reference: AttachmentReferenceSchema.nullable(),
}).strict().refine((row) => row.received <= row.bytes && (row.phase === "uploading" ? row.reference === null
  : row.received === row.bytes && row.reference !== null && row.reference.id === row.id
    && row.reference.digest === row.digest && row.reference.bytes === row.bytes && row.reference.mediaType === row.mediaType));
export const AttachmentPinSchema = z.object({
  messageId: z.uuid(), target: ManagedPeerSchema, registration: z.uuid(),
  principal: ChatPrincipalSchema, references: AttachmentReferencesSchema,
}).strict();
export const AttachmentCancellationSchema = z.object({
  id: z.uuid(), target: ManagedPeerSchema, registration: z.uuid(), principal: ChatPrincipalSchema,
  expiresAt: z.number().int().nonnegative(),
}).strict();
export const AttachmentStoreSchema = z.object({
  version: z.literal(1), records: z.array(AttachmentRecordSchema).max(ATTACHMENT_LIMITS.records),
