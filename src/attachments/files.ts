...
export function attachmentRoot(m: MachineConfig): string {
  ...
  try { mkdirSync(root, { mode: 0o700 }); } catch (error) {
    if (!(error instanceof Error && "code" in error && error.code === "EEXIST")) throw error;
  }
  const info = lstatSync(root);
  assertAttachment(info.isDirectory() && !info.isSymbolicLink() && (info.mode & 0o077) === 0, "store-root-type");
  return root;
}

export function attachmentPath(root: string, record: Pick<AttachmentRecord, "id" | "mediaType">): string {
...
export function openPrivate(path: string, flags: number): number {
  ...
  try {
    ...
    assertAttachment(info.isFile() && info.nlink === 1 && (info.mode & 0o077) === 0
      && (process.getuid === undefined || info.uid === process.getuid()), "private-file-type");
    return fd;
...
export function checkPrivateLock(path: string): void {
  ...
  assertAttachment(info.isDirectory() && !info.isSymbolicLink() && (info.mode & 0o077) === 0, "private-lock-type");
}
...
export function writePrivateJson(root: string, name: string, value: unknown): void {
  ...
  try { writePrivate(fd, Buffer.from(JSON.stringify(value)), 0); } finally { closeSync(fd); }
