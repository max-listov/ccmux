...
import { AttachmentUploadSelectorSchema, type AttachmentUploadSelector } from "./schema.ts";
import { withAttachmentStore } from "./store.ts";

export async function finalizeAttachmentUpload(m: MachineConfig, principal: ChatPrincipal,
  ...
  try {
    ...
    return await withDirectoryLock(join(root, ".decoder-lock"), async () => {
      const prepared = await withSessionRegistryLock(m, () => withAttachmentStore(m, "finalize-read", async (tx) => {
        ...
        if (row.phase !== "uploading") return { reference: verifiedReference(row), bytes: null };
        ...
        assertAttachment(bytes.length === row.bytes && createHash("sha256").update(bytes).digest("hex") === row.digest, "attachment-digest");
      ...
      return withSessionRegistryLock(m, () => withAttachmentStore(m, "finalize-commit", async (tx) => {
        const session = attachmentSession(m, parsed.target, true);
        const row = ownedAttachment(tx, session, parsed.target, principal, parsed.uploadId);
        assertAttachment(row.phase === "uploading" && row.received === row.bytes, "attachment-finalize-state");
        const current = readPrivate(attachmentPath(tx.root, row), ATTACHMENT_LIMITS.imageBytes);
        assertAttachment(current.equals(prepared.bytes) && image.mediaType === row.mediaType, "attachment-finalize-changed");
        signal.throwIfAborted();
        row.reference = AttachmentReferenceSchema.parse({ id: row.id, digest: row.digest, bytes: row.bytes, ...image });
        row.phase = "verified";
    ...
    return withAttachmentStore(m, "finalize", async () => { throw error; }, signal);
  }
}
