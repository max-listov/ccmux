...
import { AttachmentStoreSchema, type AttachmentStore } from "./schema.ts";
import { AttachmentFault, assertAttachment, attachmentRefusal } from "./errors.ts";
import { attachmentPath, attachmentRoot, checkPrivateLock, lstatExists, readPrivate, removePrivate, writePrivateJson } from "./files.ts";
...
function reapExpired(tx: AttachmentTransaction, now: number): void {
  const protectedIds = new Set(tx.store.pins.flatMap((pin) => pin.references.map((reference) => reference.id)));
  const expired = tx.store.records.filter((row) => row.phase !== "retained" && row.expiresAt <= now && !protectedIds.has(row.id));
...
function recordFailure(root: string, operation: string, error: unknown): void {
  const reason = error instanceof AttachmentFault ? error.reason
...
export async function withAttachmentStore<T>(m: MachineConfig, operation: string,
  run: (tx: AttachmentTransaction) => Promise<T>, signal?: AbortSignal): Promise<T> {
  ...
  try {
    ...
    return await withDirectoryLock(join(ownedRoot, ".lock"), async () => {
      signal?.throwIfAborted();
      const store = load(ownedRoot);
      const tx: AttachmentTransaction = { root: ownedRoot, store, persist: () => {
        const parsed = AttachmentStoreSchema.parse(store);
        assertAttachment(Buffer.byteLength(JSON.stringify(parsed)) <= STORE_BYTES, "attachment-index-quota");
    ...
    if (root !== undefined) {
      try { recordFailure(root, operation, error); } catch { /* Original refusal remains authoritative if storage is unavailable. */ }
    }
