...
import { DecodedImageSchema, type DecodedImage } from "./imageValidation.ts";
import { assertAttachment, AttachmentFault } from "./errors.ts";

const ReplySchema = z.discriminatedUnion("ok", [
  z.object({ ok: z.literal(true), image: DecodedImageSchema }).strict(),
  z.object({ ok: z.literal(false), reason: z.string().min(1).max(128).regex(/^[a-z0-9-]+$/) }).strict(),
]);
...
export async function decodeAttachment(bytes: Buffer, signal: AbortSignal): Promise<DecodedImage> {
  ...
  try {
    ...
    if (child.exitCode === null) { stop(); await child.exited; }
  }
}
