...
export const DecodedImageSchema = z.object({
...
function dimensions(width: number, height: number): void {
  assertAttachment(width > 0 && height > 0 && width <= ATTACHMENT_LIMITS.dimension
    && height <= ATTACHMENT_LIMITS.dimension && width * height <= ATTACHMENT_LIMITS.pixels, "image-pixel-budget");
}

function pngInflatedBytes(width: number, height: number, depth: number, channels: number, interlace: number): number {
  if (interlace === 0) return height * (1 + Math.ceil(width * depth * channels / 8));
  assertAttachment(interlace === 1, "png-interlace");
  const passes = [[0, 0, 8, 8], [4, 0, 8, 8], [0, 4, 4, 8], [2, 0, 4, 4],
    [0, 2, 2, 4], [1, 0, 2, 2], [0, 1, 1, 2]];
  let size = 0;
  for (const pass of passes) {
    const [x, y, dx, dy] = pass;
    assertAttachment(x !== undefined && y !== undefined && dx !== undefined && dy !== undefined, "png-pass");
...
function png(data: Buffer): DecodedImage {
  assertAttachment(data.length >= 45 && data.readUInt32BE(8) === 13 && data.toString("ascii", 12, 16) === "IHDR", "png-header");
  const width = data.readUInt32BE(16), height = data.readUInt32BE(20);
  dimensions(width, height);
  const depth = data[24], color = data[25], interlace = data[28];
  assertAttachment(depth !== undefined && [1, 2, 4, 8, 16].includes(depth) && interlace !== undefined, "png-format");
  ...
  while (offset + 12 <= data.length) {
    ...
    offset += length + 12;
    if (name === "IEND") { assertAttachment(length === 0 && offset === data.length, "png-end"); ended = true; break; }
  }
  ...
  assertAttachment(decoded.width === width && decoded.height === height && decoded.data.length === width * height * 4, "png-decoded-size");
  return { mediaType: "image/png", width, height };
}

function jpeg(data: Buffer): DecodedImage {
  assertAttachment(data.length >= 4 && data.readUInt16BE(data.length - 2) === 0xffd9, "jpeg-incomplete");
  let offset = 2, width = 0, height = 0;
  while (offset + 4 <= data.length) {
    assertAttachment(data[offset] === 0xff, "jpeg-marker");
    while (data[offset] === 0xff) offset++;
    const marker = data[offset++];
    assertAttachment(marker !== undefined && marker !== 0xda && marker !== 0xd9, "jpeg-missing-frame");
  ...
  dimensions(width, height);
  const decoded = decode(data, { useTArray: true, tolerantDecoding: false,
    maxResolutionInMP: ATTACHMENT_LIMITS.pixels / 1_000_000, maxMemoryUsageInMB: 256 });
  assertAttachment(decoded.width === width && decoded.height === height && decoded.data.length === width * height * 4, "jpeg-decoded-size");
...
export function validateImageBytes(data: Buffer): DecodedImage {
  assertAttachment(data.length > 0 && data.length <= ATTACHMENT_LIMITS.imageBytes, "image-byte-budget");
