...
}).strict().refine((value) => value.width * value.height <= ATTACHMENT_LIMITS.pixels);

export const AttachmentReferencesSchema = z.array(AttachmentReferenceSchema)
  .max(ATTACHMENT_LIMITS.imagesPerMessage)
  .refine((values) => values.reduce((bytes, value) => bytes + value.bytes, 0) <= ATTACHMENT_LIMITS.messageBytes);
