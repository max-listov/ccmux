...
export function previewBytes(path: string, reference: AttachmentReference, offset: number): Buffer {
  ...
  try {
    ...
    assertAttachment(fstatSync(fd).size === reference.bytes && offset <= reference.bytes, "attachment-preview-size");
    ...
    if (cached?.stamp !== before || cached.digest !== reference.digest) {
      ...
      for (let position = 0; position < reference.bytes; position += ATTACHMENT_LIMITS.chunkBytes) {
        digest.update(exactRead(fd, position, Math.min(ATTACHMENT_LIMITS.chunkBytes, reference.bytes - position)));
      }
      assertAttachment(digest.digest("hex") === reference.digest && before === stamp(fd), "attachment-digest");
    ...
    const bytes = exactRead(fd, offset, Math.min(ATTACHMENT_LIMITS.chunkBytes, reference.bytes - offset));
