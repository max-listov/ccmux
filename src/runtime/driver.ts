...
export interface ManagedRuntimeDriver {
  ...
  run(machine: MachineConfig, session: Session, promote?: (session: Session) => Promise<Session>): Promise<void>;
}

const drivers: Partial<Record<Session["agent"], ManagedRuntimeDriver>> = {
  codex: { preflight: preflightOwnedCodex,
    run: (m, s, promote) => runOwnedCodexProcess(m, s, promote === undefined ? undefined : uuid => promote({ ...s, uuid })) },
  opencode: { preflight: (m, flags) => {
    if (flags.length) throw new Error("OpenCode native launch accepts typed configuration, not caller flags");
    preflightOpenCode(m);
  }, run: runOpenCodeProcess },
};

export function nativeDriver(session: Pick<Session, "agent" | "runtime">): ManagedRuntimeDriver | null {
