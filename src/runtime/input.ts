...
export const writeRuntimeInput = (m: MachineConfig, s: Session, input: RuntimeInput): Promise<void> =>
  atomicWrite(path(m, s), JSON.stringify(RuntimeInputSchema.parse(input)), 0o600);
...
export function openCodeMessageId(messageId: string, timestamp: number): string {
  ...
  const suffix = [...createHash("sha256").update(messageId).digest().subarray(0, 14)]
    .map((byte) => alphabet.charAt(byte % alphabet.length)).join("");
