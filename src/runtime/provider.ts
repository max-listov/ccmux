...
export function nativeProvider(id: "opencode" | "custom"): AgentProvider {
  return {
    id,
    preflight: id === "opencode" ? preflightOpenCode : () => { throw new Error("Published custom harness is unavailable"); },
    buildArgv: (_s, m) => {
      if (id !== "opencode" || m.opencodeBin === undefined) throw new Error("Native executable is unavailable");
    ...
    launchEnvKeys: () => ["CCMUX_SESSION", CHAT_CREDENTIAL_ENV, "OPENCODE_SERVER_USERNAME", "OPENCODE_SERVER_PASSWORD"],
    launchInputs: () => [],
    historyFile: () => null,
    parse: () => { throw new Error("Native runtime history requires the structured control feed"); },
    usedTokens: () => null,
    lastModel: () => null,
    scanPane: () => ({ ready: false, state: "indeterminate", atPrompt: null, contextLabel: "-",
