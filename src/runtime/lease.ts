...
export function readRuntimeLease(snapshot: RuntimeLease, now: number, ttlMs: number):
  ...
  if (!snapshot.connected) return { status: "unavailable", reason: snapshot.reason ?? "disconnected" };
  const observed = Date.parse(snapshot.observedAt);
  const expires = Date.parse(snapshot.expiresAt);
  if (!Number.isFinite(observed) || !Number.isFinite(expires) || now < observed || expires < observed || expires - observed > ttlMs)
