...
const InterruptSchema = z.object({ generation: z.uuid(), turnId: z.string().min(1).max(256),
  phase: z.enum(["queued", "uncertain", "accepted", "rejected"]) }).strict();
...
export const readRuntimeInterrupt = (m: MachineConfig, s: Session) => readPrivateJson(path(m, s), InterruptSchema);
...
export async function requestRuntimeInterrupt(m: MachineConfig, s: Session, turnId: string, signal: AbortSignal): Promise<void> {
  const read = readManagedRuntimeStatus(m, s);
  if (read.status !== "live" || read.snapshot?.turn?.id !== turnId || read.snapshot.state !== "working" || read.snapshot.turn.status !== "inProgress")
  ...
  while (Date.now() < deadline) {
    ...
    if (receipt?.generation !== generation || receipt.turnId !== turnId) throw new AppError("TURN_MISMATCH", "Interrupt identity changed", 409);
    if (receipt.phase === "accepted") return;
    if (receipt.phase === "rejected") throw new AppError("TURN_MISMATCH", "Native turn changed", 409);
    await Bun.sleep(25);
  }
