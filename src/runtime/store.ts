...
export function readPrivateJson<T>(path: string, schema: z.ZodType<T>, maxBytes = 128 * 1024): T | null {
  ...
  try {
    ...
    return size > maxBytes ? null : schema.safeParse(JSON.parse(bytes.toString("utf8", 0, size))).data ?? null;
  } catch { return null; }
