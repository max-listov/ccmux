...
export const NativeModelSelectionSchema = z.object({
...
export const NativeTurnOptionsSchema = z.discriminatedUnion("runtime", [
  z.object({ runtime: z.literal("codex"), model: NativeModelSelectionSchema,
    mode: z.enum(["default", "plan"]),
    effort: z.enum(["none", "minimal", "low", "medium", "high", "xhigh", "max", "ultra"]).optional(),
  }).strict(),
  z.object({ runtime: z.literal("opencode"), model: NativeModelSelectionSchema,
    agent: z.string().min(1).max(128).optional(), variant: z.string().min(1).max(128).optional(),
  }).strict(),
]);
export type NativeTurnOptions = z.infer<typeof NativeTurnOptionsSchema>;
export const AcceptedTurnOptionsSchema = z.object({
  revision: z.number().int().nonnegative(), options: NativeTurnOptionsSchema,
}).strict();
export type AcceptedTurnOptions = z.infer<typeof AcceptedTurnOptionsSchema>;
export const NativeSelectionEvidenceSchema = z.object({
  model: NativeModelSelectionSchema, options: NativeTurnOptionsSchema.nullable(),
