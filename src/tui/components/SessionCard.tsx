  ...
  if (framed) {
    ...
    return (
      <Box flexDirection="column" flexShrink={0} width={cardWidth}>
        <Box>
          <Txt color={bc}>{'╭─ '}</Txt>
          <Txt color={external ? 'magenta' : undefined} bold>
            {name}
          </Txt>
          <Txt color={bc}>{' ─── '}</Txt>
          <Txt color={status.color} bold={status.active}>
        ...
        <Box>
          ...
          <Txt
            dim
          >{` · ${row.contextLabel}${row.uptimeSeconds !== null ? ` · up ${row.uptimeText}` : ''}${item.activityText ? ` · ${item.activityText}` : ''}`}</Txt>
          <Box flexGrow={1} />
          <Txt color={bc}>{'│'}</Txt>
        </Box>
        ...
        {empty ? (
          ...
          <>
            <Box>
              <Txt color={bc}>{'│ '}</Txt>
              <Txt color={selected ? 'cyan' : undefined} dim={dim}>
                {'└ '}
              </Txt>
              <Txt color={tool.topColor} bold={!dim}>{`${tool.glyph} ${tool.label} `}</Txt>
              <Box
                width={Math.max(4, lastWidth - dispWidth(`${tool.glyph} ${tool.label} `))}
                flexShrink={0}
                overflow="hidden"
              >
                <Txt dim>
                  {clipWidth(
                    ...
                    Math.max(4, lastWidth - dispWidth(`${tool.glyph} ${tool.label} `)),
                  )}
                </Txt>
              </Box>
              <Box flexGrow={1} />
            ...
            <Box>
              ...
              <Box width={Math.max(4, lastWidth - 2)} flexShrink={0} overflow="hidden">
                {isDiffStat(tool.result) ? (
                  ...
                  <Txt color={tool.resultColor} dim={!tool.resultColor}>
                    {clipWidth(sanitize(tool.result), Math.max(4, lastWidth - 2))}
                  </Txt>
                )}
              </Box>
              <Box flexGrow={1} />
  ...
  return (
    <Box marginBottom={1}>
      ...
      <Box flexDirection="column">
        <SessionRow item={item} selected={selected} spin={spin} />
        <Box paddingLeft={2}>
          <Txt color={selected ? 'cyan' : undefined} dim={!selected}>
            {'└ '}
          </Txt>
          {tool ? (
            <>
              ...
              <Txt dim>
                {clipWidth(sanitize(`${tool.request} · ${tool.result}`), Math.max(8, lastWidth))}
              </Txt>
            </>
          ) : (
...
export const SessionCard = memo(SessionCardImpl, (a, b) => {
  ...
  const x = a.item;
  const y = b.item;
  if (x.external !== y.external || x.ext !== y.ext || x.row.lastMessage !== y.row.lastMessage)
    return false;
  if (
    x.row.model !== y.row.model ||
    x.row.contextLabel !== y.row.contextLabel ||
    x.row.uptimeText !== y.row.uptimeText ||
    x.row.lifecycleError !== y.row.lifecycleError
  )
    return false;
  if (x.activityText !== y.activityText) return false;
  if (
    x.row.session.name !== y.row.session.name ||
    x.row.session.dir !== y.row.session.dir ||
    x.row.session.agent !== y.row.session.agent
  )
    return false;
  const ps = x.status;
  const qs = y.status;
