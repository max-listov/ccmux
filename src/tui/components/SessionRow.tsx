...
export function SessionRow({
  ...
  return (
    <Box>
      {showStatus ? (
        <Txt color={status.color} bold={status.active}>{`${statusMark(status, spin)} `}</Txt>
      ) : null}
      <Txt color={external ? 'magenta' : undefined} bold={selected}>
        {pad(s.name, 17)}
      </Txt>
      ...
      <Txt
        dim
      >{`${pad(row.contextLabel, 9)} ${pad(row.uptimeText, 7)} ${item.activityText ?? ''}`}</Txt>
    </Box>
  );
}
