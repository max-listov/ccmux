...
import { STEERING_LIMITS, SteeringJournalSchema, type SteeringInput, type SteeringJournal } from "./schema.ts";
...
export function readSteeringJournal(m: MachineConfig, s: Session): SteeringJournal {
  ...
  try {
    const value = lstatExists(path) ? SteeringJournalSchema.parse(JSON.parse(readPrivate(path, STEERING_LIMITS.journalBytes).toString()))
      : SteeringJournalSchema.parse({ registration: s.registrationGeneration, threadId: s.uuid, operations: [] });
    if (value.registration !== s.registrationGeneration || value.threadId !== s.uuid) return steeringFailure("IDENTITY_MISMATCH");
    return value;
  } catch { return steeringFailure("STEERING_UNAVAILABLE", 503); }
}
...
export function writeSteeringJournal(m: MachineConfig, s: Session, journal: SteeringJournal): void {
  try {
    ...
    if (Buffer.byteLength(JSON.stringify(value)) > STEERING_LIMITS.journalBytes) return steeringFailure("CAPACITY");
...
export function steeringFingerprint(principal: ChatPrincipal, input: SteeringInput): string {
  return createHash("sha256").update(JSON.stringify([chatPrincipalKey(principal), input])).digest("hex");
}
