...
import { SteeringReceiptSchema, type SteeringOperation, type SteeringJournal, type SteeringReceipt } from "./schema.ts";
import { steeringFailure, writeSteeringJournal } from "./store.ts";

export function priorSteering(journal: SteeringJournal, principal: ChatPrincipal, operationId: string,
  fingerprint?: string): SteeringOperation | null {
  const prior = journal.operations.find(row => row.receipt.operationId === operationId);
  if (!prior) return null;
  if (prior.principal !== chatPrincipalKey(principal) || (fingerprint !== undefined && prior.fingerprint !== fingerprint))
...
export async function reconcileSteering(m: MachineConfig, s: Session, journal: SteeringJournal, operation: SteeringOperation,
  connect: () => Promise<CodexAppRpc>): Promise<SteeringReceipt> {
  ...
  } catch { /* An absent or unreachable proof retains uncertainty; nothing is reinjected. */ }
  finally { rpc?.close(); }
  operation.phase = accepted ? "submitted" : "uncertain";
  operation.reason = accepted ? null : "native-acceptance-unresolved";
  operation.receipt = SteeringReceiptSchema.parse({ ...operation.receipt, state: operation.phase,
