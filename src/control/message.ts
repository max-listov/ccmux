...
import { ControlMessageSchema, type ControlMessage } from "./schema.ts";
...
export async function acceptControlMessage(m: MachineConfig, from: ChatPrincipal, request: ControlMessage, signal: AbortSignal) {
  const input = ControlMessageSchema.parse(request);
  const target = controlTarget(m, input.target);
  const accept = () => withSessionRegistryLock(m, async () => {
    ...
    if (prior) {
      if (!samePrincipal(prior.from, from) || !sameTarget(prior.to, input.target) || prior.body !== input.body
        ...
        || (prior.controlFingerprint !== undefined ? prior.controlFingerprint !== fingerprint
      ...
      return { messageId: prior.id, accepted: true as const, duplicate: true, turnOptions: prior.turnOptions ?? null };
    }
    ...
    const selection = hasNativeRuntime(session) ? await currentSelection(m, session, signal) : undefined;
    const turnOptions = selection === undefined ? undefined : { revision: selection.revision, options: input.options ?? selection.options };
    if (turnOptions !== undefined) await validateTurnOptions(m, session, turnOptions.options, signal, input.images.length > 0);
    ...
    const appended = input.images.length === 0 ? await append()
      : await withPinnedAttachments(m, from, input.target, input.messageId, input.images, append, signal);
    ...
    return { messageId: envelope.id, accepted: true as const, duplicate: false, turnOptions: envelope.turnOptions ?? null };
  });
