...
const CursorSchema = z
  ...
  .strict();
const version = (stat: Awaited<ReturnType<typeof lstat>>): string =>
  createHash('sha256')
    .update([stat.dev, stat.ino, stat.mtimeMs, stat.ctimeMs].join(':'))
    .digest('hex');
...
export async function readControlDirectory(
  input: z.output<typeof ControlDirectoryReadSchema>,
  signal: AbortSignal,
): Promise<ControlDirectoryResult> {
  ...
  try {
    ...
    return ControlDirectoryResultSchema.parse({
      path,
      parent: dirname(path) === path ? null : dirname(path),
      entries: page,
      nextCursor:
        last !== undefined && page.length < entries.length
          ? Buffer.from(
              JSON.stringify({
                path,
                version: stamp,
                after: last.name,
                hidden: input.includeHidden,
              }),
            ).toString('base64url')
          : null,
    });
