...
async function readProviderModels(rpc: CodexAppRpc, input: ModelsReadInput,
  source: ControlModelCatalog["source"]): Promise<ControlModelCatalog> {
    ...
    if (source.provider !== "openai")
      throw new AppError("UNSUPPORTED", "This provider does not expose a supported model catalog", 409);
    ...
    return ControlModelCatalogSchema.parse({
      ...(input.target === undefined ? {} : { target: input.target }), source,
...
export async function readControlModels(m: MachineConfig, input: ModelsReadInput, signal: AbortSignal,
  ...
  try {
    ...
    if (input.target !== undefined) {
      ...
      try {
        return await readProviderModels(rpc, input, { kind: "session", machine: m.rcPrefix,
    ...
    if (launch.envFile !== undefined && !isAbsolute(expandHome(launch.envFile)))
      throw new AppError("UNAVAILABLE", "Host catalog requires a host-scoped environment source", 409);
    return await withCodexCatalogRuntime(m, launch, m.codexHome, signal, async (rpc) =>
      readProviderModels(rpc, input, { kind: "host", machine: m.rcPrefix,
        provider: await nativeModelProvider(rpc),
