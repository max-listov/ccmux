...
export function createControlServer(m: MachineConfig, publisher: ControlPublisher,
  ...
  const observability = createObservability({ request: {
    ...
    write(event) {
      if (!event.ok || ["message", "start", "interrupt"].includes(event.action ?? "")) log.info({ msg: "control request", action: event.action,
        ok: event.ok, code: event.errorCode, durationMs: Math.round(event.durationMs) });
    },
    onSinkError(failure) { log.error({ msg: "control audit failed", err: String(failure.error) }); },
  } });
  const server = createServer({ unix: { path: controlSocket(m), mode: 0o600 },
    services: controls.services, maxJsonBodyBytes: 64 * 1024, logging: false,
    ...(observability.request ? { observability: observability.request } : {}),
    hooks: { authorize: async (ctx, endpoint) => {
      ...
      if (ctx.req?.headers.has("origin")) throw new AppError("FORBIDDEN", "Browser requests are not accepted on local IPC", 403);
