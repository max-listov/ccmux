...
import { readSelection, writeSelection, selectionReceipt } from "../runtime/selection.ts";
import { type AcceptedTurnOptions, type NativeTurnOptions } from "../runtime/selectionSchema.ts";
...
import { ControlModelsReadSchema, type ControlModel } from "./schema.ts";
...
export function exactNativeTarget(m: MachineConfig, input: z.output<typeof SelectionReadSchema>): Session {
  const session = controlTarget(m, input.target);
  if (!hasNativeRuntime(session) || session.archived) throw new AppError("UNSUPPORTED", "An active managed native registration is required", 409);
...
async function catalog(m: MachineConfig, s: Session, signal: AbortSignal): Promise<ControlModel[]> {
  ...
  for (let page = 0; page < 16; page++) {
    const result = await readControlModels(m, ControlModelsReadSchema.parse({ target: managedPeer(m.rcPrefix, s), cursor }), signal);
...
export async function currentSelection(m: MachineConfig, s: Session, signal: AbortSignal): Promise<AcceptedTurnOptions> {
  signal.throwIfAborted();
  const retained = readSelection(m, s);
  if (retained !== null) return retained;
  const model = s.modelSelection;
  if (model === undefined) throw new AppError("UNAVAILABLE", "Initial native selection has not been observed", 409);
  if (s.agent === "codex") return { revision: 0, options: { runtime: "codex", model,
    mode: s.launchRecipe?.collaborationMode ?? "default" } };
...
export async function validateTurnOptions(m: MachineConfig, s: Session, options: NativeTurnOptions, signal: AbortSignal,
  images = false): Promise<void> {
  if (options.runtime !== s.agent) throw new AppError("UNSUPPORTED", "Selection runtime differs from its session", 409);
  if (options.runtime === "opencode" && s.applicationPolicy !== undefined) {
    ...
    if (policy.runtime !== "opencode") policyUnavailable(policy.metadata.id, "opencode-policy-required");
  ...
  const row = models.find(item => (item.model ?? item.id) === options.model.model &&
  ...
  if (options.runtime === "codex" && options.effort !== undefined &&
  ...
  if (options.runtime === "opencode") {
    ...
    if ((options.agent !== undefined && !choices.agents.includes(options.agent)) ||
...
export function requireNativeIdle(m: MachineConfig, s: Session): void {
  ...
  if (read.status !== "live" || read.snapshot?.state !== "idle" || read.snapshot.turn?.status === "inProgress" ||
      read.snapshot.pendingRequests.length > 0 || blockingInbound(m, s, Date.now()).length > 0)
    throw new AppError("BUSY", "Native session is not between turns", 409);
}

export async function readControlSelection(m: MachineConfig, input: z.output<typeof SelectionReadSchema>, signal: AbortSignal) {
  const session = exactNativeTarget(m, input);
  return SelectionResultSchema.parse({ protocol: 1, registrationGeneration: input.registrationGeneration,
    current: await currentSelection(m, session, signal) });
}

export async function updateControlSelection(m: MachineConfig, input: z.output<typeof SelectionUpdateSchema>, signal: AbortSignal) {
  const session = exactNativeTarget(m, input);
  return withNativeAdmission(m, session, async () => {
    ...
    if (prior !== null) {
      return SelectionResultSchema.parse({ protocol: 1, registrationGeneration: input.registrationGeneration, current: prior });
    }
    requireNativeIdle(m, session);
    const current = await currentSelection(m, session, signal);
    if (current.revision !== input.expectedRevision) throw new AppError("REVISION_CONFLICT", "Session selection changed", 409);
    await validateTurnOptions(m, session, input.options, signal);
    const result = SelectionResultSchema.parse({ protocol: 1, registrationGeneration: input.registrationGeneration,
