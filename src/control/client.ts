...
}).strict().refine((value) => (value.session === undefined) === (value.credential === undefined),
...
export function createControlClient(options: ControlClientOptions = {}) {
  ...
  return { ...createClient(controlContract, transport.http), ...createClient(controlEventsContract, transport.stream), close: transport.close };
}
