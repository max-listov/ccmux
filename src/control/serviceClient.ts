...
export const ccmuxControlServiceContract = defineContract(
  { prefix: CCMUX_CONTROL_SERVICE_PREFIX },
  {
    ...
    permissionRead: {
      ...
      desc: 'Read the accepted permission mode and the one observed natively',
      input: ControlPermissionReadSchema,
      output: serviceReply(ControlPermissionResultSchema),
      idempotent: true,
      meta: { effect: controlServiceEffects['permission.read'] },
    },
    permissionUpdate: {
      method: 'POST',
      path: '/permission.update',
      desc: 'Compare-and-swap the accepted permission mode',
      input: ControlPermissionUpdateSchema,
      output: serviceReply(ControlPermissionResultSchema),
      idempotent: true,
      meta: { effect: controlServiceEffects['permission.update'] },
    },
    attachmentBegin: {
      method: 'POST',
