...
import { CCMUX_CONTROL_SERVICE_PREFIX, CCMUX_CONTROL_SERVICE_BASE_URL, ControlServiceReplyEnvelopeSchema,
  ControlServiceWaitSchema, ControlServiceOperationSchema, controlServiceEffects, controlServiceInputs } from "./serviceCatalog.ts";
...
export const ccmuxControlServiceContract = defineContract(
  ...
  {
    history: { method: "POST", path: "/history.read", desc: "Read a bounded native history page", input: ControlHistoryReadSchema, output: serviceReply(ControlHistoryResultSchema), idempotent: true, timeout: 7_000, meta: { effect: controlServiceEffects["history.read"] } },
    compact: { method: "POST", path: "/context.compact", desc: "Accept exact idle native compaction", input: ControlCompactSchema, output: serviceReply(ControlContextOperationResultSchema), idempotent: true, meta: { effect: controlServiceEffects["context.compact"] } },
    contextOperation: { method: "POST", path: "/context.operation", desc: "Read durable compaction evidence", input: ControlContextOperationReadSchema, output: serviceReply(ControlContextOperationResultSchema), idempotent: true, meta: { effect: controlServiceEffects["context.operation"] } },
    fork: { method: "POST", path: "/session.fork", desc: "Fork an exact idle native conversation", input: NativeForkRequestSchema, output: serviceReply(ControlCreateReceiptSchema), idempotent: true, timeout: 30_000, meta: { effect: controlServiceEffects["session.fork"] } },
    steer: { method: "POST", path: "/turn.steer", desc: "Submit content to an exact active native turn", input: SteeringInputSchema, output: serviceReply(SteeringReceiptSchema), idempotent: true, timeout: 15_000, meta: { effect: controlServiceEffects["turn.steer"] } },
    steeringOperation: { method: "POST", path: "/turn.steering-operation", desc: "Read exact steering acceptance", input: SteeringSelectorSchema, output: serviceReply(SteeringReadResultSchema), idempotent: true, meta: { effect: controlServiceEffects["turn.steering-operation"] } },
    selection: { method: "POST", path: "/selection.read", desc: "Read revisioned native turn defaults for an exact registration", input: SelectionReadSchema, output: serviceReply(SelectionResultSchema), idempotent: true, meta: { effect: controlServiceEffects["selection.read"] } },
    select: { method: "POST", path: "/selection.update", desc: "Compare-and-swap native turn defaults between turns", input: SelectionUpdateSchema, output: serviceReply(SelectionResultSchema), idempotent: true, meta: { effect: controlServiceEffects["selection.update"] } },
    attachmentBegin: { method: "POST", path: "/attachment.begin", desc: "Reserve an authenticated bounded image upload", input: AttachmentBeginSchema, output: serviceReply(AttachmentUploadReceiptSchema), idempotent: true, meta: { effect: controlServiceEffects["attachment.begin"] } },
    attachmentChunk: { method: "POST", path: "/attachment.chunk", desc: "Append one authenticated bounded image chunk", input: AttachmentChunkSchema, output: serviceReply(AttachmentUploadReceiptSchema), idempotent: true, meta: { effect: controlServiceEffects["attachment.chunk"] } },
    attachmentFinalize: { method: "POST", path: "/attachment.finalize", desc: "Decode and verify complete image bytes", input: AttachmentUploadSelectorSchema, output: serviceReply(AttachmentReferenceSchema), idempotent: true, timeout: 15_000, meta: { effect: controlServiceEffects["attachment.finalize"] } },
    attachmentCancel: { method: "POST", path: "/attachment.cancel", desc: "Cancel only unretained image bytes", input: AttachmentUploadSelectorSchema, output: serviceReply(AttachmentCancelReceiptSchema), idempotent: true, meta: { effect: controlServiceEffects["attachment.cancel"] } },
    attachmentRead: { method: "POST", path: "/attachment.read", desc: "Read an authenticated bounded image preview", input: AttachmentReadSchema, output: serviceReply(AttachmentReadReceiptSchema), idempotent: true, meta: { effect: controlServiceEffects["attachment.read"] } },
