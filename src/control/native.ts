...
export async function interruptControlTurn(m: MachineConfig, target: ManagedPeer, turnId: string, signal: AbortSignal) {
...
export async function waitControlSession(m: MachineConfig, publisher: ControlPublisher, target: ManagedPeer,
  ...
  let state = publisher.read().sessions.find((row) => row.identity.session === target.session) ?? null;
  for await (const snapshot of publisher.subscribe(combined)) {
    ...
    if (state?.identity.threadId !== target.threadId || state.availability !== "live"
      || Date.parse(state.observedAt) < startedAt) continue;
    if (state.turn?.status === "failed") return ControlWaitResultSchema.parse({ target, outcome: "failed", state });
    if (state.state === "idle" && state.turn?.status !== "inProgress"
      && loadCursors(m).pickups[managedPeerKey(target)] === undefined && blockingInbound(m, session, Date.now()).length === 0) {
      return ControlWaitResultSchema.parse({ target, outcome: state.turn?.status ?? "idle", state });
    }
  }
  signal.throwIfAborted();
  return ControlWaitResultSchema.parse({ target, outcome: timeout.aborted ? "timeout" : "unavailable", state });
}
