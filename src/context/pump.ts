...
import { readContextJournal, withContextJournal } from './store.ts';
...
export async function observeContextCompletion(
  m: MachineConfig,
  s: Session,
  generation: string,
  completionId?: string,
  publishBoundary?: () => Promise<void>,
): Promise<void> {
  await withContextJournal(m, s, async (journal, persist) => {
    if (
      completionId !== undefined &&
      journal.lastCompletion?.generation === generation &&
      journal.lastCompletion.id === completionId
    )
      return;
    const operation = journal.operations.find(
      (row) =>
        row.generation === generation &&
        ['dispatching', 'running', 'uncertain'].includes(row.state),
    );
    if (operation && completionId !== undefined && completionId === operation.markerBefore) return;
    if (
      !operation &&
      (completionId === undefined ||
        readManagedRuntimeStatus(m, s).snapshot?.generation !== generation)
    )
      return;
    ...
    if (operation) {
      ...
      operation.revision = journal.revision;
    }
...
async function history(
  ...
  if (request.generation !== generation || request.expiresAt <= Date.now()) {
    await writeHistoryMailbox(m, s, { ...request, state: 'failed', page: null });
    return;
  }
  try {
    ...
    const page = NativeHistoryPageSchema.parse(
      await api.history(
        request.query,
        AbortSignal.any([signal, AbortSignal.timeout(Math.max(1, request.expiresAt - Date.now()))]),
      ),
    );
    ...
    if (readHistoryMailbox(m, s)?.id === request.id)
      await writeHistoryMailbox(m, s, {
        ...request,
        state: 'failed',
        page: null,
        error:
          error instanceof AppError && error.code === 'HISTORY_CURSOR'
            ? 'HISTORY_CURSOR'
            : 'HISTORY_UNAVAILABLE',
      });
  }
...
export async function applyContextCommands(
  m: MachineConfig,
  s: Session,
  generation: string,
  api: NativeContextApi,
  ...
  const active = readContextJournal(m, s).operations.find(
    ...
    if (active.state !== 'queued') {
      try {
        ...
        if (marker !== null && marker !== active.markerBefore) {
          await observeContextCompletion(m, s, active.generation, marker, publishBoundary);
          return;
        }
      } catch (error) {
        await recordRuntimeDiagnostic(m, s.name, 'context-reconcile', error);
      }
