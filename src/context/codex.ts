...
const boundedSignal = (signal: AbortSignal) =>
  AbortSignal.any([signal, AbortSignal.timeout(HISTORY_LIMITS.deadlineMs)]);
...
const CompletionSchema = z.object({
  threadId: Id,
  turnId: Id,
  item: z.object({ type: z.literal('contextCompaction'), id: Id }),
});
...
export function codexContextApi(m: MachineConfig, s: Session, rpc: CodexAppRpc): NativeContextApi {
  const read = async (limit: number, signal: AbortSignal, cursor?: string) =>
    PageSchema.parse(
  ...
  return {
    async history(query, callerSignal) {
      ...
      for (const { turnId, item } of page.data) {
        ...
        if (item.type === 'userMessage') {
          ...
          text = inputs
            ...
            .join('\n');
          const nativeImages = inputs.filter(
            (input) => input.type === 'image' || input.type === 'localImage',
          );
          images = nativeImages.length;
          pointers = nativeImages.flatMap((input) =>
      ...
      return boundedHistoryPage(
        ...
        entries,
        page.nextCursor,
        page.nextCursor === null ? 'complete' : 'more',
      );
    },
