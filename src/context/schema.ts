...
export const NativeHistoryQuerySchema = z
  .object({
    limit: z.number().int().min(1).max(HISTORY_LIMITS.entries).default(32),
    cursor: z.string().min(1).max(8_192).optional(),
  })
  .strict();
export const NativeHistoryEntrySchema = z
  .object({
    turnId: Id,
    itemId: Id,
...
export const NativeHistoryPageSchema = z
  .object({
    ...
    nextCursor: z.string().max(8_192).nullable(),
    completeness: z.enum(['complete', 'more', 'unknown']),
    omittedItems: z.number().int().nonnegative(),
    omittedBytes: z.number().int().nonnegative(),
  })
  .strict();
export const CompactRequestSchema = z
  .object({ operationId: z.uuid(), generation: z.uuid() })
  .strict();
export const NativeForkRequestSchema = z
  .object({
