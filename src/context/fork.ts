...
export const NativeForkSourceSchema = z
  .object({
    ...
    selection: NativeTurnOptionsSchema.optional(),
  })
  .strict();
const IntentSchema = z
  .object({
    destination: z.uuid(),
    source: NativeForkSourceSchema,
    state: z.enum(['reserved', 'dispatching', 'accepted', 'uncertain']),
...
const path = (m: MachineConfig, generation: string) =>
  join(m.stateDir, 'native-forks', `${z.uuid().parse(generation)}.json`);
export function readNativeForkIntent(m: MachineConfig, s: Session): NativeForkIntent | null {
  ...
  if (row === null && existsSync(file))
    throw new Error('Native fork admission state is unavailable');
  return row;
}
export async function prepareNativeFork(
  m: MachineConfig,
  generation: string,
  ...
  if (prior) {
    if (JSON.stringify(prior.source) !== JSON.stringify(NativeForkSourceSchema.parse(source)))
      throw new Error('Native fork source changed');
    return;
  }
  ...
  await atomicWrite(
    file,
    JSON.stringify(
      IntentSchema.parse({ destination: generation, source, state: 'reserved', nativeId: null }),
    ),
    0o600,
  );
}
...
export async function admitNativeFork<T>(
  m: MachineConfig,
  s: Session,
  adapter: NativeForkAdapter<T>,
  signal: AbortSignal,
): Promise<T> {
  ...
  return withDirectoryLock(
    ...
    async () => {
      ...
      try {
        ...
        if (nativeId === intent.source.nativeId)
          throw new Error('Native fork returned its source identity');
        await persist({ ...intent, state: 'accepted', nativeId });
        return response;
      } catch (error) {
