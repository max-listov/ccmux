 ...
 * target session (mail arrives at the target's turn boundary by default, never mid-work),
...
3. DELIVER — ALWAYS: \`${cli} msg <target> --on-behalf-of owner "<instruction>"\`. Mail arrives when the target voluntarily finishes its turn; it never interrupts unless you pass --interrupt, which you do not. NEVER use \`${cli} send\` or raw keys into a live pane — that is the mid-turn interruption this whole mechanism exists to prevent.
...
7. RETRY at most twice, each naming the SPECIFIC gap ("you did X but not Y; do Y"), and re-arm a fresh watchdog. No bare "try again", no infinite re-asking.
