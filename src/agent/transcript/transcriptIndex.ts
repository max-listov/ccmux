...
import { TranscriptStatsSchema } from '../../context/transcriptJsonSchema.ts';
...
/** Metadata key holding the transcript path an index describes. */
const SOURCE_KEY = 'source';
/** An index is opened to ask for its source only once nothing has advanced it for this long: closing
 *  the last connection to a database in WAL mode can checkpoint and remove its log, and an index that
 *  is in use has a writer that must not have that done under it. */
const QUIET_MS = 24 * 60 * 60 * 1000;
/** Indexes examined between yields, so a prune of thousands never holds the daemon's loop. */
const PRUNE_BATCH = 100;

/** The transcript path an index records, or null when it records none (written before it did). */
function recordedSource(index: string): string | null {
  let db: Database | undefined;
  try {
    db = new Database(index, { readonly: true });
    const row = db.query('SELECT body FROM metadata WHERE key=?').get(SOURCE_KEY);
    const parsed = z.object({ body: z.string() }).safeParse(row);
    if (!parsed.success) return null;
    const source = z.string().safeParse(JSON.parse(parsed.data.body));
...
/**
 * Remove indexes whose transcript is gone or that nothing has used for `INDEX_IDLE_MS`, and every
 * file of the retired JSON format.
 *
 * Nothing removed an index before, and on one machine indexes of transcripts that no longer existed
 * had reached 3137 of 3420 (575 MB). An index records its transcript's path, so a missing transcript
 * is recognised directly; one written before it recorded the path goes by age, which is what the
 * cache root promises is safe — an index removed while its transcript is still read is rebuilt on the
 * next read, at the cost of one pass over the file.
 *
 * Opening every index took 1.6 s for 3477 of them, so the work yields between batches.
 */
export async function pruneTranscriptIndexes(
  now = Date.now(),
  dir = INDEX_DIR(),
  ...
  const drop = (name: string): void => {
    try {
      ...
      // Gone already, or removed by another pass; either way it is not here any more.
    }
  };
  for (const name of names) {
    ...
    try {
      // The write-ahead log moves when the index does, often without the main file; the newer of the
      // two is when this index was last advanced.
      const main = statSync(join(dir, name)).mtimeMs;
      let wal = 0;
      try {
        wal = statSync(join(dir, `${name}-wal`)).mtimeMs;
      } catch {
        wal = 0;
      }
      idleSince = Math.max(main, wal);
    } catch {
      continue;
    }
    examined++;
    if (examined % PRUNE_BATCH === 0) await Bun.sleep(0);
    const age = now - idleSince;
    const idle = age >= INDEX_IDLE_MS;
    if (!idle) {
      if (age < QUIET_MS) continue;
      const source = recordedSource(join(dir, name));
      if (source === null || existsSync(source)) continue;
    }
    for (const part of [name, `${name}-wal`, `${name}-shm`]) drop(part);
    removed++;
  }
  ...
  try {
    ...
    const index = advanceOrCommitted(
      ...
      () => {
        ...
        // The file is named by a hash, so the path it describes is written inside it: that is what
        // lets the cache prune ask whether the transcript still exists.
