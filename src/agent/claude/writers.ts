...
const RESUME_UUID_RE =
  /--(?:resume|session-id)[= ]([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/g;
...
export function classifyWriters(procs: PsProc[], uuid: string, selfPid: number): Writer[] {
  ...
  const out: Writer[] = [];
  for (const p of procs) {
    if (!p.command.includes(`--resume ${uuid}`) && !p.command.includes(`--session-id ${uuid}`))
      continue;
...
export function addEnvSelf(
  ...
  return [
    ...writers,
    {
      pid: selfPid,
      kind: 'self',
      command: '(this very conversation — via CLAUDE_CODE_SESSION_ID)',
    },
  ];
}
