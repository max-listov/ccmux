...
export const OwnedCodexStateSchema = z.enum([
  'working',
  'idle',
  'waiting-approval',
  'waiting-input',
...
export const OwnedCodexTurnSchema = z
  .object({
  ...
  .strict();
export const OwnedCodexEventSchema = z
  .object({
  ...
  .strict();
export const OwnedCodexUsageSchema = z
  .object({
  ...
  .strict();
export const OwnedCodexNativeItemSchema = z
  .object({
    sequence: z.number().int().positive(),
    at: z.iso.datetime(),
    kind: z.enum([
  ...
  .strict();
const OwnedCodexQuestionSchema = z
  .object({
    id: z.string().min(1).max(256),
  ...
  .strict();
export const OwnedCodexPendingRequestSchema = z
  .object({
  ...
  .strict();
export const OwnedCodexSnapshotSchema = z
  .object({
    ...
    nativeItems: z
      .array(OwnedCodexNativeItemSchema)
      .max(CODEX_RUNTIME_MAX_NATIVE_ITEMS)
      .default([]),
