...
import { readBoundedFile, MonitoringFileError } from "../../monitoring/native-file.ts";
import { ownedCodexStatusPath } from "./ownedPaths.ts";
import { CODEX_RUNTIME_MAX_BYTES, type OwnedCodexRead } from "./ownedSchema.ts";
import { unavailableOwnedCodex, validateOwnedCodex, validateOwnedCodexLiveness } from "./ownedStatus.ts";

export const OwnedCodexReadOptionsSchema = z.object({
  session: z.string().min(1).max(256), threadId: z.uuid(),
...
async function readConfig(path: string): Promise<string> {
  try { return await readBoundedFile(path, 128 * 1024); }
  catch (error) { if (error instanceof MonitoringFileError && error.reason === "missing") return "{}"; throw error; }
}
...
async function readOnce(session: string, threadId: string): Promise<OwnedCodexRead> {
  try {
    ...
    const bytes = await readBoundedFile(ownedCodexStatusPath(location, session), CODEX_RUNTIME_MAX_BYTES);
    const after = await readConfig(path);
    const next = resolveMonitoringLocation(JSON.parse(after));
    if (path !== machineConfigPath() || config !== after || next.stateDir !== location.stateDir || next.rcPrefix !== location.rcPrefix) {
      return unavailableOwnedCodex("config-changed");
    }
    return validateOwnedCodex(bytes, { machine: location.rcPrefix, session, threadId });
  } catch (error) { return unavailableOwnedCodex(error instanceof MonitoringFileError ? error.reason : "invalid"); }
}
...
export function readCodexRuntime(input: OwnedCodexReadOptions): Promise<OwnedCodexRead> {
  ...
  return new Promise((resolve) => {
    ...
    const deliver = (result: OwnedCodexRead) => {
      ...
      const checked = result.snapshot === null ? result : validateOwnedCodexLiveness(structuredClone(result.snapshot));
      finish(performance.now() >= deadline ? unavailableOwnedCodex("deadline") : checked);
    };
    options.signal?.addEventListener("abort", cancel, { once: true });
    const active = reads.get(key);
    if (active !== undefined) { active.waiters.add(deliver); return; }
    const entry = { waiters: new Set([deliver]), active: readOnce(options.session, options.threadId) };
    reads.set(key, entry);
