...
const NativeConfigSchema = z.object({ config: z.object({
...
export async function nativeModelProvider(rpc: CodexAppRpc, cwd?: string): Promise<string> {
  const read = NativeConfigSchema.parse(await rpc.request("config/read", {
...
export async function withCodexCatalogRuntime<T>(m: MachineConfig, launch: ResolvedControlLaunch,
  cwd: string, signal: AbortSignal, read: (rpc: CodexAppRpc) => Promise<T>): Promise<T> {
  ...
  const recipe = sessionEnvRecipe({ dir: cwd, ...(launch.envFile === undefined ? {} : { envFile: launch.envFile }) },
  ...
  try {
    const process = Bun.spawn([m.codexBin, "app-server", "--listen", `unix://${socket}`, ...flags], {
      cwd, env: { ...recipe.env, CODEX_HOME: m.codexHome }, detached: true,
      stdin: "ignore", stdout: "ignore", stderr: "pipe",
    });
    child = process;
    // Keep only a bounded private tail. Provider output never enters public responses or stderr logs.
    drain = (async () => { for await (const bytes of process.stderr)
      diagnostic = Buffer.concat([diagnostic, bytes]).subarray(-16_384); })();
    while (!existsSync(socket)) {
      signal.throwIfAborted();
      if (process.exitCode !== null) throw new Error(`Catalog runtime exited (${process.exitCode})`);
      await Bun.sleep(20);
    }
    ...
    await atomicWrite(join(directory, "catalog-diagnostic.json"), JSON.stringify({
      observedAt: new Date().toISOString(), reason: String(error).slice(0, 2_048),
