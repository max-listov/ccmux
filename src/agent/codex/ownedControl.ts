...
export function readNativeCommand(
  ...
  return (
    NativeResponseCommandSchema.safeParse(boundedJson(nativeCommandPath(m, name))).data ?? null
  );
}
export function readNativeReceipt(
  ...
  return (
    NativeResponseReceiptSchema.safeParse(boundedJson(nativeReceiptPath(m, name))).data ?? null
  );
}
