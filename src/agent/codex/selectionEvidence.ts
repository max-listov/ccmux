...
export function codexSelectionEvent(
  event: CodexRpcEvent,
  threadId: string,
  previous: NativeSelectionEvidence | null,
  ...
  if (event.method === 'thread/settings/updated') {
    ...
    const model = NativeModelSelectionSchema.parse({
  ...
  if (event.method === 'model/rerouted') {
    ...
    return {
      model: NativeModelSelectionSchema.parse({ ...previous.model, model: data.toModel }),
      options: null,
      source: 'reroute',
      turnId: data.turnId,
    };
