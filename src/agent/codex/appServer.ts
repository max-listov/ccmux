...
export async function prepareManagedCodexTurn(
  ...
  const application = session.applicationPolicy === undefined ? undefined
    : verifyApplicationPolicy(m, "codex", session.applicationPolicy);
  const skillInputs = application === undefined ? [] : policySkillInputs(application, session.dir,
    application.runtime === "codex" && application.skills.length > 0
      ? await rpc.request("skills/list", { cwds: [session.dir], forceReload: true }) : null);
  const skills = skillInputs.length > 0 ? { skillInputs } : {};
  if (options !== undefined && options.runtime !== "codex") collaborationUnavailable(session, "Runtime selection differs");
  const selected = options?.model ?? session.modelSelection;
  if (selected !== undefined && context.modelProvider !== undefined && context.modelProvider !== selected.provider)
    collaborationUnavailable(session, "Changing provider requires a new native runtime");
  const mode = options?.runtime === "codex" ? options.mode : session.launchRecipe?.collaborationMode;
  const model = selected?.model ?? context.model;
  if (model === undefined) {
    if (application !== undefined) collaborationUnavailable(session, "Native model context is unavailable for policy admission");
    return undefined;
  }
  ...
  return { model, ...skills, ...(effort === undefined ? {} : { effort }), collaborationMode: { mode, settings: {
    model,
    reasoning_effort: effort ?? (preset.reasoning_effort === undefined
...
export async function startCodexAppTurn(rpc: CodexAppRpc, threadId: string, messageId: string, input: CodexTurnInput[],
