...
const key = (m: Pick<MachineConfig, "stateDir">, name: string): string =>
  createHash("sha256").update(JSON.stringify([m.stateDir, name])).digest("hex").slice(0, 24);
...
export function privateRuntimeDirectory(path: string): void {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink() || stat.uid !== process.getuid?.() || (stat.mode & 0o077) !== 0) {
    throw new Error("Codex runtime directory must be a private directory owned by the current user");
  }
}
