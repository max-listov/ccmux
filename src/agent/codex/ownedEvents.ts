...
export function emitOwnedCodexBoundary(m: MachineConfig, s: Session, snapshot: OwnedCodexSnapshot): void {
  ...
  appendEvent(m, s, {
    ...
    ...(event.kind === "turn-end" && event.turn?.status !== "completed" ? { interrupted: true, detail: `native turn ${event.turn?.status ?? "unknown"}` } : {}),
  });
}
