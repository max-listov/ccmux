...
import { ownedChildAlive, stopOwnedChildGroup } from './child.ts';
import { OwnedCodexConnection } from './connection.ts';
import { ownedCodexArgv, ownedCodexClientArgv } from './launch.ts';
import { ownedCodexSocket } from './paths.ts';
...
async function socketOccupied(path: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    socket.once('error', (error) =>
      finish(!('code' in error && ['ENOENT', 'ECONNREFUSED'].includes(String(error.code)))),
    );
  });
}

/** One native server per session; the provider's own OS writer lock also guards its UUID. */
export async function runOwnedCodexProcess(
  m: MachineConfig,
  initial: Session,
  promote?: (uuid: string) => Promise<Session>,
): Promise<void> {
  verifyManagedLaunchRecipe(m, initial);
  if (initial.applicationPolicy !== undefined)
    verifyApplicationPolicy(m, 'codex', initial.applicationPolicy);
  ...
  await withLock(
    ...
    async () => {
      ...
      if (await socketOccupied(socket))
        throw new Error('Native runtime endpoint already has an owner');
      if (promote === undefined && historyFile(initial, m) === null)
        throw new Error(
          'Native resume history is missing; refusing to create another conversation',
        );
  ...
  try {
    ...
    drain = (async () => {
      for await (const bytes of child.stderr)
        stderrTail = (stderrTail + Buffer.from(bytes).toString()).slice(-8_192);
    })();
    ...
    while (!abort.signal.aborted) {
      ...
      if (!ownedChildAlive(child.pid))
        throw new ManagedRuntimeExit('Native provider launcher exited');
      if ((client === null || client.exitCode !== null) && Date.now() >= clientAt) {
        client = Bun.spawn(ownedCodexClientArgv(session, m), {
          cwd: session.dir,
          env,
          stdin: 'inherit',
          stdout: 'inherit',
      ...
      if (Date.now() >= reconnectAt) {
        ...
        try {
          ...
          if (!abort.signal.aborted)
            log.warn({
              msg: 'native observer reconnecting',
              name: session.name,
              error: String(error),
            });
          await connection
            ?.close('disconnected')
            .catch((error: unknown) =>
              log.error({ msg: 'native disconnect publication failed', error: String(error) }),
            );
          connection = null;
    ...
    if (stopping) return;
    log.error({
      msg: 'owned Codex runtime failure',
      name: initial.name,
      error: String(error),
      stderr: stderrTail,
    });
    ...
    await connection
      ?.close('stopped')
      .catch((error: unknown) =>
        log.error({ msg: 'native stop publication failed', error: String(error) }),
      );
