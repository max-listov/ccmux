...
import { projectNativeEvent, projectNativeRequest, resolvedRequestId } from './native.ts';
import type { OwnedCodexSnapshot } from './schema.ts';
...
export class OwnedCodexProjection {
  ...
  constructor(m: MachineConfig, s: Session, providerPid: number, now = Date.now()) {
    this.value = {
      ...
      ...(s.applicationPolicy === undefined
        ? {}
        : { applicationPolicy: { policy: s.applicationPolicy, state: 'desired' } }),
    };
  }
    ...
    this.value.turn =
      turn === null
        ...
        : {
            ...
            startedAt:
              turn.startedAt ??
              (this.value.turn?.id === turn.id ? this.value.turn.startedAt : null),
          };
  }
  ...
  event(event: CodexRpcEvent, now = Date.now()): boolean {
    ...
    if (selection !== null) {
      this.selectionEvidence(selection);
      this.touch(now);
      return true;
    }
    ...
    if (event.method === 'turn/started' || event.method === 'turn/completed') {
      ...
      if (
        ...
        this.value.turn !== null &&
        this.value.turn.id !== turn.id
      )
        return false;
    ...
    const native = projectNativeEvent(
      event,
      this.value.threadId,
      this.value.nativeSequence + 1,
      now,
    );
    if (native !== null) {
      this.appendNative(native);
      changed = true;
    }
    return changed;
  }
  ...
  request(request: CodexRpcRequest, now = Date.now()): boolean {
    ...
    if (
      pending === null ||
      this.value.pendingRequests.some((item) => item.requestId === pending.requestId) ||
      this.submittedRequests.has(pending.requestId)
    )
      return false;
    ...
    this.touch(now);
    this.appendNative({
      sequence: this.value.nativeSequence + 1,
      at: pending.requestedAt,
      kind: pending.kind,
      stage: 'requested',
      nativeId: pending.itemId,
      turnId: pending.turnId,
      requestId: pending.requestId,
      status: null,
      text: pending.reason,
      tool: pending.approvalKind,
      usage: null,
    });
    return true;
  }
  ...
  submitRequest(requestId: string, now = Date.now()): boolean {
    ...
    this.settleRequest(pending, 'submitted', now);
    return true;
  }
  ...
  resolveRequest(requestId: string, now = Date.now()): boolean {
    ...
    this.settleRequest(pending, 'resolved', now);
    return true;
  }

  /** Take a request off the open list and record how it left; the turn works again once none
   *  is left open. */
  private settleRequest(
    pending: NativePendingRequest,
    stage: 'submitted' | 'resolved',
    now: number,
  ): void {
    this.value.pendingRequests = this.value.pendingRequests.filter(
      (item) => item.requestId !== pending.requestId,
    );
    if (this.value.pendingRequests.length === 0 && this.value.turn?.status === 'inProgress')
  ...
  private applyStatus(status: unknown, now: number): void {
    ...
    const changed =
      this.value.state !== native.state || this.value.reason !== reason || !this.value.connected;
    this.value.connected = true;
