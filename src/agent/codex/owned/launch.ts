...
import { promptInvocation, UID } from '../../../util/env.ts';
...
export function ownedCodexFlags(flags: readonly string[]): { server: string[]; client: string[] } {
  ...
  for (let i = 0; i < flags.length; i++) {
    ...
    if (flag === '--no-alt-screen') {
      client.push(flag);
      continue;
    }
    if (flag === '--dangerously-bypass-approvals-and-sandbox') {
      if (UID === 0)
        throw new Error('An owned root runtime cannot bypass approval and sandbox policy');
    ...
    throw new Error(
      `Unsupported App Server flag: ${flag}; use -c key=value for native configuration`,
    );
  }
...
export function ownedCodexArgv(s: Session, m: MachineConfig, cli = promptInvocation()): string[] {
  ...
  return [
    ...
    'app-server',
    '--listen',
    `unix://${ownedCodexSocket(m, s.name)}`,
    ...ownedCodexFlags([...s.flags, ...m.extraFlags]).server,
    ...modelSelectionFlags(s.modelSelection),
    '-c',
...
export function ownedCodexClientArgv(s: Session, m: MachineConfig): string[] {
  ...
  return [
    ...
    ...ownedCodexFlags([...s.flags, ...m.extraFlags]).client,
  ];
}
...
export function ownedCodexDeveloperInstructions(s: Session, m: MachineConfig): string {
  ...
  return s.applicationPolicy === undefined
    ? supervisor
    : composePolicyDeveloperInstructions(
        verifyApplicationPolicy(m, 'codex', s.applicationPolicy),
        supervisor,
      );
}
...
export function supportsOwnedCodexVersion(text: string): boolean {
  ...
  return (
    version?.[1] !== undefined &&
    compareSemver(version[1], '0.151.0') >= 0 &&
...
export function preflightOwnedCodex(m: MachineConfig, flags: readonly string[]): void {
  ...
  if (!m.codexBin) throw new Error('codexBin is not configured');
  const versionResult = Bun.spawnSync([m.codexBin, '--version'], {
    stdout: 'pipe',
    stderr: 'pipe',
    timeout: 5_000,
  });
  ...
  for (const args of [
    ...
    const result = Bun.spawnSync([m.codexBin, ...args], {
      stdout: 'pipe',
      stderr: 'pipe',
      timeout: 5_000,
    });
    ...
    if (
      result.exitCode !== 0 ||
      !help.includes('unix://PATH') ||
      (args[0] === 'resume' && !help.includes('--remote'))
    ) {
      throw new Error(
        'The configured Codex binary must support App Server Unix sockets and native resume --remote',
      );
    }
  }
}
