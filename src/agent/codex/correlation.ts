...
function hasExactForkMarker(path: string, marker: string): boolean {
  ...
  for (const line of readTailLines(path, 256, 16 * 1024 * 1024)) {
    try {
      ...
      if (
        parsed.success &&
        parsed.data.payload.content.some((item) => item.text.includes(expected))
      )
        return true;
    } catch {}
  }
...
export function rolloutIdsForMarker(m: MachineConfig, marker: string): string[] {
  ...
  for (const path of new Glob('**/rollout-*.jsonl').scanSync({ cwd: root, absolute: true })) {
    ...
    if (
      !readHeadLines(path, 64 * 1024)
        .join('\n')
        .includes(markerField)
    )
      continue;
...
export function forkRolloutIdsForMarker(
  m: MachineConfig,
  marker: string,
