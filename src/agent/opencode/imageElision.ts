...
export class InlineImageElider {
  ...
  feed(byte: number, emit: (byte: number) => void): void {
    if (this.eliding) {
      if (byte === 34) { this.eliding = false; this.quoted = false; this.closedToken = ""; emit(byte); return; }
  ...
  private ordinary(byte: number, emit: (byte: number) => void): void {
    emit(byte);
    if (this.quoted) {
      if (this.escaped) { this.escaped = false; this.token = "!"; return; }
      if (byte === 92) { this.escaped = true; this.token = "!"; return; }
      if (byte === 34) { this.quoted = false; this.closedToken = this.token; return; }
      this.token = this.token.length < 8 ? this.token + String.fromCharCode(byte) : "!";
      return;
    }
    if (byte === 34) {
      this.quoted = true; this.token = "";
      if (this.valueKey === "url") this.prefix = [];
      this.valueKey = "";
    } else if (byte === 58) {
      this.valueKey = this.closedToken; this.closedToken = "";
  ...
  finish(emit: (byte: number) => void): void {
    ...
    if (this.prefix !== null) { for (const byte of this.prefix) emit(byte); this.prefix = null; }
  }
}
