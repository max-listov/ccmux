...
export class OpenCodeProjection {
  ...
  constructor(
    ...
    if (session.nativeSession?.runtime !== 'opencode')
      throw new Error('OpenCode continuation is missing');
    ...
    this.value = {
      ...
      ...(session.applicationPolicy === undefined
        ? {}
        : { applicationPolicy: { policy: session.applicationPolicy, state: 'desired' } }),
    };
  }
  ...
  policyEvidence(evidence: ApplicationPolicyEvidence): void {
    if (this.value.applicationPolicy?.policy.digest !== evidence.policy.digest)
      throw new Error('Native policy identity changed');
    this.value.applicationPolicy = ApplicationPolicyEvidenceSchema.parse(evidence);
  }
  private own(id: string): boolean {
    return id === this.value.nativeSession?.id;
  }
  ...
  append(item: Item): void {
    ...
    this.value.nativeItems.push({
      ...item,
      sequence: this.value.nativeSequence,
      at: new Date().toISOString(),
    });
    while (
      this.value.nativeItems.length > 128 ||
      Buffer.byteLength(JSON.stringify(this.value)) > 112 * 1024
    ) {
      if (this.value.nativeItems.length === 0)
        throw new Error('Native request projection exceeded its byte budget');
  ...
  message(message: OpenCodeMessage, complete = true): void {
    ...
    if (message.role === 'user') {
      if (
        ...
        message.time.created < Date.parse(this.value.turn.startedAt)
      )
        return;
    ...
    if (this.parents.size >= 256 && !this.parents.has(message.id))
    ...
    if (message.summary === true) {
      ...
      while (this.privateMessages.size > 256)
        this.privateMessages.delete(this.privateMessages.values().next().value ?? '');
    }
    if (message.providerID && message.modelID)
      this.value.nativeSelection = {
        model: { provider: message.providerID, model: message.modelID },
        options: null,
        source: 'assistant',
        turnId: message.parentID,
      };
  ...
  part(part: OpenCodePart): void {
    ...
    if (part.type === 'text' || part.type === 'reasoning') {
      this.append({
        ...ItemDefaults,
        kind: part.messageID === turnId ? 'user' : part.type === 'text' ? 'assistant' : 'reasoning',
        stage: part.time?.end === undefined ? 'updated' : 'completed',
        nativeId: part.id,
        turnId,
        text: part.text?.slice(-8_192) ?? null,
      });
    } else if (part.type === 'tool' && part.state && part.tool) {
      if (part.state.status === 'error')
        this.report?.({ partId: part.id, error: part.state.error });
      this.append({
        ...ItemDefaults,
        kind: 'tool',
        stage: ['completed', 'error'].includes(part.state.status) ? 'completed' : 'started',
        nativeId: part.callID ?? part.id,
        turnId,
        status: part.state.status,
        tool: part.tool.slice(0, 128),
  ...
  private delta(raw: unknown): void {
    ...
    if (
      ...
      delta.field !== 'text' ||
      this.privateMessages.has(delta.messageID) ||
      this.parents.get(delta.messageID) !== this.value.turn?.id
    )
      return;
    const prior = this.value.nativeItems.findLast(
      (item) => item.nativeId === delta.partID && item.turnId === this.value.turn?.id,
    );
  ...
  resolve(requestId: string): void {
    ...
    if (!request) return;
    this.value.pendingRequests = this.value.pendingRequests.filter(
