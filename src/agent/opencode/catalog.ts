...
const AgentsSchema = z.array(z.object({ name: z.string().min(1).max(128),
  mode: z.enum(["subagent", "primary", "all"]), hidden: z.boolean().nullish() })).max(128);
const PreparedCatalogSchema = z.object({ registrationGeneration: z.uuid(), models: ModelsSchema,
  agents: z.array(z.string().min(1).max(128)).max(128) }).strict();
...
export async function nativeOpenCodeModels(client: OpenCodeClient, signal: AbortSignal): Promise<ControlModel[]> {
  ...
  return ModelsSchema.parse(catalog.providers.flatMap(provider => Object.values(provider.models).map(model => ({
...
export async function prepareOpenCodeCatalog(m: MachineConfig, s: Session, client: OpenCodeClient, signal: AbortSignal): Promise<void> {
  ...
  const agents = AgentsSchema.parse((await client.app.agents(undefined, { signal })).data)
    .filter(agent => agent.mode !== "subagent" && !agent.hidden).map(agent => agent.name).sort();
  const bytes = JSON.stringify(PreparedCatalogSchema.parse({ registrationGeneration: s.registrationGeneration, models, agents }));
...
export function preparedOpenCodeChoices(m: MachineConfig, s: Session) {
  ...
  if (value === null || value.registrationGeneration !== s.registrationGeneration || readManagedRuntimeStatus(m, s).status !== "live")
