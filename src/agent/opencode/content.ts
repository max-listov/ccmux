...
import { openCodeToolObservation } from './toolObservation.ts';
...
export class OpenCodeContentObserver {
  ...
  event(raw: unknown): void {
    ...
    if (decoded.type === 'message.updated') this.message(decoded.message);
    ...
    else if (decoded.type === 'message.part.delta') {
      const delta = decoded.delta;
      const turn = this.parents.get(delta.messageID);
      if (
      ...
      if (request.sessionID === this.nativeId && id !== undefined)
        this.buffer.lifecycle(
          ...
          null,
          id,
          decoded.name.endsWith('asked') ? 'requested' : 'resolved',
        );
    }
  }
}
