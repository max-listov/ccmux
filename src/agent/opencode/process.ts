  ...
  await withDirectoryLock(join(root, "owner.lock"), async () => {
    ...
    try {
      const child = await startOpenCodeServer(m, initial, abort.signal,
        { name: initial.name, credential: rotateChatCredential(m, initial) });
      server = child;
      void child.child.exited.then(() => abort.abort());
      let session = await admitOpenCode(m, initial, child, promote !== undefined,
      ...
      while (!abort.signal.aborted) {
        ...
        if (!current || current.uuid !== session.uuid || current.archived || current.agent !== "opencode" ||
            current.registrationGeneration !== session.registrationGeneration || current.nativeSession?.id !== session.nativeSession?.id)
        ...
        try {
          ...
          log.warn({ msg: "native observer unavailable", name: session.name, runtime: "opencode" });
          await connection?.close("disconnected"); connection = null;
      ...
      log.error({ msg: "managed native runtime failed", name: initial.name, runtime: "opencode" });
      if (admitted && server?.child.exitCode !== null) throw new ManagedRuntimeExit("OpenCode provider exited");
