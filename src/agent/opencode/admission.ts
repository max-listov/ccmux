...
const AdmissionSchema = z
  .object({ generation: z.uuid(), nativeId: z.string().nullable() })
  .strict();

/** Persist intent before POST. An uncertain POST is reconciled, never repeated. */
export async function admitOpenCode(
  m: MachineConfig,
  initial: Session,
  server: Pick<OpenCodeServer, 'client' | 'version'>,
  fresh: boolean,
  signal: AbortSignal,
): Promise<Session> {
  if (initial.applicationPolicy !== undefined) {
    const policy = verifyApplicationPolicy(m, 'opencode', initial.applicationPolicy);
    selectOpenCodePolicyAgent(
      policy,
      (await server.client.app.agents({ directory: initial.dir }, { signal })).data,
    );
  }
  ...
  if (prior !== null && prior.generation !== generation)
    throw new Error('Native admission generation changed');
  ...
  if (nativeId === null && fork !== null) {
    const native = await admitNativeFork(
      m,
      initial,
      {
        ...
        resume: async (sessionID, nativeSignal) =>
          OpenCodeSessionSchema.parse(
            (await server.client.session.get({ sessionID }, { signal: nativeSignal })).data,
          ),
      },
      signal,
    );
  ...
  if (fork !== null && nativeId !== null) {
    ...
    await server.client.session.update(
      {
        sessionID: nativeId,
        title: `CCMux ${generation}`,
        metadata: { ccmuxRegistration: generation },
      },
      { signal },
    );
    ...
    await server.client.v2.session.switchModel(
      {
        sessionID: nativeId,
        model: {
          providerID: selected.model.provider,
          id: selected.model.model,
    ...
    if (selected.agent !== undefined)
      await server.client.v2.session.switchAgent(
        { sessionID: nativeId, agent: selected.agent },
        { signal },
      );
  }
  ...
  if (nativeId === null && prior !== null) {
    const candidates = z
      .array(OpenCodeSessionSchema)
      .max(128)
      .parse(
        (
          await server.client.session.list(
            { directory: initial.dir, search: generation, limit: 128 },
            { signal },
          )
        ).data,
      );
    const matching = candidates.filter(
      (session) => session.metadata?.ccmuxRegistration === generation,
    );
    if (matching.length !== 1)
      throw new Error('Native admission cannot be reconciled unambiguously');
    nativeId = matching[0]?.id ?? null;
  }
  if (nativeId === null) {
    await atomicWrite(path, JSON.stringify({ generation, nativeId: null }), 0o600);
    const created = OpenCodeSessionSchema.parse(
      (
        await server.client.session.create(
          {
            ...
            ...(initial.modelSelection === undefined
              ...
              : {
                  model: {
                    id: initial.modelSelection.model,
                    providerID: initial.modelSelection.provider,
                  },
                }),
          },
          { signal },
  ...
  const native = OpenCodeSessionSchema.parse(
    (await server.client.session.get({ sessionID: nativeId }, { signal })).data,
  );
  if (
    native.id !== nativeId ||
    realpathSync(native.directory) !== realpathSync(initial.dir) ||
    native.metadata?.ccmuxRegistration !== generation
  )
    throw new Error('Native continuation identity or workspace mismatch');
  const retained = readSelection(m, initial);
  if (
    fresh &&
    retained === null &&
    initial.modelSelection &&
    (native.model?.id !== initial.modelSelection.model ||
      native.model.providerID !== initial.modelSelection.provider)
  )
    throw new Error('Native admission changed the selected provider or model');
  const session = SessionSchema.parse({
    ...initial,
    nativeSession: { runtime: 'opencode', id: native.id, version: server.version },
  });
  if (retained === null) {
    if (native.model === undefined)
      throw new Error('Native admission did not report its selected model');
    const source = fork?.source.selection;
    await seedNativeSelection(m, session, {
      runtime: 'opencode',
      model: { provider: native.model.providerID, model: native.model.id },
      ...(source?.runtime === 'opencode' && source.agent !== undefined
        ? { agent: source.agent }
        : {}),
      ...(source?.runtime === 'opencode' && source.variant !== undefined
