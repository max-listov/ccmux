...
const MessageSchema = z.object({
  ...
  parts: z.array(OpenCodePartSchema).max(256),
});
...
export class OpenCodeConnection {
  ...
  constructor(
    ...
    this.projection = new OpenCodeProjection(m, session, server.child.pid, (error) => {
      this.diagnostic = this.diagnostic.then(() =>
        recordRuntimeDiagnostic(m, session.name, 'native-outcome', error),
      );
    });
    ...
    this.contentObserver = new OpenCodeContentObserver(
      this.content.buffer,
      session.nativeSession?.id ?? '',
    );
    this.contextPump = new NativeContextPump((error) => {
      this.diagnostic = this.diagnostic.then(() =>
        recordRuntimeDiagnostic(m, session.name, 'native-context', error),
      );
    });
  }
  async open(signal: AbortSignal): Promise<void> {
      ...
      try {
        for await (const event of stream) {
          if (this.admitting) {
            ...
            if (this.buffered.length >= 128 || this.bufferedBytes > 512 * 1024)
              throw new Error('Native admission event window exceeded');
            this.buffered.push(event);
          } else {
            this.projection.event(event);
            this.contentObserver.event(event);
            this.content.publish();
            this.queuePublish();
            if (
              event.type === 'session.compacted' &&
              event.properties.sessionID === this.session.nativeSession?.id
            ) {
    ...
    const history = HistorySchema.parse(
      (await this.server.client.session.messages({ sessionID, limit: 1 }, { signal: readSignal }))
        .data,
    );
    ...
    if (last?.info.role === 'assistant' && last.info.parentID) {
      const parent = MessageSchema.parse(
        (
          await this.server.client.session.message(
            { sessionID, messageID: last.info.parentID },
            { signal: readSignal },
          )
        ).data,
      );
      if (
        ...
        parent.info.id !== last.info.parentID ||
    ...
    for (const message of history) {
      ...
      for (const part of message.parts) {
        this.projection.part(part);
        this.contentObserver.part(part);
      }
      this.projection.message(message.info);
    }
    for (const request of z
      .array(z.unknown())
      .max(128)
      .parse((await this.server.client.permission.list(undefined, { signal: readSignal })).data))
      this.projection.permission(request);
    for (const request of z
      .array(z.unknown())
      .max(128)
      .parse((await this.server.client.question.list(undefined, { signal: readSignal })).data))
      this.projection.question(request);
  ...
  private async refresh(signal: AbortSignal): Promise<void> {
    ...
    const statuses = z
      .record(z.string(), OpenCodeStatusSchema)
      .parse((await this.server.client.session.status(undefined, { signal })).data);
    this.projection.status(
      statuses[this.session.nativeSession?.id ?? ''] ?? { type: 'idle' },
      revision,
    );
    await this.publish();
  }
  async tick(signal: AbortSignal): Promise<void> {
    ...
    this.contextPump.start(AbortSignal.any([signal, this.abort.signal]), async (contextSignal) => {
      const api = openCodeContextApi(this.m, this.session, this.server.client),
        generation = this.projection.snapshot().generation;
      const completion = this.contextCompletionSeen;
      if (completion !== this.contextCompletionApplied) {
        const marker = await api.compactionMarker(
          AbortSignal.any([contextSignal, AbortSignal.timeout(5_000)]),
        );
        if (marker === null) throw new Error('Completed native context marker is unavailable');
        await observeContextCompletion(this.m, this.session, generation, marker, () =>
