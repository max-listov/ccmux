...
export async function applyOpenCodeInput(
  m: MachineConfig,
  session: Session,
...
async function applyLocked(
  ...
  if (
    ...
    (session.applicationPolicy === undefined ||
      projection.snapshot().applicationPolicy?.state === 'applied')
  )
    return;
  const policy =
    session.applicationPolicy === undefined
  ...
  const agent =
    policy === undefined
      ...
      : selectOpenCodePolicyAgent(
          policy,
          (await client.app.agents({ directory: session.dir }, { signal })).data,
        );
  if (input.phase === 'dispatching' || input.phase === 'uncertain' || input.phase === 'accepted') {
    try {
      const found = z
        .object({ info: OpenCodeMessageSchema.extend({ agent: z.string().optional() }) })
        .parse(
          (await client.session.message({ sessionID, messageID: input.nativeId }, { signal })).data,
        );
      if (
        found.info.id !== input.nativeId ||
        found.info.sessionID !== sessionID ||
        found.info.role !== 'user'
      )
        throw new Error('Native input correlation mismatch');
      if (policy !== undefined && found.info.agent !== agent)
        policyUnavailable(policy.metadata.id, 'native-agent-receipt-mismatch');
      await writeRuntimeInput(m, session, { ...input, phase: 'accepted' });
      if (policy !== undefined)
        projection.policyEvidence(applicationPolicyEvidence(policy, 'applied'));
    } catch (error) {
    ...
    await client.session.promptAsync(
      {
        ...
        parts: [
          ...
          ...images.map((image) => ({
            type: 'file' as const,
            mime: image.reference.mediaType,
