...
export function preflightOpenCode(m: MachineConfig): void {
  ...
  const result = Bun.spawnSync([m.opencodeBin, "--version"], { stdout: "pipe", stderr: "pipe", timeout: 5_000 });
  const version = result.stdout.toString().trim();
  if (result.exitCode !== 0 || !/^\d+\.\d+\.\d+$/.test(version) || compareSemver(version, "1.18.20") < 0)
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.listen(0, "127.0.0.1", () => {
      ...
      socket.close((error) => {
        ...
        else if (address === null || typeof address === "string") reject(new Error("No loopback listener address"));
...
export async function startOpenCodeServer(m: MachineConfig, session: Pick<Session, "dir" | "envFile">, signal: AbortSignal,
  ...
  const child = Bun.spawn([m.opencodeBin, "serve", "--hostname", "127.0.0.1", "--port", String(port)], {
    cwd: session.dir, detached: true, stdin: "ignore", stdout: "ignore", stderr: "pipe",
  ...
  const client = createOpencodeClient({ baseUrl: `http://127.0.0.1:${port}`, directory: session.dir,
    headers: { Authorization: `Basic ${Buffer.from(`opencode:${password}`).toString("base64")}` },
    fetch: boundedOpenCodeFetch, throwOnError: true });
  let stopped = false;
  const close = async () => { if (!stopped) { stopped = true; await stopOwnedChildGroup(child); await stderr.closed; } };
  try {
    ...
    while (Date.now() < deadline) {
      ...
      try {
        const health = HealthSchema.parse((await client.global.health({ signal: AbortSignal.any([signal, AbortSignal.timeout(500)]) })).data);
    ...
    await close();
    await recordRuntimeDiagnostic(m, identity?.name ?? null, "server-readiness", error, stderr.text());
