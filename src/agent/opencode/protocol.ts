...
export const OpenCodeSessionSchema = z.object({
  id: Id.regex(/^ses_/), directory: z.string(), version: z.string(),
...
export const OpenCodeMessageSchema = z.object({
  ...
  tokens: z.object({ total: z.number().optional(), input: z.number(), output: z.number(), reasoning: z.number(),
...
export const OpenCodePartSchema = z.object({
  ...
  state: z.object({ status: z.enum(["pending", "running", "completed", "error"]),
    input: z.record(z.string(), z.unknown()).optional(), output: z.string().optional(), error: z.string().optional() }).optional(),
});
export type OpenCodePart = z.infer<typeof OpenCodePartSchema>;
export const OpenCodeDeltaSchema = z.object({ sessionID: Id, messageID: Id, partID: Id, field: z.string(), delta: z.string() });
export const OpenCodePermissionSchema = z.object({
  id: Id, sessionID: Id, permission: z.string().max(256),
  tool: z.object({ messageID: Id, callID: Id }).optional(),
});
export const OpenCodeQuestionSchema = z.object({
  id: Id, sessionID: Id,
  questions: z.array(z.object({ header: z.string().max(256), question: z.string().max(2_048),
    custom: z.boolean().optional(), multiple: z.boolean().optional(),
    options: z.array(z.object({ label: z.string().max(256), description: z.string().max(1_024) })).max(32) })).min(1).max(3),
  tool: z.object({ messageID: Id, callID: Id }).optional(),
});
...
export function openCodeTerminal(message: OpenCodeMessage): "completed" | "failed" | "interrupted" | null {
  if (message.role !== "assistant") return null;
  if (message.error !== undefined) return message.error.name === "MessageAbortedError" ? "interrupted" : "failed";
  if (message.time.completed === undefined || !message.finish || ["tool-calls", "unknown"].includes(message.finish)) return null;
  return "completed";
}
