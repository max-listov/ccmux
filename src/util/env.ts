...
function resolveSelfArgv(): string[] {
  ...
  // Running under bun. This module is either src/util/env.ts (dev → the entry is src/cli.ts, one
  // level up) or it's been bundled into the single prod file (→ that file IS the entry to re-exec).
 ...
 * `agent/launch/sessionEnv.ts` subtracts those names again when building the agent's environment: the
...
export function withNoEnvFile(argv: readonly string[]): readonly string[] {
  ...
  // purpose: `agent/launch/sessionEnv.ts` is what holds the guarantee in that shape.
