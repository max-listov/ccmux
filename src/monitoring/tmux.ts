...
let execCount = 0;
let childCpuUs = 0;
export function observationExecCount(): number { return execCount; }
export function observationChildCpuUs(): number { return childCpuUs; }
...
async function invoke(argv: string[]): Promise<{ code: number; stdout: string; stderr: string }> {
  ...
  async function read(stream: ReadableStream<Uint8Array>): Promise<string> {
    ...
    try {
      for (;;) {
        ...
        if (done) break;
        length += value.length;
        if (length > MAX_OUTPUT) { proc.kill("SIGKILL"); await reader.cancel(); throw new Error("observation output limit"); }
        chunks.push(value);
      }
  ...
  try {
    ...
    clearTimeout(timeout); proc.kill("SIGKILL"); await proc.exited;
...
export async function observedSessionInventory(m: MachineConfig): Promise<Map<string, number>> {
  ...
  const result = await invoke(tmuxArgv(m, "list-sessions", "-F", "#{session_name} #{session_created}"));
  // A missing tmux server is positive evidence of no running panes, not an IO failure.
  if (result.code !== 0 && !/no server running|no sessions|error connecting .*No such file/.test(result.stderr)) {
...
export async function observedPane(m: MachineConfig, name: string): Promise<string | null> {
  const result = await invoke(tmuxArgv(m, "capture-pane", "-t", paneTarget(name), "-p", "-S", "-40"));
