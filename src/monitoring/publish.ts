...
export class MonitoringPublisher {
  ...
  async publish(m: MachineConfig): Promise<MonitoringSnapshot> {
    ...
    const snapshot = MonitoringSnapshotSchema.parse({
      ...
      rcPrefix: m.rcPrefix,
      scope: 'managed',
      observedAt: new Date(this.started).toISOString(),
      generatedAt: new Date(now).toISOString(),
      refreshDurationMs: now - this.started,
      maxAgeMs: STATUS_MAX_AGE_MS,
      limits: { items: STATUS_MAX_ITEMS, bytes: STATUS_MAX_BYTES },
  ...
  stop(): void {
    if (
      this.machine !== null &&
      readMonitoringStatus(this.machine).snapshot?.generation === this.generation
    ) {
