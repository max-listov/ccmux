...
export function projectMonitoringRow(
  ...
  const evidence =
    scan === null
      ...
      : turnState({
          ...
          endedOnAssistantText: assistantEndedCurrentTurn(
            lastTranscriptMessage(s, m),
            activity,
            claimed,
          ),
  ...
  const state: MonitoringRow['state'] = blocked
    ? 'blocked'
    : !running
      ? 'stopped'
      : native !== null
        ? native.read.status !== 'live' || native.read.snapshot?.state === 'unknown'
          ? 'unknown'
          : native.atPrompt !== null
            ? 'prompt'
            : native.state
        : scan?.atPrompt !== null && scan?.atPrompt !== undefined
          ? 'prompt'
          : scan === null || (!scan.ready && scan.state === 'indeterminate')
            ? 'unknown'
  ...
  return {
    ...
    turnStartedAt:
      native !== null
        ? native.turnStartedAt
        : state === 'working' && claimed !== null
