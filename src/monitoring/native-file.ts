...
export class MonitoringFileError extends Error {
  constructor(readonly reason: NonNullable<MonitoringRead["reason"]>) { super(reason); }
}
...
export async function readBoundedFile(path: string, limit: number): Promise<string> {
  const file = await open(path, constants.O_RDONLY | constants.O_NONBLOCK | constants.O_NOFOLLOW).catch((error: unknown) => {
    throw new MonitoringFileError(error instanceof Error && "code" in error && error.code === "ENOENT" ? "missing" : "read-failed");
  });
  try {
    const stat = await file.stat();
    if (!stat.isFile()) throw new MonitoringFileError("invalid");
    if (stat.uid !== process.getuid?.() || (stat.mode & 0o022) !== 0) throw new MonitoringFileError("unauthorized");
