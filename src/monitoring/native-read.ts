...
import { STATUS_MAX_BYTES, type MonitoringRead } from "./schema.ts";
...
async function readConfig(path: string): Promise<string> {
  try { return await readBoundedFile(path, MONITORING_CONFIG_MAX_BYTES); }
  catch (error) {
...
async function readOnce(): Promise<MonitoringRead> {
  try {
    ...
    if (configPath !== machineConfigPath() || config !== after || next.stateDir !== location.stateDir || next.rcPrefix !== location.rcPrefix) {
...
export function readMonitoringStatus(options: MonitoringReadOptions = {}): Promise<MonitoringRead> {
  ...
  return new Promise((resolve) => {
    ...
    const deliver = (result: MonitoringRead) => {
      ...
      const delivered = result.snapshot ? validateLiveness(structuredClone(result.snapshot)) : result;
