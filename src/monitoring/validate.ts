import { MonitoringSnapshotSchema, STATUS_MAX_AGE_MS, type MonitoringRead, type MonitoringSnapshot } from "./schema.ts";
...
export function validateSnapshot(bytes: string, rcPrefix: string, now = Date.now()): MonitoringRead {
  try {
