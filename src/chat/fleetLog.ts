...
export const LogRowSchema = z.object({
  ...
  // sent-restart — we handed off work via `restart --then` to another machine (outbox)
  kind: z.enum(["chat", "sent", "sent-restart"]),
  from: z.string(),
  to: z.string(),
...
export function localRows(machine: string, ledger: ChatMessage[], outbox: Outbound[], settled: ReadonlySet<string> = new Set()): LogRow[] {
  ...
  for (const msg of ledger) {
    rows.push({
      ...
      // reply address can be read off directly — the whole point of carrying `fromMachine`.
      from: msg.fromMachine === null ? msg.from : `${msg.fromMachine}:${msg.from}`,
      to: msg.to,
      task: msg.task,
      body: msg.body,
      note: "",
  ...
  for (const o of outbox) {
    rows.push({
      machine,
      ts: o.ts,
      kind: o.kind === "restart-then" ? "sent-restart" : "sent",
      from: o.from,
...
const ARROW: Record<LogRow["kind"], string> = { chat: "→", sent: "⇢", "sent-restart": "⇢" };
...
export function fmtRow(r: LogRow, machineWidth = 0): string {
  ...
  const verb = r.kind === "sent-restart" ? " restart --then" : "";
  const note = r.note === "" ? "" : `   [${r.note}]`;
  return `[${t}] ${col}${r.from} ${ARROW[r.kind]} ${r.to}${verb}${task}: ${r.body}${note}`;
}
