...
async function deliverLocked(m: MachineConfig, s: Session, ledger: readonly LedgerSlot[],
  ...
  if (read.status !== "live" || read.snapshot === null) { await hold("native runtime is unavailable"); return 0; }
  const current = loadSessions(m).find(row => row.name === s.name);
  if (current?.uuid !== s.uuid || current.registrationGeneration !== s.registrationGeneration || current.nativeSession?.id !== s.nativeSession?.id)
  ...
  if (pickup !== undefined) {
    if (input?.messageId !== pickup.messageId) {
      const previousComplete = input === null || (input.phase === "accepted" &&
      ...
      await hold("native dispatch receipt is unavailable"); return 0;
    }
    if (input.phase !== "accepted") {
      if (input.phase === "uncertain") await hold("native acceptance is indeterminate; automatic resubmission is blocked");
      return 0;
    }
    pickup.native = { phase: "accepted", turnId: input.nativeId };
    const terminal = read.snapshot.turn?.id === input.nativeId && read.snapshot.turn.status !== "inProgress";
