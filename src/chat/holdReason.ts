...
export function holdReason(msg: ChatMessage, ctx: HoldContext): HoldReason {
  ...
  if (ctx.isOwner === true) {
    return {
      kind: 'owner',
      text: 'addressed to you — there is no pane to deliver to; it surfaces via the Telegram mirror if configured',
    };
  }
  if (ctx.recipient === undefined) {
    return {
      kind: 'recipient-unknown',
      text: `no exact session '${targetLabel(msg.to)}' on this machine — it may belong to another fleet machine or have been replaced`,
    };
  }
  if (!ctx.chatEnabled) {
    return {
      kind: 'chat-off',
      text: `recipient has chat disabled (ccmux chat on ${sessionName}, then restart it)`,
    };
  }
  ...
  if (ctx.chatDeliverable === false) {
    return {
      ...
      text: `recipient runs ${ctx.recipient.agent}, which cannot receive chat — this will never be delivered`,
    };
  }
  if (!ctx.running)
    return {
      kind: 'recipient-stopped',
      text: `recipient is not running (ccmux start ${sessionName})`,
    };
  if (msg.notBefore !== null) {
    ...
    if (Number.isFinite(due) && ctx.nowMs < due) {
      return {
        kind: 'not-due',
        text: `scheduled — not before ${msg.notBefore} (${Math.ceil((due - ctx.nowMs) / 1000)}s)`,
      };
    }
  }
  ...
  if (msg.defer)
    return {
      ...
      text: "deferred — delivered at the recipient's next turn boundary; no live reason on record (the daemon has not picked it up yet, or is not running)",
    };
  return {
    kind: 'pending',
    text: 'queued — the daemon delivers it (if nothing appears, check the daemon is running: ccmux doctor)',
  };
}
