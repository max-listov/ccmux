...
export const contentPath = (
  ...
  session: Pick<Session, 'name' | 'uuid'>,
...
export class ContentWriter {
  ...
  constructor(m: MachineConfig, session: Session) {
    this.path = contentPath(m, session);
  ...
  flush(): Promise<void> {
    ...
    if (Buffer.byteLength(bytes) > CONTENT_FILE_MAX_BYTES)
      throw new Error('Native content exceeds its byte budget');
    this.writing = atomicWrite(this.path, bytes, 0o600)
      .then(() => contentNotice(`${this.path}.notice`, true))
      .finally(() => {
        this.writing = null;
