...
let activeMachine = MachineConfigSchema.parse({
  ...base,
...
async function waitFor(label: string, predicate: () => boolean | Promise<boolean>, timeoutMs = 90_000): Promise<void> {
...
async function restartDaemon(): Promise<void> {
  ...
  await waitFor("old daemon stopped", async () => !(await hasSession(activeMachine, "probe-daemon")), 10_000);
  await Bun.sleep(300);
  await newSession(activeMachine, "probe-daemon", root,
    [process.execPath, "--no-env-file", cli, "daemon"], environment);
  await waitFor("control service restarted", async () => {
    try {
      const response = await fetch("http://ccmux.local/control/sessions", { unix: controlSocket(activeMachine) });
      return response.ok;
    } catch { return false; }
  });
}
...
async function requestAndAnswer(target: Awaited<ReturnType<typeof remote.create>>["target"], marker: string) {
  const before = await remote.native({ target, cursor: null });
  await remote.message({ target, messageId: crypto.randomUUID(), defer: false, notBefore: null, task: null,
  ...
  let frame = await remote.native({ target, cursor: { generation: before.generation, sequence: before.sequence } });
  ...
  while (!frame.pending.some((request) => request.kind === "input")) {
    ...
    await Bun.sleep(150);
    frame = await remote.native({ target, cursor: { generation: before.generation, sequence: before.sequence } });
  }
  ...
  const answers = Object.fromEntries(input.questions.map((question) => [question.id,
    [question.options?.[0]?.label ?? "Red"]]));
  await expectCode(() => remote.respond({ target, operationId: crypto.randomUUID(), generation: crypto.randomUUID(),
    requestId: input.requestId, kind: "input", decision: null, answers }), "STALE_REQUEST");
  await expectCode(() => remote.respond({ target, operationId: crypto.randomUUID(), generation: frame.generation,
    requestId: `${input.requestId}-wrong`, kind: "input", decision: null, answers }), "STALE_REQUEST");
  await expectCode(() => remote.respond({ target, operationId: crypto.randomUUID(), generation: frame.generation,
    requestId: input.requestId, kind: "approval", decision: "accept", answers: null }), "STALE_REQUEST");
  await expectCode(() => remote.respond({ target, operationId: crypto.randomUUID(), generation: frame.generation,
    requestId: input.requestId, kind: "input", decision: null, answers: { wrong: ["Red"] } }), "STALE_REQUEST");
  const operationId = crypto.randomUUID();
  const receipt = await remote.respond({ target, operationId, generation: frame.generation,
  ...
  check((await remote.respond({ target, operationId, generation: frame.generation,
    requestId: input.requestId, kind: "input", decision: null, answers })).outcome === "submitted",
  ...
  await expectCode(() => remote.respond({ target, operationId, generation: frame.generation,
    requestId: input.requestId, kind: "input", decision: null, answers: changedAnswers }), "IDEMPOTENCY_CONFLICT");
  await waitFor("answered turn terminal success", async () => {
...
try {
  ...
  check(created.launchRecipe?.collaborationMode === "plan" && created.launchRecipe.capabilities.includes("input-requests"),
  ...
  check(session !== undefined && session.uuid === target.threadId, "managed registry identity missing");
  await waitFor("managed policy session idle", () => readOwnedCodexStatus(activeMachine, session).snapshot?.state === "idle");
  ...
  await waitFor("same identity after provider restart", () => {
    ...
    return next !== null && next.providerPid !== beforeRestart.providerPid &&
      next.generation !== beforeRestart.generation && next.state === "idle";
  });
  check(loadSessions(activeMachine).find((item) => item.name === sessionName)?.uuid === target.threadId,
    "provider restart changed native identity");

  await restartDaemon();
  const retried = await remote.create(createInput);
  check(retried.duplicate && retried.target.threadId === target.threadId &&
    retried.launchRecipe?.digest === created.launchRecipe.digest && retried.launchRecipe.collaborationMode === "plan",
  ...
  check(row.launchRecipe?.digest === created.launchRecipe.digest && row.launchRecipe.collaborationMode === "plan",
    "status projection changed collaboration policy metadata");
  const archive = await remote.archive({ target });
  archived = archive.archived;
  console.log(JSON.stringify({ ok: true, policy: row.launchRecipe, sameIdentity: true,
    first, second, providerRestart: true, daemonRestart: true, staleRefusals: true,
    exactResponse: true, archived }));
} finally {
