...
function report(phase: string, evidence: unknown) {
...
if (root === undefined) {
  ...
  await atomicWrite(
    join(probe, 'workspace', 'opencode.json'),
  ...
  await atomicWrite(
    ...
    JSON.stringify({
      ...machine,
      rcPrefix: 'probe',
      stateDir: join(probe, 'state'),
      tmuxSocket: `ccmux-opencode-${crypto.randomUUID().slice(0, 8)}`,
      fleet: {},
      launchRecipes: {},
      extraFlags: [],
      remoteControl: false,
  ...
  const env: Record<string, string | undefined> = {
    ...process.env,
    CCMUX_CONFIG: config,
    CCMUX_STATE_DIR: join(probe, 'state'),
    CCMUX_CACHE_DIR: join(probe, 'cache'),
    CCMUX_DATA_DIR: join(probe, 'data'),
...
check(
  m.stateDir === join(root, 'state') && !m.telegram && !Object.keys(m.fleet ?? {}).length,
...
try {
  ...
  check(
    catalog.runtimes.some((row) => row.runtime === 'custom' && row.availability === 'unavailable'),
    'Custom capability was fabricated',
  );
  const create = {
    runtime: 'opencode',
  ...
  report('created-through-public-service', {
    receipt,
    oneWriter: loadSessions(m).length === 1,
  ...
  check(
    ...
    'No real tool completion',
  );
  check(
    content.some((item) => item.kind === 'terminal' && item.status === 'completed'),
  ...
  check(
    after.every(
      (row, i) =>
        row && row.providerPid === before[i]?.providerPid && row.threadId === before[i]?.threadId,
    ),
  ...
  report('daemon-restart', {
    writersPreserved: true,
  ...
  check(
    loadSessions(m)[0]?.nativeSession?.id === receipt.nativeSession?.id,
    'Restart replaced native identity',
  );
  check(
    readFileSync(join(root, 'workspace', 'effect.txt'), 'utf8').trim() === 'effect',
