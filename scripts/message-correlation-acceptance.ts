...
type Selector = Parameters<Client['messageOperation']>[0];
...
async function terminal(client: Client, selector: Selector) {
  await until('exact message terminal', async () => {
    const result = await client.messageOperation(selector);
  ...
  const frame = await client.native({ target: selector.target });
  check(
...
async function prove(receipt: ControlCreateReceipt) {
  ...
  await p.service.message(first);
  check((await p.service.message(first)).duplicate, 'Lost caller ACK retry was not idempotent');
  ...
  await p.service.message(second);
  check(
    (await p.service.messageOperation(selector(second.messageId))).evidence?.state === 'queued',
  ...
  await external.message(intervening);
  const outside = await terminal(external, selector(intervening.messageId));
  check(
    (await p.service.messageOperation(selector(intervening.messageId))).outcome === 'unavailable',
    'Other sender correlation leaked',
  );
  const queued = await p.service.messageOperation(selector(second.messageId));
  ...
  const firstResult = await reconnected.messageOperation(selector(first.messageId));
  check(
    new Set([firstResult.evidence?.turnId, secondResult.evidence?.turnId, outside.evidence?.turnId])
      .size === 3,
    'Native turns were conflated',
  );
  check((await reconnected.message(second)).duplicate, 'Deferred retry duplicated');
  check(
    JSON.stringify(await reconnected.messageOperation(selector(second.messageId))) ===
      JSON.stringify(secondResult),
    'Retry changed terminal binding',
  );
  const history = await reconnected.history({ target, registrationGeneration, limit: 64 });
...
try {
  ...
  for (const proof of proofs) {
    for (const retained of proof.retained) {
      ...
      check(
        JSON.stringify(
          await p.service.messageOperation({ target, registrationGeneration, messageId }),
    ...
    for (const retained of proof.retained)
      check(
        JSON.stringify(
          await p
            .client('probe-client')
            .messageOperation({ target, registrationGeneration, messageId: retained.messageId }),
        ) === JSON.stringify(retained),
        'Provider restart changed correlation',
      );
    const resumed = { target, messageId: crypto.randomUUID(), body };
    await p.service.message(resumed);
