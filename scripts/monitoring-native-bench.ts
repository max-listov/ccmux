...
import { MONITORING_MAX_READERS, readMonitoringStatus } from '../src/monitoring-reader.ts';
...
const forbidden = (): never => {
  execAttempts++;
  throw new Error('native reader attempted a subprocess');
};
...
async function read() {
  ...
  if (result.status !== 'live' || !result.snapshot)
    throw new Error(`native read: ${result.status}/${result.reason}`);
...
console.log(
  JSON.stringify({
    ...
    producerVersion,
    execAttempts,
    readerCpuMs: (cpu.user + cpu.system) / 1000,
    ...
    maxRssBytes: maxRss,
    retainedSnapshotCacheEntries: 0,
    maxConcurrentCallers: MONITORING_MAX_READERS,
  }),
);
