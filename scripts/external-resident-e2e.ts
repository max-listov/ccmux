...
if (!Bun.argv.includes("--run")) {
  console.error("usage: bun scripts/external-resident-e2e.ts --run [--source] (creates two read-only test threads)");
  process.exit(2);
}
const source = Bun.argv.includes("--source"), machine = loadMachineConfig();
const root = mkdtempSync("/tmp/ccmux-resident-e2e-"), ipc = { ...machine, stateDir: root };
...
const owned = source ? createControlServer(ipc, managed, undefined, () => ipc, external) : undefined;
const client = createControlClient(source ? { socket: controlSocket(ipc) } : {});
const rpc = await connectCodexAppServer(machine);
const threads: string[] = [], turns = new Map<string, string>();
...
async function matches(states: string[]): Promise<void> {
  ...
  while (Date.now() < until) {
    ...
    if (threads.every((id, n) => snapshot.sessions.find((s) => s.identity.threadId === id)?.turnState.state === states[n])) return;
    await Bun.sleep(100);
  }
...
try {
  if (source) {
    await observer.refresh(); timer = setInterval(() => void observer.refresh(), 2000);
  }
  const first = await client.external();
  if (first.status !== "live" || first.version !== VERSION) throw new Error("live installed/candidate version does not match checkout");
    ...
    try {
      for await (const snapshot of stream) {
        ...
        for (const row of currentExternalStatus(snapshot).sessions) {
          ...
          states.add(row.turnState.state); observed.set(row.identity.threadId, states);
        }
      }
    } catch (error) { if (!abort.signal.aborted) throw error; }
  })();
  for (let n = 0; n < 2; n++) {
    const response = Thread.parse(await rpc.request("thread/start", { cwd: root, approvalPolicy: "never", sandbox: "read-only",
  ...
  for (const [index, id] of threads.entries()) {
    const text = index === 0 ? "Run the shell command sleep 20 exactly once, wait, then reply DONE. Do not modify files."
      : "Reply exactly DONE without using tools.";
    const result = Turn.parse(await rpc.request("turn/start", { threadId: id, input: [{ type: "text", text, text_elements: [] }] }));
  ...
  const firstId = threads[0]; if (!firstId) throw new Error("test identity missing");
  ...
  try {
    ...
    if (!threads.every((id) => snapshot.sessions.some((s) => s.identity.threadId === id && s.turnState.state === "idle"))) throw new Error("reconnect lost native idle identity");
  } finally { await reconnect.close(); }
  if (!threads.every((id) => observed.get(id)?.has("working") && observed.get(id)?.has("idle"))) throw new Error("stream missed test transitions");
  if (source && observer.metrics().notifications === 0) throw new Error("native notifications were not delivered");
  console.log(JSON.stringify({ success: true, version: VERSION, mode: source ? "source" : "installed", frames,
    states: threads.map((id, index) => ({ thread: index === 0 ? "test-A" : "test-B", states: [...observed.get(id) ?? []] })),
    ...(source ? { observer: observer.metrics() } : {}) }));
} catch (error) {
  console.error(JSON.stringify({ phase: "failed", error: String(error), observer: observer.metrics(),
    observed: threads.map((id) => [...observed.get(id) ?? []]), testDirectory: root }));
  throw error;
} finally {
  if (timer) clearInterval(timer);
  abort.abort(); await streamTask;
  ...
  for (const threadId of threads) {
    ...
    await rpc.request("thread/archive", { threadId }).catch((error) => {
      cleanupFailed = true; console.error(JSON.stringify({ phase: "archive-failed", threadId, error: String(error), testDirectory: root }));
    });
  }
  rpc.close(); await client.close(); await observer.close(); managed.close();
  if (owned) { await owned.server.shutdown({ gracePeriodMs: 200, forceTimeoutMs: 100 }); await owned.observability.close(); }
  if (!cleanupFailed) rmSync(root, { recursive: true, force: true });
  if (cleanupFailed) throw new Error("test thread archive failed; retain test directory for inspection");
}
