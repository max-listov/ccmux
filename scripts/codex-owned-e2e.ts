...
if (
  ...
  throw new Error('Probe must be isolated, with no Telegram or fleet routes');
...
check(
  snapshot(a)?.state === 'idle' && snapshot(b)?.state === 'idle',
  'Both native sessions must be live and idle before the test',
);
progress(
  'baseline',
  [a, b].map((s) => ({ name: s.name, uuid: s.uuid, snapshot: snapshot(s) })),
);
await Promise.all([
  command(['wait', a.name, '--timeout', '10']),
  command(['wait', b.name, '--timeout', '10']),
]);
...
const peerA = managedPeer(m.rcPrefix, a),
  peerB = managedPeer(m.rcPrefix, b);
const roundTrip = () => {
  const messages = loadLedger(m).filter((msg) => msg?.body.includes(token));
  return (
    messages.some(
      (msg) =>
        msg !== null &&
        chatPrincipalKey(msg.from) === chatPrincipalKey(peerA) &&
        chatTargetKey(msg.to) === chatTargetKey(peerB),
    ) &&
    messages.some(
      (msg) =>
        msg !== null &&
        chatPrincipalKey(msg.from) === chatPrincipalKey(peerB) &&
        chatTargetKey(msg.to) === chatTargetKey(peerA),
    )
  );
};
...
progress(
  'round-trip',
  loadLedger(m)
    .filter((msg) => msg?.body.includes(token))
    .map((msg) => (msg === null ? null : { id: msg.id, from: msg.from, to: msg.to })),
);

const rpc = await connectOwnedCodex(m, a);
try {
  const busyId = await startCodexAppTurn(
    rpc,
    a.uuid,
    crypto.randomUUID(),
    codexTextInput(
      'Run the shell command sleep 15, then reply BUSY_TEST_DONE. This is a timing test. Do not message anyone or do other work.',
    ),
  );
  await until(
    'native working',
    () => snapshot(a)?.turn?.id === busyId && snapshot(a)?.state === 'working',
    15_000,
  );
  await command([
    'msg',
    a.name,
    `Deferred test ${token}: reply DEFERRED_DONE and do not contact anyone.`,
    '--defer',
  ]);
...
const reads = await Promise.all(
  Array.from({ length: 100 }, () =>
    readCodexRuntime({ session: a.name, threadId: a.uuid, timeoutMs: 1000 }),
  ),
);
check(
  reads.every((read) => read.status === 'live' && read.snapshot?.threadId === a.uuid),
  'Resident concurrency returned missing/wrong identity',
);
progress('100-concurrent-native-reads', {
