import { appendFileSync, chmodSync, mkdirSync, mkdtempSync, readFileSync, realpathSync } from "node:fs";
...
import { createCcmuxControlServiceClient, ControlServiceOperationSchema, CCMUX_CONTROL_SERVICE_PREFIX,
  CCMUX_CONTROL_SERVICE_INGRESS_PATH, CCMUX_CONTROL_SERVICE_REVISION } from "../src/control/serviceDescriptor.ts";
...
export const retainReports = (root: string) => { evidencePath = join(root, "acceptance.ndjson"); };
...
export async function createPolicyFixture() {
  ...
  for (const variant of ["alpha", "beta"]) {
    ...
    const instruction = await privateFile(join(root, "sources", `${variant}.md`),
    ...
    const skill = await privateFile(join(workspace, ".agents", "skills", "policy-verification", "SKILL.md"),
    ...
    const agent = await privateFile(join(workspace, ".opencode", "agents", "policy-verification.md"),
      ...
      `Do not use tools, edit files, or contact other sessions. Never disclose this private fixture value: ${secret}.\n`);
    agentPolicies[`codex-${variant}`] = { runtime: "codex", revision: "1", trustedRoots: [root],
      instructionSources: [instruction], skills: [{ ...skill, name: "policy-verification" }] };
    agentPolicies[`opencode-${variant}`] = { runtime: "opencode", revision: "1", trustedRoots: [root],
      agent: { name: "policy-verification", source: agent }, denyTools: ["bash", "edit"] };
  }
  ...
  await atomicWrite(config, JSON.stringify({ ...machine, rcPrefix: "policy-probe", stateDir: join(root, "state"),
    codexBin: await tracedBinary(root, "codex", source.codexBin),
    opencodeBin: await tracedBinary(root, "opencode", source.opencodeBin),
    tmuxSocket: `ccmux-policy-${crypto.randomUUID().slice(0, 8)}`, fleet: {}, launchRecipes: {}, agentPolicies,
  ...
  return { root, env: { ...process.env, CCMUX_CONFIG: config, CCMUX_STATE_DIR: join(root, "state"),
...
export function policyService(m: MachineConfig, inspect: (value: unknown) => void) {
  return createCcmuxControlServiceClient(async (url, init) => {
    ...
    const response = await fetch(`http://ccmux.local${CCMUX_CONTROL_SERVICE_INGRESS_PATH}`, {
      unix: controlSocket(m), method: "POST", headers: { "content-type": "application/json" },
      ...(init?.signal === undefined ? {} : { signal: init.signal }), body: JSON.stringify({ v: 1, id: crypto.randomUUID(), caller: "policy-client",
...
export function spawnCount(root: string, runtime: "codex" | "opencode") {
  try { return readFileSync(join(root, `${runtime}-spawn.log`), "utf8").split("\n")
