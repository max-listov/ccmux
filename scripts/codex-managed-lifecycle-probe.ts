...
try {
  for (const path of [work, codexSessions, stateDir, cacheDir])
    mkdirSync(path, { recursive: true });
  copyFileSync(join(homedir(), '.codex', 'auth.json'), join(codexHome, 'auth.json'));

  if (!codexBin || !claudeBin || !tmuxBin)
    throw new Error('probe requires codex, claude, and tmux in PATH');
  ...
  function registry(): Array<{ name: string; uuid: string; agent: string }> {
    ...
    try {
      return readFileSync(path, 'utf8')
        ...
        .map((line) => SessionSchema.parse(JSON.parse(line)));
    } catch {
      return [];
    }
  }
  ...
  function hasAssistantExact(uuid: string, expected: string): boolean {
    return readSession(rolloutFor(uuid), 'codex').some(
      (message) =>
  ...
  async function listed(name: string) {
    ...
    return ListJsonSchema.parse(JSON.parse(result.stdout)).sessions.find(
  ...
  async function approveTrust(name: string): Promise<void> {
    ...
    await waitFor(`${name} trust or bind`, async () => {
      ...
      if (
        !sent &&
        text.includes('project-local config, hooks') &&
  ...
  if (ready?.agent !== 'codex') throw new Error('agent-a was not promoted as Codex');
  ...
  if (rolloutPaths().length !== initialRolloutCount)
    throw new Error('child death minted a new rollout');
  ...
  if (restarted.code !== 0)
    throw new Error(`restart failed: ${restarted.stderr || restarted.stdout}`);
  await waitFor(
    'single restart',
    async () => (await hasSession('agent-a')) && (await panePid('agent-a')) !== beforeRestartPid,
  );
  ...
  if (restartAll.code !== 0)
    throw new Error(`restart --all failed: ${restartAll.stderr || restartAll.stdout}`);
  await waitFor(
    'restart all',
    async () => (await hasSession('agent-a')) && (await panePid('agent-a')) !== beforeAllPid,
  );
  ...
  await waitFor('daemon-healed writer admission', () =>
    existsSync(join(codexHome, 'thread-writer-locks', `${uuid}.lock`)),
  );
  ...
  await waitFor('active writer refusal', async () =>
    (await pane('contender')).includes('already has an active writer'),
  );
  ...
  if (!sessionB || !sessionC || sessionB.uuid === sessionC.uuid)
    throw new Error('concurrent sessions did not bind distinct UUIDs');
  ...
  if (stoppedForConflict.code !== 0)
    throw new Error(`conflict setup stop failed: ${stoppedForConflict.stderr}`);
  ...
  await waitFor('external owner admitted', async () =>
    (await pane('external-owner')).includes('›'),
  );
  const conflictedStart = await cli('start', 'agent-a');
  if (conflictedStart.code !== 0)
    throw new Error(`managed conflict start command failed: ${conflictedStart.stderr}`);
  await waitFor(
    'managed writer conflict blocked',
    async () => (await listed('agent-a'))?.state === 'blocked',
  );
  const conflictRow = await listed('agent-a');
  if (!conflictRow?.lifecycleError?.includes('active writer'))
    throw new Error('blocked writer reason is not observable');
  ...
  if (resumedAfterConflict.code !== 0)
    throw new Error(`resume after writer release failed: ${resumedAfterConflict.stderr}`);
  ...
  if (stoppedForMissing.code !== 0)
    throw new Error(`missing-history setup stop failed: ${stoppedForMissing.stderr}`);
  renameSync(heldRollout, heldPath);
  try {
    const missingStart = await cli('start', 'agent-b');
    if (missingStart.code !== 0)
      throw new Error(`missing-history start command failed: ${missingStart.stderr}`);
    await waitFor(
      'missing history blocked',
      async () => (await listed('agent-b'))?.state === 'blocked',
    );
    const missingRow = await listed('agent-b');
    if (!missingRow?.lifecycleError?.includes('missing rollout'))
      throw new Error('missing-history reason is not observable');
  } finally {
    renameSync(heldPath, heldRollout);
  }
  const recoveredMissing = await cli('start', 'agent-b');
  if (recoveredMissing.code !== 0)
    throw new Error(`missing-history recovery failed: ${recoveredMissing.stderr}`);
  ...
  if (tmuxBin)
    Bun.spawnSync([tmuxBin, '-L', tmuxSocket, 'kill-server'], {
      stdout: 'ignore',
      stderr: 'ignore',
    });
