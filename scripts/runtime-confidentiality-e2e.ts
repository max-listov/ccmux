...
export async function verifyRuntimeConfidentiality(
  m: MachineConfig,
  session: Session,
  publicFrame: unknown,
  root: string,
  ...
  const version = Bun.spawnSync([process.execPath, '--no-env-file', cli, 'version'], {
    stdout: 'pipe',
    stderr: 'pipe',
  });
  ...
  console.log(
    JSON.stringify({
      phase: 'installed-runtime-confidentiality',
      evidence: {
        version: version.stdout.toString().trim(),
        fixtureReachedProvider: true,
        metadataSafe: true,
        argvSafe: true,
        logsSafe: true,
      },
