...
export async function packageControlServiceClient(directory: string): Promise<{
  ...
  if (!build.success) throw new Error(`Control service client build failed: ${build.logs.join("\n")}`);
  ...
  for (const file of await readdir(join(stage, "types"), { recursive: true })) {
    ...
    await writeFile(
      path,
      content.replace(/from (["'])(\.[^"']+)\.ts\1/g, (_match, quote: string, name: string) =>
        `from ${quote}${name}.js${quote}`,
      ),
    );
  }
  ...
  await writeFile(join(destination, "control-service-client.sha256"), `${hash}  ${basename(artifact)}\n`);
  return { artifact, sha256: hash, bytes: bytes.byteLength };
}

if (import.meta.main) {
  const destination = Bun.argv[2];
  if (!destination) throw new Error("usage: bun scripts/package-control-service.ts <output-directory>");
