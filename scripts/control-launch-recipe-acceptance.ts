...
const definition = {
  ...
  flags: [
    '-c',
    `model_provider="${recipeId}"`,
    '-c',
    `model_providers.${recipeId}.name="Provider A"`,
    '-c',
    `model_providers.${recipeId}.base_url="http://127.0.0.1:9/v1"`,
    '-c',
    `model_providers.${recipeId}.env_key="MODEL_SERVICE_TOKEN"`,
    '-c',
    `model_providers.${recipeId}.wire_api="responses"`,
  ],
...
async function restartDaemon(machine: typeof base): Promise<void> {
  ...
  await waitFor(
    'old daemon tmux session stopped',
    async () => !(await hasSession(machine, 'probe-daemon')),
    10_000,
  );
  await Bun.sleep(300);
  await newSession(
...
let activeMachine = MachineConfigSchema.parse({
  ...base,
  launchRecipes: { [recipeId]: definition },
});
...
try {
  ...
  check(
    ...
    'retry changed managed identity',
  );
  check(
    created.launchRecipe?.id === recipeId && created.launchRecipe.revision === revision,
    'create receipt omitted safe recipe identity',
  );
  check(
    created.launchRecipe.digest === retried.launchRecipe?.digest,
    'retry changed recipe digest',
  );

  const session = loadSessions(activeMachine).find((row) => row.name === sessionName);
  check(
    session !== undefined && session.launchRecipe?.digest === created.launchRecipe.digest,
    'registry did not persist recipe identity',
  );
  await waitFor(
    'recipe native projection',
    () => readOwnedCodexStatus(activeMachine, session).status === 'live',
  );
  ...
  check(
    processArgv.includes('MODEL_SERVICE_TOKEN'),
    'provider argv lost the configured environment key name',
  );
  ...
  await waitFor('recipe restart', () => {
    ...
    return (
      next !== null &&
      next.providerPid !== before.providerPid &&
  ...
  check(
    daemonRetry.launchRecipe?.digest === created.launchRecipe.digest,
  ...
  await expectCode(
    () =>
      remote.create({
        ...
        workspace: root,
        flags: [],
        launchRecipe: { id: 'missing-recipe', revision },
      }),
    'LAUNCH_RECIPE_UNAVAILABLE',
  );
  check(
    loadSessions(activeMachine).length === registryBeforeUnknown &&
      !(await hasSession(activeMachine, unknownName)),
  ...
  activeMachine = MachineConfigSchema.parse({
    ...
    launchRecipes: {
      [recipeId]: {
        ...definition,
        flags: [...definition.flags, '-c', 'model="changed"'],
      },
    },
  });
  await writeMachine(activeMachine);
  ...
  check(
    !readFileSync(join(activeMachine.stateDir, 'ccmux.log'), 'utf8').includes(secret),
  ...
  console.log(
    JSON.stringify({
      ok: true,
      identityHash: hash(`${target.machine}:${target.session}:${target.threadId}`),
      recipeDigest: created.launchRecipe.digest,
      oneWriter: true,
