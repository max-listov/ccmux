...
export async function buildMonitoringReader(directory: string): Promise<void> {
  const result = await Bun.build({
    entrypoints: [join(import.meta.dir, "../src/monitoring-reader.ts")], target: "bun",
  });
  const [artifact] = result.outputs;
  if (!result.success || !artifact) throw new Error(`monitoring reader build failed: ${result.logs.join("\n")}`);
...
if (import.meta.main) {
  ...
  if (!directory) throw new Error("usage: bun scripts/build-monitoring-reader.ts <output-directory>");
