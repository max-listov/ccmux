...
if (m.stateDir !== join(root, "state") || m.telegram || Object.keys(m.fleet ?? {}).length
  || !m.tmuxSocket?.startsWith("ccmux-owned-")) throw new Error("Probe is not isolated");
...
try {
  const first = await client.create({ requestId, name, workspace: root, flags: [
    "--sandbox", "workspace-write", "--ask-for-approval", "on-request", "--no-alt-screen",
  ] });
  const retry = await client.create({ requestId, name, workspace: root, flags: [
    "--sandbox", "workspace-write", "--ask-for-approval", "on-request", "--no-alt-screen",
  ] });
  check(!first.duplicate && retry.duplicate && JSON.stringify(first.target) === JSON.stringify(retry.target),
  ...
  check(rows.length === 1 && rows[0]?.uuid === first.target.threadId, "Create published more than one registry identity");
  const session = rows[0]!;
  const before = readOwnedCodexStatus(m, session).snapshot;
  const history = historyFile(session, m);
  check(before?.providerPid && history && existsSync(history), "Created identity has no live provider writer or history");

  const archived = await client.archive({ target: first.target });
  check(archived.archived && !archived.duplicate && archived.stopped, "First archive did not stop the owned runtime");
  ...
  check(retained?.archived && retained.uuid === session.uuid, "Archive changed or removed the canonical identity");
  check(existsSync(history), "Archive deleted provider history");
  console.log(JSON.stringify({ phase: "control-lifecycle-complete", requestId, target: first.target,
    providerPid: before.providerPid, oneWriter: true, retryDuplicate: retry.duplicate,
    archiveDuplicate: archivedAgain.duplicate, historyRetained: true }));
