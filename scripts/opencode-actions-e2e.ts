...
async function approve(client: Client, target: ManagedPeer): Promise<void> {
  ...
  for (const request of frame.pending.filter((request) => request.kind === 'approval')) {
    const input = {
      ...
      kind: 'approval',
      decision: 'accept',
    } satisfies Parameters<Client['native.respond']>[0];
    await until(
      'exact native approval acknowledgement',
      async () => (await client['native.respond'](input)).outcome === 'submitted',
      10_000,
    );
  }
}
async function settle(client: Client, target: ManagedPeer, outcome: string): Promise<void> {
  await until(`native ${outcome}`, async () => {
    ...
    const result = await client['session.wait']({ target, timeoutMs: 1_000 });
...
export async function verifyOpenCodeActions(client: Client, target: ManagedPeer): Promise<void> {
  ...
  let frame = await client['native.read']({ target });
  await until('native question request', async () => {
    frame = await client['native.read']({ target });
  ...
  try {
    await client['native.respond']({ ...response, generation: crypto.randomUUID() });
  } catch (error) {
  ...
  check(
    (await client['native.read']({ target })).pending.some(
  ...
  await until(
    ...
    async () => (await client['native.respond'](response)).outcome === 'submitted',
    10_000,
  );
  check(
    (await client['native.respond'](response)).outcome === 'submitted',
  ...
  await until('working tool before defer', async () => {
    await approve(client, target);
    const state = await client['session.get']({ target });
    return (
      state.state === 'working' &&
      (await client['native.read']({ target })).baseline.some(
        (item) =>
  ...
  check(
    (await client['session.wait']({ target, timeoutMs: 1_000 })).outcome === 'timeout',
    'Busy/deferred wait settled early',
  );
  check(
    (await client['session.get']({ target })).turn?.id === busy.turn?.id,
  ...
  const active = await client['session.get']({ target });
  check(active.turn, 'No exact native turn');
  await client['turn.interrupt']({
    target,
    generation: (await client['native.read']({ target })).generation,
