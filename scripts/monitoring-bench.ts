...
import { observeOnce, type Observed } from "../src/events/observe.ts";
...
async function read(): Promise<string> {
  ...
  const proc = Bun.spawn([process.execPath, cli, "status", "--json"], {
    env: { ...process.env, CCMUX_CONFIG: config }, stdout: "pipe", stderr: "pipe", stdin: "ignore",
  });
...
while (Date.now() - started < seconds * 1000) {
  await writeSessionsUnlocked(machine, loadSessions(actual).map((session) => ({ ...session, archived: true })));
  ...
  if (passes === 1) for (let i = 0; i < 10; i++) await Promise.all(Array.from({ length: 10 }, () => read()));
  maxRss = Math.max(maxRss, process.memoryUsage().rss);
  if (passes % 15 === 0) console.log(JSON.stringify({ elapsedSeconds: Math.round((Date.now() - started) / 1000), reads, passes, rows }));
  await Bun.sleep(2000);
}
publisher.stop();
const cpu = process.cpuUsage(cpuStart);
const percentile = (values: number[], p: number): number => [...values].sort((a, b) => a - b)[Math.floor((values.length - 1) * p)] ?? 0;
const result = {
  elapsedMs: Date.now() - started, reads, passes, rows,
  ...
  observationExecs: observationExecCount(), readerExecs: reads,
  latencyP50Ms: percentile(latencies, 0.5), latencyP95Ms: percentile(latencies, 0.95),
  freshnessP95Ms: percentile(ages, 0.95), freshnessMaxMs: Math.max(...ages), maxRssBytes: maxRss,
};
