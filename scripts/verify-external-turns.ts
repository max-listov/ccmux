...
if (Bun.argv[2] !== "--run") {
  console.error("usage: bun scripts/verify-external-turns.ts --run (creates and archives two test threads; uses installed ccmux)");
  process.exit(2);
}
...
const ThreadResponse = z.object({ thread: z.object({ id: z.uuid(), status: z.object({ type: z.string() }) }) });
...
async function start(threadId: string, text: string): Promise<void> {
  const response = TurnResponse.parse(await rpc.request("turn/start", {
...
async function snapshot(label: string) {
  ...
  try {
    const [out, err] = await Promise.all([new Response(process.stdout).text(), new Response(process.stderr).text()]);
    ...
    if (inventory.version !== VERSION) throw new Error("installed CLI version does not match checkout");
    ...
    console.log(JSON.stringify({ label, version: inventory.version, rows: rows.map((s) => ({
      thread: s.threadId === ids[0] ? "test-A" : "test-B",
      writer: s.writerEvidence, runtime: s.writerRuntime?.kind, turn: s.turnState,
    })) }));
    return rows;
...
function sharedWriter(rows: Awaited<ReturnType<typeof snapshot>>): boolean {
  ...
  return typeof pid === "number" && rows.every((s) => s.writerEvidence === "observed"
    && s.writerRuntime?.kind === "shared" && s.writerRuntime.pid === pid);
}

try {
  for (let i = 0; i < 2; i++) {
    const response = ThreadResponse.parse(await rpc.request("thread/start", {
      cwd: root, approvalPolicy: "never", sandbox: "read-only",
      baseInstructions: "You are a lifecycle test agent. Follow the explicit instruction. Do not modify files.",
  ...
  await start(a, "Run the shell command sleep 25 exactly once, wait for it, then reply DONE. Do not modify files.");
  ...
  while (Date.now() < deadline) {
    ...
    if (rows.some((s) => s.threadId === a && s.turnState.state === "working")
  ...
  for (let i = 0; i < 15; i++) {
    ...
    if (rows.every((s) => s.turnState.state === "idle") && sharedWriter(rows)) { idle = true; break; }
    await Bun.sleep(500);
  }
  ...
  if (!rows.every((s) => s.turnState.state === "idle") || !sharedWriter(rows)) throw new Error("reconnect proof failed");
} finally {
  let cleanupFailed = false;
  for (const threadId of ids) {
    const turnId = turns.get(threadId);
    if (turnId) await rpc.request("turn/interrupt", { threadId, turnId }).catch(() => {});
    await rpc.request("thread/archive", { threadId }).catch(() => { cleanupFailed = true; });
  }
  rpc.close();
  if (cleanupFailed) throw new Error("test thread archive failed; inspect provider test threads before rerunning");
}
