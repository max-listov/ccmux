...
`;
  ...
  const warningObserved =
    executed.stderr.includes('running under ssh without a managed sender') &&
    executed.stderr.includes('cannot reply to the originating agent');
  const deliveryObserved =
    executed.stdout.includes('sent ccmux/cli@host-a') &&
  ...
  if (remoteDir !== null) {
    ...
    if (cleanup !== null && cleanup.code !== 0)
      cleanupErrors.push(new Error('remote cleanup failed'));
  }
  try {
