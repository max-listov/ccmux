...
if (!basename(root).startsWith("ccmux-owned-probe-")) throw new Error("Reuse requires an isolated owned-runtime probe directory");
const config = reuse ?? join(root, "machine.json");
const work = process.cwd();
const machine = MachineConfigSchema.parse({ ...(reuse === undefined ? loadMachineConfig() : MachineConfigSchema.parse(JSON.parse(readFileSync(config, "utf8")))),
  stateDir: join(root, "state"), rcPrefix: "test", tmuxSocket: `ccmux-owned-${root.split("-").at(-1)}`,
  fleet: {}, wire: { peers: [] }, autoUpdate: false, chatEnabled: true, sessionEvents: true,
  remoteControl: false, codexCorrelationTimeoutMs: 45_000, telegram: undefined,
});
writeFileSync(config, JSON.stringify(machine), { mode: 0o600 });
const cli = process.argv[3] ?? join(work, "src/cli.ts");
const env: Record<string, string> = { CCMUX_CONFIG: config, CCMUX_STATE_DIR: machine.stateDir,
...
async function command(args: string[], timeout = 60_000): Promise<string> {
  const child = Bun.spawn([process.execPath, "--no-env-file", cli, ...args], { cwd: root, env,
...
if (reuse !== undefined) {
  ...
  await withSessionRegistryLock(machine, () => writeSessionsUnlocked(machine, loadSessions(machine).map((s) => ({
    ...s, flags: ["--sandbox", "workspace-write", "--ask-for-approval", "on-request", "--no-alt-screen"],
  }))));
}
for (const name of ["agent-a", "agent-b"]) {
  if (reuse === undefined) await command(["new", name, root, "--agent", "codex", "--runtime", "app-server", "--", "--sandbox", "workspace-write", "--ask-for-approval", "on-request", "--no-alt-screen"]);
  ...
  while (readOwnedCodexStatus(machine, session).status !== "live") {
    if (Date.now() >= deadline) throw new Error("Native session has no live resident status after admission");
    await Bun.sleep(200);
  }
  ...
  try {
    ...
    if (resident.snapshot?.threadId !== thread.id) throw new Error("Native and resident identities disagree");
    console.log(JSON.stringify({ phase: "ready", name, uuid: session.uuid, native: thread.status, resident }));
  } finally { rpc.close(); }
}
await newSession(machine, "probe-daemon", root, [process.execPath, "--no-env-file", cli, "daemon"], env);
console.log(JSON.stringify({ phase: "launched", config, sessions: loadSessions(machine).map(({ name, uuid }) => ({ name, uuid })) }));
