...
if (config === undefined || !basename(dirname(config)).startsWith("ccmux-owned-probe-")) throw new Error("Pass an isolated runtime probe config");
...
if (m.stateDir !== join(root, "state") || m.telegram !== undefined || Object.keys(m.fleet ?? {}).length || !m.sessionEvents) throw new Error("Probe must be isolated with events enabled");
...
async function until(label: string, condition: () => boolean, timeout = 60_000) {
  ...
  while (!condition()) { check(Date.now() < deadline, `Timed out: ${label}`); await Bun.sleep(50); }
}

const baseline = snapshot(); check(baseline?.state === "idle", "Known positive live idle required");
let rpc = await connectOwnedCodex(m, s);
check((await readCodexAppThread(rpc, s.uuid)).status.type === "idle", "Native baseline not idle");
check(inspectNativeCodexInput(await capturePaneStyled(m, s.name, 40)).state === "deliverable", "Interactive composer not ready");
...
check(await sendKeysLiteral(m, s.name, "Reply SAME_WRITER_TEST briefly, without tools or messages."), "Interactive typing failed");
...
await until("interactive client started a native turn", () => snapshot()?.turn?.id !== baseline.turn?.id && snapshot()?.state === "working");
const interactiveTurn = snapshot()?.turn?.id;
check((await readCodexAppThread(rpc, s.uuid)).status.type === "active", "Interactive turn missing in native RPC");
await until("interactive completion", () => snapshot()?.turn?.id === interactiveTurn && snapshot()?.state === "idle");
check(snapshot()?.providerPid === baseline.providerPid, "Interactive client launched another writer");
...
check(snapshot()?.providerPid === baseline.providerPid && snapshot()?.turn?.id === active, "Daemon restart replaced provider or turn");
await until("turn survived daemon restart", () => snapshot()?.turn?.id === active && snapshot()?.state === "idle");
...
const args = Bun.spawnSync(["ps", "-p", String(baseline.providerPid), "-o", "args="], { stdout: "pipe", stderr: "pipe" });
check(args.exitCode === 0 && args.stdout.toString().includes(`app-server --listen unix://${ownedCodexSocket(m, s.name)}`), "Crash target does not match isolated provider");
rpc.close();
const killedAt = performance.now(); process.kill(baseline.providerPid, "SIGKILL");
...
await until("supervisor resumes pinned UUID", () => {
  const next = snapshot(); return next !== null && next.providerPid !== baseline.providerPid && next.generation !== baseline.generation && next.state === "idle";
});
check(loadSessions(m).find((row) => row.name === s.name)?.uuid === s.uuid, "Crash recovery changed UUID");
rpc = await connectOwnedCodex(m, s);
check((await readCodexAppThread(rpc, s.uuid)).id === s.uuid, "Recovered native identity mismatch");
rpc.close();
progress("provider-crash-resume", { threadId: s.uuid, beforePid: baseline.providerPid, afterPid: snapshot()?.providerPid, expiredMs });

process.env.CCMUX_CONFIG = config;
process.env.CCMUX_STATE_DIR = m.stateDir;
const benchPid = snapshot()?.providerPid;
let completedAt: number | null = null, deliveredAfterMs: number | null = null, loadTurn: string | null = null;
let sawWorking = false, sawCompletion = false;
rpc = await connectOwnedCodex(m, s, { onEvent(event) {
...
try { while (performance.now() - started < 60_000) {
  ...
  if (loadTurn !== null && observed?.turn?.id === loadTurn) {
    ...
    if (observed.turn.status === "completed") {
      sawCompletion = true;
      if (deliveredAfterMs === null && completedAt !== null) deliveredAfterMs = Date.now() - completedAt;
    }
  }
  calls += batch.length;
...
check(sawWorking && sawCompletion && deliveredAfterMs !== null,
...
progress("resident-load", { elapsedMs: performance.now() - started, calls, cpu: process.cpuUsage(cpu), initialRss, peakRss, finalRss: process.memoryUsage().rss, maxBytes, providerPid: benchPid, deliveredAfterMs });
